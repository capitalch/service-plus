        ...
        <div className="flex items-center gap-1">
            <button
                ...
                className={`group flex flex-1 items-center gap-2 rounded px-2 py-2 transition-colors duration-150 ${
                    disabled
                        ? 'cursor-not-allowed text-left text-(--cl-text-muted) opacity-40'
                        : isActive
                            ? 'cursor-pointer bg-(--cl-accent) text-left text-white shadow-md'
                            : 'cursor-pointer text-left text-(--cl-text-muted) hover:bg-(--cl-hover) hover:text-(--cl-text)'
                }`}
            >
...
export const ClientExplorerPanel = ({ activeSection }: Props) => {
    ...
    return (
        <aside id="client-explorer" aria-label={`${SECTION_TITLES[activeSection]} navigation`} className={`fixed top-12 z-30 flex h-[calc(100%-4.5rem)] w-64 flex-col border-r border-(--cl-border) bg-(--cl-surface) transition-transform duration-200 ease-in-out left-0 md:left-16 ${translateClass}`}>
