...
export const ClientLayout = ({ children }: ClientLayoutProps) => {
    ...
    useEffect(() => {
        ...
        void apolloClient.query<{ genericQuery: { setting_key: string; setting_value: unknown }[] }>({
            ...
            if (typeof rawTerms === 'string') { try { parsedTerms = JSON.parse(rawTerms); } catch { /* keep raw */ } }
        ...
        <PortalContainerContext.Provider value={themeRoot}>
            <div ref={setThemeRoot} className="client-theme relative h-full bg-(--cl-bg) text-(--cl-text)" data-theme={isDark ? 'dark' : 'light'}>
                ...
                <main className={`absolute bottom-6 right-0 top-9.5 flex flex-col p-4 pt-3 sm:p-6 sm:pt-4 transition-[left] duration-200 ${mainLeft}`}>
                    <div className="mb-3 flex items-center justify-between gap-4 sm:mb-2">
                        <p aria-live="polite" className="text-xs font-bold tracking-wider text-(--cl-accent-text)">
                            {displayTitle}
                        </p>
                        {currentDivision && (
                            <div aria-label={`Working in ${currentDivision.name}, ${isGstMode ? "GST" : "Non-GST"} mode`} className="flex items-center gap-2 rounded-md bg-indigo-50 px-2.5 py-1.5 leading-none dark:bg-indigo-950/40">
                                ...
                                <span className="hidden rounded-sm bg-indigo-50 px-2 py-0.5 text-[10px] tracking-wide text-indigo-900 md:inline">
                                    Default division
                                </span>
                            </div>
                        )}
