...
class SeedBuData:
    ...
    BU_SEED_SQL = """
        ...
        INSERT INTO app_setting (id, setting_key, setting_value, description, is_editable) VALUES
            ...
            (16, 'extended_warranty_notifications_enabled', 'false', 'Master switch for the Extended Warranty add-on: shows the Custom -> Extended Warranty menu. Sending also requires whatsapp_notifications.EXTENDED_WARRANTY', true),
            (17, 'extended_warranty',                '{"auto_send_enabled": false, "contact_phone": "", "daily_send_cap": 250, "notify_email": "", "reminder_days_before": [30, 7, 0], "staff_whatsapp_number": "", "whatsapp_number": ""}', 'Extended Warranty configuration: reminder stages, daily send cap, the phone/WhatsApp numbers printed in the customer message, and the staff number that receives lead alerts', true)
        ON CONFLICT (id) DO NOTHING;
    """
