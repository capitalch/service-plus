...
class AppMessages:
    ...
    BASIC_TIER_USER_LIMIT = "This client is on the Basic plan, which allows only one business user"
    BASIC_TIER_ROLE_RESTRICTED = "This client is on the Basic plan — its one business user must have the Manager role"
    MANAGER_ROLE_RESTRICTED = "A Manager cannot create another Manager"
    MANAGER_BU_NOT_OWNED = "You can only create users for a BU you manage"
    BRANCH_NOT_IN_BU = "One or more selected branches don't belong to this BU"
    BASIC_TIER_BRANCH_LIMIT = "This client is on the Basic plan, which allows only one branch per BU"
