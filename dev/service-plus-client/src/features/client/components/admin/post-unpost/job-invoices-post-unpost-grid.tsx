...
export function JobInvoicesPostUnpostGrid({ pendingChanges, onChangeToggle, onStatsLoaded, onRowsLoaded, refreshTrigger }: Props) {
        ...
        try {
            ...
            const [dataRes, countRes, statsRes] = await Promise.all([
                apolloClient.query<GenericQueryData<JobInvoicePostUnpostRow>>({
                    ...
                    variables: { db_name: dbName, schema, value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_INVOICES_POST_UNPOST_PAGED, sqlArgs: { ...args, limit: PAGE_SIZE, offset: (pg - 1) * PAGE_SIZE } }) },
                }),
                apolloClient.query<GenericQueryData<{ total: number }>>({
                    fetchPolicy: "network-only",
                    query: GRAPHQL_MAP.genericQuery,
                    variables: { db_name: dbName, schema, value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_INVOICES_POST_UNPOST_COUNT, sqlArgs: args }) },
                }),
                apolloClient.query<GenericQueryData<PostUnpostStats>>({
                    fetchPolicy: "network-only",
                    query: GRAPHQL_MAP.genericQuery,
                    variables: { db_name: dbName, schema, value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_INVOICES_POST_UNPOST_STATS, sqlArgs: args }) },
                }),
            ]);
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border)">
                <div className="flex-1 overflow-x-auto overflow-y-auto">
                    {loading ? (
                        <table className="min-w-full border-collapse"><tbody>{Array.from({ length: 8 }).map((_, i) => (<tr key={i} className="animate-pulse">{Array.from({ length: 9 }).map((__, j) => (<td key={j} className={tdClass}><div className="h-4 w-20 rounded bg-(--cl-border)" /></td>))}</tr>))}</tbody></table>
                    ) : rows.length === 0 ? (
