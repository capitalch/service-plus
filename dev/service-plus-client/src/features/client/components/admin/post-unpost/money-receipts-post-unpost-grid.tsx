...
import type { MoneyReceiptPostUnpostRow, PostUnpostStats } from "./post-unpost-schema";
...
export function MoneyReceiptsPostUnpostGrid({ pendingChanges, onChangeToggle, onStatsLoaded, onRowsLoaded, refreshTrigger }: Props) {
    ...
    const loadData = useCallback(async (bId: number, q: string, pg: number) => {
        ...
        try {
            ...
            const [dataRes, countRes, statsRes] = await Promise.all([
                apolloClient.query<GenericQueryData<MoneyReceiptPostUnpostRow>>({
                    fetchPolicy: "network-only",
                    query: GRAPHQL_MAP.genericQuery,
                    variables: { db_name: dbName, schema, value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_PAYMENTS_POST_UNPOST_PAGED, sqlArgs: { ...args, limit: PAGE_SIZE, offset: (pg - 1) * PAGE_SIZE } }) },
                }),
                apolloClient.query<GenericQueryData<{ total: number }>>({
                    fetchPolicy: "network-only",
                    query: GRAPHQL_MAP.genericQuery,
                    variables: { db_name: dbName, schema, value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_PAYMENTS_POST_UNPOST_COUNT, sqlArgs: args }) },
                }),
                apolloClient.query<GenericQueryData<PostUnpostStats>>({
                    fetchPolicy: "network-only",
                    query: GRAPHQL_MAP.genericQuery,
                    variables: { db_name: dbName, schema, value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_PAYMENTS_POST_UNPOST_STATS, sqlArgs: args }) },
                }),
            ]);
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
            <div className="flex flex-wrap items-center gap-2 py-2">
                <div className="relative flex-1 sm:max-w-md">
                    ...
                    <Input className="h-8 border-(--cl-border) bg-(--cl-surface) pl-8 text-xs" placeholder="Job no, customer, receipt, mode" value={search} onChange={e => handleSearchChange(e.target.value)} />
                    {search && (
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border)">
                <div className="flex-1 overflow-x-auto overflow-y-auto">
                    {loading ? (
                        ...
                        <div className="flex h-32 items-center justify-center text-sm text-(--cl-text-muted)">No money receipts found.</div>
                    ) : (
