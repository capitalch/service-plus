...
export const StateSection = () => {
	...
	return (
		<>
							...
							<Table>
								<TableHeader>
									<TableRow className="sticky top-0 z-10 bg-(--cl-surface-3) hover:bg-(--cl-surface-3)">
										...
										<TableHead className={thSortClass} onClick={() => handleSort("country_code")}>
											Country
											<SortIcon col="country_code" />
										</TableHead>
										<TableHead className={thSortClass} onClick={() => handleSort("gst_state_code")}>
											GST Code
											<SortIcon col="gst_state_code" />
										</TableHead>
										...
										displayStates.map((st, idx) => (
											...
											>
												...
												<TableCell>
													...
													>
														<span
															className={`mr-1 h-1.5 w-1.5 rounded-full ${st.is_active ? "bg-emerald-500" : "bg-slate-400"}`}
														/>
			...
			{/* ── Dialogs ──────────────────────────────────────────────────────── */}
			<AddStateDialog open={addOpen} onOpenChange={setAddOpen} onSuccess={loadStates} />
			{editState && (
				<EditStateDialog
					state={editState}
					open={!!editState}
