...
import { AddFinancialYearDialog } from "./add-financial-year-dialog";
...
export const FinancialYearSection = () => {
	...
	return (
		<>
							...
							<Table>
								<TableHeader>
									<TableRow className="sticky top-0 z-10 bg-(--cl-surface-3) hover:bg-(--cl-surface-3)">
										...
										<TableHead className={thSortClass} onClick={() => handleSort("id")}>
											Year
											<SortIcon col="id" />
										</TableHead>
										<TableHead className={thSortClass} onClick={() => handleSort("start_date")}>
											Start Date
											<SortIcon col="start_date" />
										</TableHead>
										<TableHead className={thSortClass} onClick={() => handleSort("end_date")}>
											End Date
											<SortIcon col="end_date" />
										</TableHead>
			...
			{/* ── Dialogs ──────────────────────────────────────────────────────── */}
			<AddFinancialYearDialog open={addOpen} onOpenChange={setAddOpen} onSuccess={loadFys} />
			{editFy && (
