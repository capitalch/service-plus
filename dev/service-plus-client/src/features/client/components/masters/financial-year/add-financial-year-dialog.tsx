...
export const AddFinancialYearDialog = ({ onOpenChange, onSuccess, open }: AddFinancialYearDialogPropsType) => {
	...
	const debouncedId = useDebounce(idValue, FIELD_VALIDATION_DEBOUNCE_MS);
	...
	useEffect(() => {
		const year = Number(debouncedId);
		if (!year || !dbName || !schema) {
			setIdTaken(null);
			return;
		}
		if (form.getFieldState("id").invalid) {
			setIdTaken(null);
			return;
		}
		setCheckingId(true);
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent aria-describedby={undefined} className="sm:max-w-sm">
				<DialogHeader>
					<DialogTitle className="text-base font-semibold text-foreground">Add Financial Year</DialogTitle>
				</DialogHeader>
