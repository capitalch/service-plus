...
export const EditTechnicianDialog = ({
	...
	useEffect(() => {
		...
		apolloClient
			...
			.then((res) => {
				...
				setCodeTaken(exists);
				if (exists)
					form.setError("code", { message: MESSAGES.ERROR_TECHNICIAN_CODE_EXISTS_EDIT, type: "manual" });
	...
	const submitDisabled =
		checkingCode || codeTaken === true || Object.keys(errors).length > 0 || form.formState.isSubmitting;

	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent aria-describedby={undefined} className="sm:max-w-lg">
				<DialogHeader>
					<DialogTitle className="text-base font-semibold text-foreground">Edit Technician</DialogTitle>
				</DialogHeader>
				...
				<form className="flex flex-col gap-4 pt-1" onSubmit={form.handleSubmit(onSubmit)}>
					...
					<div className="grid grid-cols-2 gap-4">
						...
						<div className="flex flex-col gap-1.5">
							...
							<Input autoComplete="off" id="et_name" placeholder="Full name" {...form.register("name")} />
					...
					<div className="flex flex-col gap-1.5">
						...
						<Input id="et_leaving" type="date" {...form.register("leaving_date")} />
					</div>
