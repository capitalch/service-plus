...
export const TechnicianSection = () => {
	...
	return (
		<>
							...
							<Table>
								<TableHeader>
									<TableRow className="sticky top-0 z-10 bg-(--cl-surface-3) hover:bg-(--cl-surface-3)">
										...
										<TableHead className={thSortClass} onClick={() => handleSort("specialization")}>
											Specialization
											<SortIcon col="specialization" />
										</TableHead>
										...
										displayTechnicians.map((technician, idx) => (
											...
											>
												...
												<TableCell>
													...
													>
														<span
															className={`mr-1 h-1.5 w-1.5 rounded-full ${technician.is_active ? "bg-emerald-500" : "bg-slate-400"}`}
														/>
			...
			{/* ── Dialogs ──────────────────────────────────────────────────────── */}
			<AddTechnicianDialog branches={branches} open={addOpen} onOpenChange={setAddOpen} onSuccess={loadData} />
			{editTechnician && (
