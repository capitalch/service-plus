...
export const DeleteVendorDialog = ({
    ...
    const deleteEnabled = inUse === false && !checkingInUse && confirmValue.toLowerCase() === vendor.name.toLowerCase();
    ...
    async function handleDelete() {
        ...
        try {
            ...
            toast.success(MESSAGES.SUCCESS_VENDOR_DELETED);
            onSuccess();
            onOpenChange(false);
        } catch {
            toast.error(MESSAGES.ERROR_VENDOR_DELETE_FAILED);
        } finally {
            setSubmitting(false);
        }
    }
                ...
                <div className="flex flex-col gap-4">
                    ...
                    {inUse === true && (
                        <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
                            ...
                            <p className="text-sm text-amber-800">{MESSAGES.ERROR_VENDOR_DELETE_IN_USE}</p>
                        </div>
                    )}

                    {inUse !== true && (
                        <div className="flex flex-col gap-1.5">
                            <Label htmlFor="confirm_vendor_name">
                                Type{" "}
                                <span className="font-semibold text-slate-800">{vendor.name}</span>{" "}
                                to confirm
                            </Label>
