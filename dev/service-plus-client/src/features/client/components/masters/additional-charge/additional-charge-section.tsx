		...
		>
			<DialogContent className="sm:max-w-sm">
				...
				<DialogFooter>
					...
					<Button className="bg-red-600 hover:bg-red-700 text-white" disabled={saving} onClick={onConfirm}>
...
export const AdditionalChargeSection = () => {
			...
			await apolloClient.mutate({
				...
				variables: {
					...
					value: encodeObj({
						...
						xData: { id, name: addName.trim(), hsn_code, isIdInsert: true },
					}),
				},
			});
		...
		try {
			await apolloClient.mutate({
				...
				variables: {
					...
					value: encodeObj({
						tableName: "additional_charge",
						xData: { id: editRow.id, name: editName.trim(), hsn_code },
					}),
				},
			});
		...
		try {
			await apolloClient.mutate({
				...
				variables: {
					db_name: dbName,
					schema,
					value: encodeObj({ tableName: "additional_charge", deletedIds: [deleteRow.id] }),
				},
			});
	...
	const thClass =
		"sticky top-0 z-10 bg-(--cl-surface-2) px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-(--cl-text-muted) border-b border-(--cl-border)";
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-xs text-(--cl-text-muted)">
							Manage predefined additional charge names used in jobs.
						</p>
					</div>
				</div>
				<Button
			...
			<div className="min-h-0 flex-1 overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
				<div className="h-full overflow-y-auto">
					<table className="min-w-full border-collapse">
						<thead>
							<tr>
								<th className={thClass} style={{ width: "4%" }}>
									#
								</th>
								...
								Array.from({ length: 6 }).map((_, i) => (
									<tr key={i} className="animate-pulse">
										<td className={tdClass}>
											<div className="h-4 w-6 rounded bg-(--cl-border)" />
										</td>
										<td className={tdClass}>
								...
								<tr>
									<td colSpan={4} className="px-4 py-12 text-center text-sm text-(--cl-text-muted)">
										{search
											...
											: "No charges yet. Click Add Charge to create one."}
									</td>
								</tr>
							) : (
								filtered.map((row, idx) => (
									...
									>
										...
										<td className={`${tdClass} font-mono text-xs`}>
											{row.hsn_code ?? <span className="text-(--cl-text-muted)">—</span>}
										</td>
