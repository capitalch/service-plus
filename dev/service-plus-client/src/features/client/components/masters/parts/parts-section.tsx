...
export const PartsSection = () => {
	...
	const loadParts = useCallback(
		async (brand: BrandOption, currentPage: number, q: string) => {
			if (!dbName || !schema_) return;
		...
		<>
												...
												parts.map((part, idx) => (
													...
													>
														...
														<TableCell>
															...
															>
																<span
																	className={`mr-1 h-1.5 w-1.5 rounded-full ${part.is_active ? "bg-emerald-500" : "bg-amber-400"}`}
																/>
								...
								{totalPages > 1 && (
									<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
										<p className="text-xs text-(--cl-text-muted)">
											{total === 0
												? "No parts"
												: `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} parts (Page ${page} of ${totalPages})`}
										</p>
			...
			{dbName && schema_ && selectedBrand && (
				<ImportPartDialog
					...
					onSuccess={() => {
						setPage(1);
						loadParts(selectedBrand, 1, searchQ);
					}}
				/>
			)}
