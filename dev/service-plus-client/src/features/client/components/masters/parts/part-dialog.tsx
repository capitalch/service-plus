...
export type PartDialogProps = BaseProps & ({ mode: "add"; prefillCode?: string } | { mode: "edit"; part: PartType });
...
const schema = z
	.object({
		...
		hsn_code: z
			.string()
			.optional()
			.refine((v) => !v || (/^\d+$/.test(v) && [4, 6, 8].includes(v.length)), {
				message: "HSN must be 4, 6, or 8 digits",
			}),
		gst_rate: z.coerce
			.number()
			.min(0)
			.lt(60, "GST rate must be less than 60%")
			.optional()
			.or(z.literal(""))
			.transform((v) => (v === "" ? undefined : Number(v))),
	})
...
export const PartDialog = (props: PartDialogProps) => {
	...
	useEffect(() => {
		...
		if (!debouncedCode || !bid || !dbName || !schema_) {
			setCodeTaken(null);
			return;
		}
		if (form.getFieldState("part_code").invalid) {
			setCodeTaken(null);
			return;
		}
			...
			.query<CheckQueryDataType>({
				...
				variables: {
					...
					value: graphQlUtils.buildGenericQueryValue({
						...
						sqlId: isEdit ? SQL_MAP.CHECK_PART_CODE_EXISTS_EXCLUDE_ID : SQL_MAP.CHECK_PART_CODE_EXISTS,
					}),
				},
			})
			.then((res) => {
				...
				setCodeTaken(exists);
				if (exists)
					form.setError("part_code", {
						message: "This part code already exists for the selected brand.",
						type: "manual",
					});
	...
	async function onSubmit(data: FormType) {
		...
		try {
			...
			toast.error(
				isEdit ? "Failed to update part. Please try again." : "Failed to create part. Please try again.",
			);
		} finally {
			setSubmitting(false);
		}
	...
	const title = isEdit ? (defaultBrandId ? `Update Part: ${part!.part_code}` : "Edit Part") : "Add Part";
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				...
				<div className="flex items-center justify-between gap-4 pr-8 pb-3 border-b border-border">
					<DialogTitle className="text-base font-semibold text-foreground shrink-0">{title}</DialogTitle>
					<div className="flex items-center gap-2">
						...
						<Input
							disabled
							className="h-8 bg-slate-200 dark:bg-slate-800 font-extrabold text-slate-950 dark:text-slate-100 text-sm w-44 border-slate-300 dark:border-slate-700"
							value={brandName}
						/>
								...
								{...form.register("cost_price", {
									onChange: (e) => {
										...
										if (markupPct > 0 && cost > 0 && !isNaN(cost)) {
											...
											const mrp = (
												Math.round(sp * (1 + (gst > 0 ? gst : 0) / 100) * 100) / 100
											).toFixed(2);
							...
							<Input
								{...form.register("mrp", {
									onChange: (e) => {
										...
										if (mrp > 0 && !isNaN(mrp)) {
											const sp = (
												Math.round((mrp / (1 + (gst > 0 ? gst : 0) / 100)) * 100) / 100
											).toFixed(2);
