			...
			return (
				<div key={label} className="flex items-center gap-1">
					...
					{idx < STEP_LABELS.length - 1 && (
						<div
							className={`mb-3 h-px w-6 transition-colors ${isDone ? "bg-teal-300" : "bg-(--cl-border)"}`}
						/>
					)}
				</div>
			);
	...
	const processFile = async () => {
		...
		try {
			...
			for (const t of TARGET_FIELDS) {
				...
				const matched = cols.find((col) => col.toLowerCase().replace(/ /g, "_") === t.value);
		...
		switch (step) {
				...
				return (
					<div className="flex flex-col gap-4">
						...
						>
							...
							/>
							<UploadCloudIcon
								className={`h-10 w-10 ${dragOver ? "text-teal-500" : "text-(--cl-text-muted)"}`}
							/>
							<div className="text-center">
								<p className="text-sm font-medium text-(--cl-text)">
									Drop file here or click to browse
								</p>
								<p className="mt-0.5 text-xs text-(--cl-text-muted)">Supported: .csv, .xlsx, .xls</p>
							</div>
							{selectedFile && (
					...
					<div className="flex flex-col gap-3">
						...
						<div className="h-64 overflow-y-auto rounded-xl border border-(--cl-border)">
							<Table>
								...
								<TableBody>
									{rows.map((row, i) => (
										<TableRow key={i} className="border-b border-(--cl-border) last:border-b-0">
											{row.map((cell, j) => (
												<TableCell
													key={j}
			...
			case 4: {
						...
						<div className="flex flex-col justify-center gap-4 py-10">
							<div className="flex flex-col gap-2">
								<div className="flex items-center justify-between text-sm">
									<span className="text-(--cl-text)">{progressLabel || "Importing…"}</span>
				...
				return (
					<div className="flex flex-col gap-4">
						<p className="text-xs text-(--cl-text-muted)">
							Validation complete. You can proceed to import only the valid rows. Invalid rows will be
							ignored.
						</p>
											...
											<TableBody>
												{validRows.map((e, i) => (
													...
													>
														...
														<TableCell className="w-24 py-1.5 text-xs font-mono text-(--cl-text)">
															{e.part_code}
														</TableCell>
														<TableCell className="py-1.5 text-xs text-(--cl-text) truncate max-w-[200px]">
															{e.part_name}
														</TableCell>
													</TableRow>
												))}
