...
export const DeleteLookupDialog = ({ config, onOpenChange, onSuccess, open, record }: DeleteLookupDialogPropsType) => {
	...
	const deleteEnabled =
		...
		inUse === false &&
		!checkingInUse &&
		confirmValue.toLowerCase() === record.name.toLowerCase();
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						Permanently delete <span className="font-semibold text-foreground">{record.name}</span>?
					</DialogDescription>
				</DialogHeader>
					...
					{!record.is_system && inUse !== true && (
						<div className="flex flex-col gap-1.5">
							<Label htmlFor="confirm_lookup_name">
								Type <span className="font-semibold text-foreground">{record.name}</span> to confirm
							</Label>
							<Input
								autoComplete="off"
