...
export const LookupSection = ({ config }: LookupSectionProps) => {
			...
			await Promise.all([
				apolloClient.mutate({
					...
					variables: {
						...
						value: graphQlUtils.buildGenericUpdateValue({
							tableName: config.tableName,
							xData: { id: a.id, display_order: bOrder },
						}),
					},
				}),
				apolloClient.mutate({
					...
					variables: {
						...
						value: graphQlUtils.buildGenericUpdateValue({
							tableName: config.tableName,
							xData: { id: b.id, display_order: aOrder },
						}),
					},
				}),
			]);
	...
	return (
		<>
							...
							<Table>
								<TableHeader>
									<TableRow className="sticky top-0 z-10 bg-(--cl-surface-3) hover:bg-(--cl-surface-3)">
										...
										{config.hasIsActive && <TableHead className={thClass}>Status</TableHead>}
										{config.hasSystemFlag !== false && (
											<TableHead className={thClass}>System</TableHead>
										)}
										{canManage && <TableHead className={thClass}>Actions</TableHead>}
									</TableRow>
								</TableHeader>
								<TableBody>
											...
											>
																...
																(movingOrderId === record.id ? (
																	...
																	<div className="flex flex-col gap-0.5">
																		<button
																			...
																			disabled={
																				orderedRecords[0]?.id === record.id ||
																				movingOrderId !== null
																			}
																			title="Move up"
																			type="button"
																			onClick={() =>
																				void handleMoveOrder(record, "up")
																			}
																		>
																		...
																		<button
																			className="flex h-6 w-6 cursor-pointer items-center justify-center rounded border border-(--cl-border) bg-(--cl-surface-2) text-(--cl-text-muted) shadow-sm hover:border-(--cl-accent) hover:bg-(--cl-accent)/10 hover:text-(--cl-accent) disabled:cursor-not-allowed disabled:opacity-30 transition-colors"
																			disabled={
																				orderedRecords.at(-1)?.id ===
																					record.id || movingOrderId !== null
																			}
																			title="Move down"
																			type="button"
																			onClick={() =>
																				void handleMoveOrder(record, "down")
																			}
																		>
												...
												{config.hasIsActive && (
													<TableCell>
														...
														>
															<span
																className={`mr-1 h-1.5 w-1.5 rounded-full ${record.is_active ? "bg-emerald-500" : "bg-slate-400"}`}
															/>
			...
			{/* ── Dialogs ──────────────────────────────────────────────────────── */}
			{canManage && (
				<AddLookupDialog config={config} open={addOpen} onOpenChange={setAddOpen} onSuccess={loadData} />
			)}
			{editRecord && (
				<EditLookupDialog
					config={config}
