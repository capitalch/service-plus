...
export const EditLookupDialog = ({ config, onOpenChange, onSuccess, open, record }: EditLookupDialogPropsType) => {
	...
	const formSchema = useMemo(
		() =>
			z.object({
				...
				prefix: config.hasPrefix ? z.string().min(1, "Prefix is required").max(10) : z.string().optional(),
			}),
		[],
	...
	useEffect(() => {
		...
		if (debouncedCode.toUpperCase() === record.code) {
			setCodeTaken(false);
			return;
		}
