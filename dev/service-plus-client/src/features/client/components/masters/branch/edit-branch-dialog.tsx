...
const editBranchSchema = z.object({
	...
	gstin: z
		.string()
		.regex(/^[0-9A-Z]{15}$/, "Invalid GSTIN (15 characters)")
		.or(z.literal(""))
		.optional(),
...
export const EditBranchDialog = ({ branch, onOpenChange, onSuccess, open }: EditBranchDialogPropsType) => {
	...
	useEffect(() => {
		...
		if (debouncedName.toLowerCase() === branch.name.toLowerCase()) {
			setNameTaken(false);
			return;
		}
	...
	const submitDisabled =
		checkingName || Object.keys(errors).length > 0 || nameTaken === true || form.formState.isSubmitting;
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				<DialogHeader>
					<DialogTitle className="text-base font-semibold text-foreground">Edit Branch</DialogTitle>
				</DialogHeader>
				...
				<form className="flex flex-col gap-4 pt-1" onSubmit={form.handleSubmit(onSubmit)}>
					...
					<div className="flex flex-col gap-1.5">
						...
						<div className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
							<span className="font-mono text-sm font-semibold text-slate-700">{branch.code}</span>
						</div>
					...
					<div className="grid grid-cols-2 gap-4">
						...
						<div className="flex flex-col gap-1.5">
							...
							<Input autoComplete="off" id="ebr_city" placeholder="City" {...form.register("city")} />
						</div>
					</div>
