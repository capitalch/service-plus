...
export const PartLocationSection = () => {
	...
	const displayLocations = useMemo(() => {
		...
		if (search.trim()) {
			...
			rows = rows.filter(
				(r) => r.location.toLowerCase().includes(q) || (r.branch_name?.toLowerCase().includes(q) ?? false),
			);
		}
		if (sortCol) {
			rows = [...rows].sort((a, b) => {
		...
		<>
			...
			>
				...
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div>
						<h1 className="text-xl font-bold text-(--cl-text)">Part Locations</h1>
							...
							<Table>
								<TableHeader>
									<TableRow className="sticky top-0 z-10 bg-(--cl-surface-3) hover:bg-(--cl-surface-3)">
										...
										<TableHead className={thSortClass} onClick={() => handleSort("location")}>
											Location
											<SortIcon col="location" />
										</TableHead>
										<TableHead className={thSortClass} onClick={() => handleSort("branch_name")}>
											Branch
											<SortIcon col="branch_name" />
										</TableHead>
										...
										displayLocations.map((loc, idx) => (
											...
											>
												...
												<TableCell>
													...
													>
														<span
															className={`mr-1 h-1.5 w-1.5 rounded-full ${loc.is_active ? "bg-emerald-500" : "bg-slate-400"}`}
														/>
			...
			{/* ── Dialogs ──────────────────────────────────────────────────────── */}
			<AddPartLocationDialog open={addOpen} onOpenChange={setAddOpen} onSuccess={loadData} />
			{editLocation && (
