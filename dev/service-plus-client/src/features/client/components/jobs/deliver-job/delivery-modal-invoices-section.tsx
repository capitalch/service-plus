...
function computeTaxSummary(job: JobDeliveryFullDetail) {
	...
	for (const p of job.parts ?? []) {
		...
		if (p.gst_rate > 0) {
			if (isIgst) {
				...
				const half = (taxable * p.gst_rate) / 200;
				cgst += half;
				sgst += half;
			}
		}
	...
	for (const c of job.charges ?? []) {
		...
		if (c.gst_rate > 0) {
			if (isIgst) {
				igst += (taxable * c.gst_rate) / 100;
			} else {
				const half = (taxable * c.gst_rate) / 200;
				cgst += half;
				sgst += half;
			}
		}
	...
	return (
		<div className="flex items-center justify-between text-sm tabular-nums text-(--cl-text-muted)">
			<div className="flex items-center gap-3">
				<span>
					Taxable <span className="text-(--cl-text)">{fmtCurrency(tax.aggregate)}</span>
				</span>
				<span className="opacity-30">·</span>
				{tax.isIgst ? (
					<span>
						IGST <span className="text-(--cl-text)">{fmtCurrency(tax.igst)}</span>
					</span>
				) : (
					<>
						<span>
							CGST <span className="text-(--cl-text)">{fmtCurrency(tax.cgst)}</span>
						</span>
						<span className="opacity-30">·</span>
						<span>
							SGST <span className="text-(--cl-text)">{fmtCurrency(tax.sgst)}</span>
						</span>
					</>
				)}
				<span className="opacity-30">·</span>
				<span>
					Total <span className="font-semibold text-(--cl-text)">{fmtCurrency(tax.total)}</span>
				</span>
			</div>
			<span>
				Amt <span className="font-semibold text-(--cl-text)">{fmtCurrency(jobAmount)}</span>
			</span>
		</div>
	);
}

export function DeliveryModalInvoicesSection({
	jobs,
	availableDivisions,
	loadingPdfJobId,
	deletingInvoiceJobId,
	regeneratingInvoiceJobId,
	onPrintInvoice,
	onDeleteInvoice,
	onRegenerateInvoice,
}: Props) {
	return (
		<div className="space-y-3">
			{jobs.map((job) => {
				...
				const hasTaxLines =
					isGst &&
					((job.parts ?? []).some((p) => p.gst_rate > 0) || (job.charges ?? []).some((c) => c.gst_rate > 0));
