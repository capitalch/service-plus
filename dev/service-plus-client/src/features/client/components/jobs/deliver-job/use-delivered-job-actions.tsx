			...
			<>
				{undoPendingJob && (
					<Dialog
						open
						onOpenChange={(open) => {
							if (!open) setUndoPendingJob(null);
						}}
					>
						<DialogContent className="sm:max-w-sm bg-white dark:bg-zinc-950 border-(--cl-border)">
							...
							<div className="space-y-3 text-sm text-(--cl-text)">
								...
								<p className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-400">
									This will mark the job as <span className="font-bold">not delivered</span>. The job
									will return to the deliverable list.
								</p>
							</div>
										...
										try {
											await apolloClient.mutate({
												...
												variables: {
													db_name: dbName,
													schema,
													value: encodeObj({ job_id: undoPendingJob.id }),
												},
											});
											toast.success(
												`Delivery undone — Job #${undoPendingJob.job_no} restored to its previous status.`,
											);
											...
											const msg =
												(err as { graphQLErrors?: { message: string }[] })?.graphQLErrors?.[0]
													?.message ?? "Failed to undo delivery. Please try again.";
											toast.error(msg);
										} finally {
