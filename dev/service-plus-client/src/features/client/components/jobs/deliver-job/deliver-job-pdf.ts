...
function drawInvoiceContent(
	...
	const custLines = buildAddrLines([fullAddrLine, job.customer_gstin ? `GSTIN: ${job.customer_gstin}` : null]);

	const shipLines = buildAddrLines([job.customer_name ?? null, fullAddrLine]);
	...
	const totalTax = lines.reduce(
		(s, l) => s + Number(l.cgst_amount) + Number(l.sgst_amount) + Number(l.igst_amount),
		0,
	);
	...
	autoTable(doc, {
		...
		body: lines.map((l, i) => {
			...
			const subLines = [l.part_code ? `Part: ${l.part_code}` : null, l.hsn_code ? `HSN: ${l.hsn_code}` : null]
				.filter(Boolean)
				.join("  ");
	...
	const sumRows: { label: string; value: number; bold?: boolean }[] = [
		...
		{ label: "Igst:", value: invoice.igst_amount },
		{
			...
			value: invoice.aggregate + invoice.cgst_amount + invoice.sgst_amount + invoice.igst_amount,
			bold: false,
		},
...
export function buildPackedInvoicePdf(items: InvoiceItem[], branchName?: string | null, copies = 1): jsPDF {
	const allItems = copies > 1 ? items.flatMap((item) => Array<InvoiceItem>(copies).fill(item)) : items;
...
function drawDeliveryNoteHeader(
	...
	if (entityName) {
		...
		const hdrAddr = buildAddrLines([
			division?.address_line1,
			division?.address_line2,
			division?.city,
			division?.pincode,
		]);
		if (hdrAddr.length > 0) {
...
function drawDeliveryNoteContent(
	...
	const addrWrapped = addrParts.length > 0 ? (doc.splitTextToSize(addrParts.join(", "), colW) as string[]) : [];
...
function drawCombinedDeliveryNoteContent(
	...
	const addrParts = buildAddrLines([
		first.customer_address_line1 ?? null,
		first.customer_address_line2 ?? null,
		first.customer_landmark ?? null,
		first.customer_city ?? null,
...
export function buildReceiptPdf(
	...
	function drawContent(yOffset: number) {
		...
		const customerAddr = addrParts.length > 0 ? addrParts.join(", ") : (job.address_snapshot ?? null);
