...
import {
	deliveryModalSchema,
	getDeliveryModalDefaults,
	type DeliveryModalFormValues,
	type JobDeliveryFullDetail,
...
function StepSection({
	...
	return (
		<section
			className={`rounded-xl border border-(--cl-border) border-l-4 ${a.left} bg-(--cl-surface) shadow-sm overflow-hidden`}
		>
			<div className="flex items-center gap-3 px-4 py-3 bg-(--cl-surface-2)/50">
				<span
					className={`flex h-6 w-6 items-center justify-center rounded-full text-white text-xs font-bold shrink-0 ${a.bubble}`}
				>
					{step}
				</span>
				<span className={`text-sm font-bold uppercase tracking-widest ${a.title}`}>{title}</span>
				{count !== undefined && <span className="text-sm text-(--cl-text-muted) font-medium">({count})</span>}
			</div>
...
export function DeliveryModal({
	...
	useEffect(() => {
		...
		Promise.all(
			divIds.map((divId) =>
				apolloClient
					.query<GenericQueryData<DocumentSequenceRow>>({
						...
						variables: {
							...
							value: graphQlUtils.buildGenericQueryValue({
								...
								sqlArgs: { branch_id: branchId, division_id: divId },
							}),
						},
					})
	...
	useEffect(() => {
		...
		apolloClient
			.query<GenericQueryData<{ id: number; code: string }>>({
				fetchPolicy: "network-only",
				query: GRAPHQL_MAP.genericQuery,
				variables: {
					db_name: dbName,
	...
	const divLabel =
		singleDivId != null ? (availableDivisions.find((d) => d.id === singleDivId)?.name ?? "—") : "Multiple";
	const allGst = jobDetails.every((j) =>
		isGstDivision(availableDivisions.find((d) => d.id === j.division_id) ?? null),
	);
	const noneGst = jobDetails.every(
		(j) => !isGstDivision(availableDivisions.find((d) => d.id === j.division_id) ?? null),
	);
	const gstLabel = allGst ? "GST" : noneGst ? "Non-GST" : "Mixed";
	const totalAmt = jobDetails.reduce((s, j) => s + Number(j.amount ?? 0), 0);
	const totalPaid = jobDetails.reduce(
	...
	const isDelivered = jobDetails.some(
		(j) => j.job_status_code === "DELIVERED_OK" || j.job_status_code === "DELIVERED_NOT_OK",
	);
	...
	async function handleReceiptsDeliveryInvoice() {
		...
		if (badGstin) {
			toast.error(
				`Enter a valid 15-character GSTIN for Job #${badGstin.job_no}, or clear it, before delivering.`,
			);
			return;
		}

		const needReceipt = jobDetails.filter(
			(j) =>
				Number(j.amount ?? 0) > 0 &&
	...
	async function doDelivery() {
		...
		try {
			for (const job of jobDetails) {
				const statusId = job.job_status_code === "COMPLETED_OK" ? deliveredOkStatusId : deliveredNotOkStatusId;
	...
	async function handleInvoiceReceipt() {
		...
		if (items.length === 0) {
			toast.error("No invoices to show.");
			return;
		}
	...
	async function handleAddReceipt(values: AddReceiptFormValues) {
		...
		const divSeqs =
			(receiptJob.division_id != null ? divisionSequences.get(receiptJob.division_id) : undefined) ?? [];
	...
	return (
		<>
			...
			>
				...
				>
					...
					<Button
						...
						variant="ghost"
						onClick={() => {
							if (didDeliver.current) onDelivered();
							onClose();
						}}
					>
						...
						<div className="flex items-start justify-between gap-4">
									...
									{isSingleJob ? (
										<>
											<div className="mt-1 flex items-center gap-2 min-w-0">
												<span className="font-semibold text-(--cl-text) text-sm truncate">
													{firstJob.customer_name}
												</span>
												{firstJob.mobile && (
													<span className="text-sm text-(--cl-text-muted) shrink-0">
														{firstJob.mobile}
													</span>
												)}
											</div>
											{addressStr && (
												<p className="text-xs text-(--cl-text-muted) mt-0.5 truncate">
													{addressStr}
												</p>
											)}
										</>
							...
							<div className="flex flex-wrap items-start gap-2 justify-end shrink-0">
								<div className="flex items-center gap-1.5 rounded-lg border border-(--cl-border) bg-(--cl-surface-2) px-3 py-2">
									<span className="text-xs font-medium text-(--cl-text-muted) uppercase tracking-wide">
										Div
									</span>
								...
								<div className="flex items-center gap-0 rounded-lg border border-(--cl-border) overflow-hidden divide-x divide-(--cl-border)">
									<div className="flex items-center gap-1.5 bg-(--cl-surface-2) px-3 py-2">
										<span className="text-xs text-(--cl-text-muted) uppercase font-medium">
											Total
										</span>
										<span className="text-sm font-bold tabular-nums text-(--cl-text)">
									...
									>
										<span
											className={`text-xs uppercase font-medium ${totalDue > 0 ? "text-amber-600" : "text-emerald-600 dark:text-emerald-400"}`}
										>
											Due
										</span>
										<span
											className={`text-sm font-extrabold tabular-nums ${totalDue > 0 ? "text-amber-700 dark:text-amber-400" : "text-emerald-700 dark:text-emerald-400"}`}
										>
							...
							<div className="flex flex-wrap items-end gap-4">
								...
								<div className="flex-1 min-w-[200px] max-w-[280px]">
									...
									<Select
										value={mannerVal}
										onValueChange={(v) =>
											form.setValue("delivery_manner", v, { shouldValidate: true })
										}
					...
					receiptJob
						? Math.max(
								...
								Number(receiptJob.amount ?? 0) -
									(receiptJob.payments ?? []).reduce((s, p) => s + Number(p.amount), 0),
							)
						: 0
				}
				subtitle={
			...
			<AlertDialog
				...
				onOpenChange={() => {
					/* blocked — user must click Continue */
				}}
			>
			...
			<AlertDialog
				...
				onOpenChange={(open) => {
					if (!open) setConfirmRegenerateJob(null);
				}}
			>
							...
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									...
									<span className="font-semibold text-foreground">
										{confirmRegenerateJob.invoice_no}
									</span>
						...
						<>
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									...
									for job{" "}
									<span className="font-semibold text-foreground">
										#{confirmRegenerateJob?.job_no}
									</span>{" "}
									({confirmRegenerateJob?.customer_name}) will be regenerated from the current parts
									and charges. The invoice number will be preserved.
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
								...
								<AlertDialogAction
									onClick={() => {
										void executeRegenerateInvoice(confirmRegenerateJob!);
									}}
								>
									Regenerate
								</AlertDialogAction>
					...
					{confirmDeleteJob?.invoice_is_posted ? (
						<>
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									Invoice{" "}
									<span className="font-semibold text-foreground">{confirmDeleteJob.invoice_no}</span>
						...
						<>
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									Invoice{" "}
									<span className="font-semibold text-foreground">
										{confirmDeleteJob?.invoice_no}
									</span>{" "}
									for job{" "}
									<span className="font-semibold text-foreground">#{confirmDeleteJob?.job_no}</span> (
									{confirmDeleteJob?.customer_name}) will be permanently deleted. This cannot be
									undone.
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
								...
								<AlertDialogAction
									variant="destructive"
									onClick={() => {
										void executeDeleteInvoice(confirmDeleteJob!);
									}}
								>
									Delete Invoice
								</AlertDialogAction>
