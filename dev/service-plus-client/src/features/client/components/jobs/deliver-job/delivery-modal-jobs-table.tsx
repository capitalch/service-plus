		...
		<div className="flex flex-col gap-2">
			{jobs.map((job) => {
				...
				return (
					<div
						key={job.id}
						className="rounded-lg border border-(--cl-border) bg-(--cl-surface) px-4 py-3 flex flex-col gap-1.5"
					>
						{/* Row 1: job # · customer · mobile | job-type badge · date · view */}
						<div className="flex items-center gap-2 min-w-0">
							<span className="font-mono font-bold text-(--cl-accent) text-base shrink-0">
								#{job.job_no}
							</span>
							...
							<div className="flex items-baseline gap-2 flex-1 min-w-0">
								<span className="font-semibold text-(--cl-text) truncate text-base">
									{job.customer_name}
								</span>
								{job.mobile && (
							...
							<span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-(--cl-text-muted)">
								GSTIN
							</span>
							<Input
						...
						<div className="flex items-center flex-wrap gap-x-1.5 gap-y-0 text-sm text-(--cl-text-muted)">
							...
							{job.receive_condition_name && (
								<>
									...
									<span>{job.receive_condition_name}</span>
								</>
							)}
							{job.technician_name && (
								<>
									<span className="opacity-30">·</span>
									<span className="text-(--cl-text)">{job.technician_name}</span>
								</>
							)}
						</div>
						...
						<div className="flex items-center justify-end gap-4 pt-1.5 border-t border-(--cl-border)/50 text-sm tabular-nums">
							<span className="text-(--cl-text-muted)">
								Est <span className="text-(--cl-text)">{fmtCurrency(job.estimate_amount)}</span>
							</span>
							<span className="text-(--cl-text-muted)">
								Amt <span className="font-semibold text-(--cl-text)">{fmtCurrency(job.amount)}</span>
							</span>
			...
			{jobs.length > 1 && (
				<div className="flex items-center justify-between rounded-lg border border-(--cl-border) bg-(--cl-surface-2) px-4 py-2 text-sm tabular-nums">
					...
					<div className="flex items-center gap-4">
						<span className="text-(--cl-text-muted)">
							Amt <span className="font-bold text-(--cl-text)">{fmtCurrency(totalAmt)}</span>
						</span>
						<span className={`font-bold ${totalDue > 0 ? "text-amber-600" : "text-emerald-500"}`}>
							Due {fmtCurrency(totalDue)}
						</span>
					</div>
				</div>
			)}
