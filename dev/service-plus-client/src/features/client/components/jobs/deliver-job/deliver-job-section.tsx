...
export const DeliverJobSection = ({ onBack, initialTab }: DeliverJobSectionProps = {}) => {
		...
		const countPromise = apolloClient
			.query({
				...
				variables: {
					...
					schema,
					value: graphQlUtils.buildGenericQueryValue({
						sqlId: SQL_MAP.GET_DELIVERED_JOBS_COUNT,
						sqlArgs: args,
					}),
				},
			})
		...
		const rowsPromise = apolloClient
			.query({
				...
				variables: {
					...
					schema,
					value: graphQlUtils.buildGenericQueryValue({
						sqlId: SQL_MAP.GET_DELIVERED_JOBS_PAGED,
						sqlArgs: args,
					}),
				},
			})
	...
	function handleDeliveredSearchChange(value: string) {
		...
		deliveredDebounceRef.current = setTimeout(() => {
			setDeliveredPage(1);
			setDeliveredSearchQ(value);
		}, SEARCH_DEBOUNCE_MS);
	}
	...
	function handleDeliveredSelectionChange(row: DeliveredJobRow, checked: boolean) {
		setSelectedDeliveredRows((prev) => {
			...
			if (
				reference &&
				(reference.customer_contact_id !== row.customer_contact_id ||
					reference.delivery_date !== row.delivery_date)
			) {
	...
	async function handleOpenDeliveryModal() {
		...
		try {
			...
			const details = res.data?.genericQuery ?? [];
			if (details.length === 0) {
				toast.error("No job details found. Please try again.");
				return;
			}
			setModalJobDetails(details);
			setShowDeliveryModal(true);
	...
	return (
		...
		>
			...
			<div className="flex flex-wrap items-center gap-x-4 gap-y-3 border-b border-(--cl-border) bg-(--cl-surface) px-4 py-1">
				...
				{loadingModal && <Loader2 className="h-4 w-4 animate-spin text-(--cl-accent)" />}

				<div className="flex shrink-0 items-center gap-2 rounded-xl border-2 border-(--cl-border) bg-(--cl-surface-2) p-1 shadow-md mr-44">
					<button
						className={`h-9 gap-2 px-4 text-sm transition-transform duration-200 rounded-lg border-0 cursor-pointer ${
							activeTab === "deliverable"
				...
				<DeliveredJobsGrid
					...
					onUndoDelivery={(row) => {
						if (row.invoice_is_posted) {
							toast.error(
								`Cannot undo delivery — invoice ${row.invoice_no ? `#${row.invoice_no}` : ""} is already posted to accounts.`,
							);
							return;
						}
