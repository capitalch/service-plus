...
function modeBadgeClass(mode: string): string {
	switch (mode) {
		...
		default:
			return "bg-(--cl-surface-2) text-(--cl-text-muted)";
	}
}
	...
	return (
		<div className="space-y-3">
			{jobs.map((job) => {
				...
				const excess = Math.max(0, paid - Number(job.amount ?? 0));
				return (
					<div key={job.id} className="rounded-lg border border-(--cl-border) bg-(--cl-surface-2) px-4 py-3">
						...
						<div className="flex flex-wrap gap-5 mb-3 text-sm">
							...
							<span className="text-(--cl-text-muted)">
								Due:{" "}
								<strong className={due > 0 ? "text-amber-600" : "text-emerald-600"}>
									{fmtCurrency(due)}
								</strong>
							</span>
							{excess > 0.005 && (
								<span className="text-(--cl-text-muted)">
									Excess: <strong className="text-amber-700">{fmtCurrency(excess)}</strong>
								</span>
							)}
						</div>
						...
						{excess > 0.005 && (
							<div className="mb-3 flex items-start gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-300">
								...
								<span>
									Receipts (<strong>{fmtCurrency(paid)}</strong>) exceed this job&apos;s total (
									<strong>{fmtCurrency(Number(job.amount ?? 0))}</strong>) by{" "}
									<strong>{fmtCurrency(excess)}</strong>. {MESSAGES.WARN_EXCESS_PAYMENT_ACCOUNTING}
								</span>
							</div>
						)}
									...
									<thead>
										<tr className="bg-(--cl-surface-2)">
											{["Receipt No", "Date", "Mode", "Amount", "Ref No", "Remarks"].map((h) => (
												<th
													key={h}
													className={`px-3 py-2 text-xs font-semibold uppercase tracking-wide text-(--cl-text-muted) border-b border-(--cl-border) ${h === "Amount" ? "text-right" : "text-left"}`}
												>
													{h}
												</th>
											))}
									...
									<tbody className="divide-y divide-(--cl-border) bg-(--cl-surface)">
										{(job.payments ?? []).map((p, i) => (
											<tr key={p.id} className="hover:bg-(--cl-accent)/5">
												<td className="px-3 py-2 text-sm font-mono font-semibold text-(--cl-accent)">
													{p.receipt_no ?? `#${i + 1}`}
												</td>
												<td className="px-3 py-2 text-sm">{p.payment_date.slice(0, 10)}</td>
												<td className="px-3 py-2">
													<span
														className={`rounded px-2 py-0.5 text-xs font-semibold ${modeBadgeClass(p.payment_mode)}`}
													>
														{p.payment_mode}
													</span>
												</td>
												<td className="px-3 py-2 text-sm text-right tabular-nums font-semibold">
