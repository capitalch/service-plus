...
export const SingleJobViewModal = ({ isOpen, job, onClose, onPrint }: JobViewModalPropsType) => {
    ...
    useEffect(() => { setFileCount(job?.file_count ?? 0); }, [job]);
    ...
    return (
        <>
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 mb-1">
                                        ...
                                        {fileCount > 0 && (
                                            <button
                                                ...
                                                className="flex items-center gap-1 text-[10px] font-bold text-teal-600 bg-teal-50 dark:bg-teal-950/30 px-2 py-0.5 rounded-full border border-teal-200 dark:border-teal-800/30 hover:bg-teal-100 dark:hover:bg-teal-900/40 hover:border-teal-400 dark:hover:border-teal-700 transition-colors cursor-pointer"
                                            >
            ...
            <JobAttachDialog
                jobId={showAttach ? job.id : null}
                jobNo={job.job_no}
                mode="view"
                onClose={() => setShowAttach(false)}
                onFilesChanged={setFileCount}
            />
        </>
    );
};
