...
export const SingleJobSection = ({
	...
	onNavigateToBatchEdit?: (batchNo: number) => void;
	forceView?: boolean;
	...
	const jobSheetSequence = docSequences.find((ds) => ds.document_type_code === "JOB_SHEET" && ds.id != null) ?? null;
		...
		try {
			if (editJob) {
				...
				if (newDivId && editJob.division_id && newDivId !== editJob.division_id) {
					...
					if (!!oldDiv?.gstin !== !!newDiv?.gstin) {
						try {
							...
							/* allow proceed on query failure */
						}
					}
				}
				...
				const createRes = await apolloClient.mutate<{
					createSingleJob: { job_id: number; job_no: string } | null;
				}>({
				...
				toast.success(
					created?.job_no
						? `${MESSAGES.SUCCESS_JOB_CREATED} (${created.job_no})`
						: MESSAGES.SUCCESS_JOB_CREATED,
					created && branchId
						? {
								action: {
				...
				const [
					...
					apolloClient.query<GenericQueryData<JobLookupRow>>({
						...
						variables: {
							db_name: dbName,
							schema,
							value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_RECEIVE_CONDITIONS }),
						},
					}),
				...
				<div className="flex-1 overflow-y-auto">
					<FormProvider {...form}>
						<NewSingleJobForm
							...
							onAttachFiles={(jobNo: string, jobId: number) => {
				...
				<>
					...
					<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
								...
								<table className="min-w-full border-collapse">
									...
									<tbody className="divide-y divide-(--cl-border) bg-(--cl-surface)">
										{jobs.map((job, idx) => (
											<tr
												key={job.id}
												className={`group transition-colors hover:bg-(--cl-accent)/5 ${job.batch_no ? "border-l-2 border-l-violet-400 dark:border-l-violet-500" : ""}`}
											>
												...
												<td className={tdClass}>
													<div className="flex flex-col gap-0.5">
														...
														{job.batch_no && (
															<span className="text-[9px] font-bold text-violet-600 dark:text-violet-400 w-fit bg-violet-50 dark:bg-violet-950/40 rounded px-1 py-0.5">
																Batch #{job.batch_no}
															</span>
														)}
														{job.purchase_date && (
													...
													<div className="flex items-center justify-center">
														<DropdownMenu>
															<DropdownMenuTrigger asChild>
																<Button
																	className="h-8 w-8 p-0 hover:bg-(--cl-accent)/15"
																	variant="ghost"
																>
															...
															<DropdownMenuContent
																align="end"
																className="w-[140px] bg-white dark:bg-zinc-950 border-(--cl-border) shadow-[0_10px_30px_rgba(0,0,0,0.2)] z-50"
															>
																<DropdownMenuItem
																...
																<DropdownMenuItem
																	...
																	title={
																		job.transaction_count > 1
																			? "Cannot delete: job has activity"
																			: undefined
																	}
																	onClick={() => {
						...
						<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
							<span className="text-xs text-(--cl-text-muted)">
								{total === 0
									? "No jobs"
									: `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} job${total !== 1 ? "s" : ""} (Page ${page} of ${totalPages})`}
							</span>
					...
					>
						<DialogContent
							...
							className="sm:max-w-sm bg-white dark:bg-zinc-950 text-(--cl-text) shadow-2xl border border-(--cl-border)"
						>
							<DialogHeader>
								<DialogTitle>Delete Job</DialogTitle>
							</DialogHeader>
							<p className="text-sm text-(--cl-text-muted)">
								This will permanently delete the job and all associated records. This action cannot be
								undone.
							</p>
							<DialogFooter>
			...
			<PdfPreviewModal
				...
				onPrintCopiesChange={(n) => {
					...
					if (pendingPrintRef.current) {
						...
						setPdfPreviewUrl(
							getJobSheetBlobUrl(job, division ?? null, branchCode, n, {
								clientName,
								buName: currentBu?.name ?? null,
								trackJobUrl,
								termsAndConditions: jobTermsAndConditions,
