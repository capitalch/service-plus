			...
			<div className="flex flex-col gap-5 mt-2 bg-(--cl-surface) p-4 rounded-xl border border-(--cl-border) shadow-sm">
				...
				{!readOnly && (
					...
					>
						...
						>
							<CloudUpload
								className={`w-5 h-5 transition-colors ${isDragActive ? "text-(--cl-accent)" : "text-(--cl-text-muted) group-hover:text-(--cl-accent)"}`}
							/>
						</motion.div>
						<div className="flex flex-col flex-1 items-center md:items-start">
							<h4 className="text-sm font-semibold text-(--cl-text)">
								Drag & Drop files here{" "}
								<span className="font-normal text-(--cl-text-muted)">or click to browse</span>
							</h4>
							<div className="mt-1.5 flex flex-wrap justify-center md:justify-start items-center gap-1.5 text-[10px] font-medium text-(--cl-text-muted)/80">
								<span className="px-1.5 py-0.5 rounded bg-(--cl-surface) border border-(--cl-border)">
									JPEG
								</span>
								<span className="px-1.5 py-0.5 rounded bg-(--cl-surface) border border-(--cl-border)">
									PNG
									...
									>
										...
										<div className="flex-1 min-w-0 flex flex-col gap-1.5 pt-0.5">
											<div className="flex items-start justify-between gap-2">
												<p
													className="text-[11px] font-medium text-(--cl-text) truncate"
													title={pf.file.name}
												>
													{pf.file.name}
												</p>
											...
											>
												{isPdf ? (
													<div className="flex flex-col items-center gap-2 group-hover:scale-110 transition-transform duration-500 ease-out">
														...
														<span className="text-[9px] font-bold uppercase tracking-widest text-(--cl-text-muted)">
															PDF
														</span>
													</div>
												) : (
