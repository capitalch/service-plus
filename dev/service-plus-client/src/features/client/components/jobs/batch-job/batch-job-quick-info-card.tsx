...
export function BatchJobQuickInfoCard({ onAttachJob, onEdit, onView, onPrint, refreshTrigger }: Props) {
	...
	const fetchBatch = useCallback(
		async (off: number, nav = false) => {
			if (!dbName || !schema || !branchId) return;
			if (nav) setNavLoading(true);
				...
				<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 py-3 gap-3">
								...
								<div key={row.job_id} className="flex items-center gap-2 text-xs min-w-0">
									...
									{row.serial_no && (
										<>
											...
											<span className="text-(--cl-text-muted) font-mono shrink-0">
												{row.serial_no}
											</span>
										</>
									)}
					...
					<div className="flex items-center gap-2 shrink-0">
						...
						<div className="flex items-center gap-1.5 p-1.5 rounded-xl border border-(--cl-border) bg-(--cl-surface) shadow-sm">
							...
							/>
							{navLoading && (
								<Loader2 className="h-4 w-4 animate-spin text-(--cl-accent) ml-0.5 shrink-0" />
							)}
						</div>
					</div>
				</div>
