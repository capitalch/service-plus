...
export const BatchJobViewModal = ({
	isOpen,
	batchNo,
	jobs,
	loading,
		...
		<>
									...
									<tbody>
										{jobs.map((job, idx) => (
											<tr key={job.id} className="group hover:bg-(--cl-accent)/5">
												...
												<td
													className={`${tdClass} font-mono text-xs font-medium text-(--cl-accent)`}
												>
			...
			<JobAttachDialog
				...
				onFilesChanged={(count) => {
					if (onFileCountChange && attachJobId !== null) onFileCountChange(attachJobId, count);
				}}
			/>
		</>
	);
