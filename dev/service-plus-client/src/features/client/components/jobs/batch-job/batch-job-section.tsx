...
import type {
	...
	JobLookupRow,
	ModelRow,
} from "@/features/client/types/job";
...
export const BatchJobSection = ({
	...
	initialEditBatchNo?: number | null;
	...
	const pendingPrintRef = useRef<{
		jobs: JobDetailType[];
		division: ReturnType<typeof availableDivisions.find>;
		branchCode?: string;
	} | null>(null);
		...
		try {
			if (editBatchNo) {
				...
				const data = result.data as {
					createJobBatch?: { batch_no: number; job_ids: number[]; job_nos: string[] };
				};
	...
	useEffect(() => {
		...
		const fetchMeta = async () => {
			try {
					...
					await Promise.all([
						...
						apolloClient.query<GenericQueryData<JobLookupRow>>({
							...
							variables: {
								db_name: dbName,
								schema,
								value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_TYPES }),
							},
						}),
						apolloClient.query<GenericQueryData<JobLookupRow>>({
							fetchPolicy: "network-only",
							...
							variables: {
								db_name: dbName,
								schema,
								value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_RECEIVE_MANNERS }),
							},
						}),
						apolloClient.query<GenericQueryData<JobLookupRow>>({
							fetchPolicy: "network-only",
							...
							variables: {
								...
								value: graphQlUtils.buildGenericQueryValue({
									sqlId: SQL_MAP.GET_JOB_RECEIVE_CONDITIONS,
								}),
							},
						}),
				...
				setMasterStates(
					(stateRes.data?.genericQuery ?? []).map((s) => ({
						id: s.id,
						code: (s as { gst_state_code?: string }).gst_state_code ?? s.code,
						name: s.name,
					})),
				);
			} catch {
	...
	const loadData = useCallback(
		async (bId: number, q: string, pg: number) => {
			if (!dbName || !schema) return;
			setLoading(true);
			try {
				...
				const [dataRes, countRes] = await Promise.all([
					apolloClient.query<GenericQueryData<JobInBatchRow>>({
						...
						variables: {
							...
							schema,
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_JOB_BATCHES_WITH_JOBS_PAGED,
					...
					apolloClient.query<GenericQueryData<{ total: number }>>({
						...
						variables: {
							...
							schema,
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_JOB_BATCHES_WITH_JOBS_COUNT,
								sqlArgs: commonArgs,
							}),
						},
					}),
		...
		try {
			const res = await apolloClient.query<GenericQueryData<JobBatchDetailRow>>({
				...
				variables: {
					...
					schema,
					value: graphQlUtils.buildGenericQueryValue({
						sqlId: SQL_MAP.GET_JOB_BATCH_DETAIL,
						sqlArgs: { batch_no: batchNo },
					}),
				},
			});
	...
	return (
		...
		>
			...
			<div className="flex flex-wrap items-center gap-x-4 gap-y-3 border-b border-(--cl-border) bg-(--cl-surface) px-4 py-1">
				<div className="flex items-center gap-3 overflow-hidden">
					...
					<div className="flex items-baseline gap-2 overflow-hidden">
						<h1 className="text-lg font-bold text-(--cl-text) truncate">
							Batch Jobs
							{mode === "new" && !editBatchNo && (
								<span className="ml-2 text-sm font-medium text-(--cl-text-muted) whitespace-nowrap">
									— New
								</span>
							)}
							{mode === "new" && editBatchNo && (
								<span className="ml-2 text-sm font-medium text-amber-500 whitespace-nowrap">
									— Edit #{editBatchNo}
								</span>
							)}
							{mode === "view" && (
				...
				<div
					className={`flex items-center gap-2 ${mode !== "new" ? "hidden md:flex md:invisible pointer-events-none" : ""}`}
				>
					<Button
			...
			{mode === "new" ? (
				<div className="flex-1 overflow-y-auto pb-6 flex flex-col gap-4">
					...
					<BatchJobQuickInfoCard
						...
						onPrint={(batchNo) => {
							if (!dbName || !schema) return;
							void apolloClient
								.query<GenericQueryData<JobDetailType>>({
									fetchPolicy: "network-only",
									query: GRAPHQL_MAP.genericQuery,
									variables: {
										db_name: dbName,
					...
					<FormProvider {...form}>
						<NewBatchJobForm
							...
							onAttachFiles={(jobId, jobNo, rowIdx) => {
								setAttachJobId(jobId);
								setAttachJobNo(jobNo);
								setAttachRowIdx(rowIdx ?? null);
							}}
							form={form}
						/>
					</FormProvider>
				...
				<>
					...
					<div className="flex flex-wrap items-center gap-2 px-4 py-2 bg-(--cl-surface-2)/30">
						<div className="relative flex-1 sm:max-w-xs">
							...
							<Input
								...
								placeholder="Batch no, alt job no, customer or mobile…"
								value={search}
					...
					<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
								...
								<table className="min-w-full border-collapse">
									<thead className="sticky top-0 z-10">
										<tr>
											...
											<th className={`${thClass} sticky right-0 z-20 !bg-(--cl-surface-2)`}>
												Actions
											</th>
										</tr>
									</thead>
									<tbody className="divide-y divide-(--cl-border) bg-(--cl-surface)">
										{groupedBatches.map((batch, groupIdx) => (
											<BatchGroupRow
													...
													void apolloClient
														.query<GenericQueryData<JobDetailType>>({
															...
															variables: {
																...
																value: graphQlUtils.buildGenericQueryValue({
																	sqlId: SQL_MAP.GET_JOB_BATCH_DETAIL,
																	sqlArgs: { batch_no: batch.batch_no },
																}),
															},
														})
														.then((res) => {
												...
												onDelete={() => {
													setDeleteBatchNo(batch.batch_no);
													setDeleteJobCount(batch.job_count);
												}}
												onAttachJob={(jobId, jobNo) => {
													setAttachJobId(jobId);
													setAttachJobNo(jobNo);
												}}
												onDeleteJob={(jobId, jobNo, _batchNo, jobCount) => {
													setDeleteJobId(jobId);
													setDeleteJobNo(jobNo);
													setDeleteJobCount(jobCount);
												}}
												onWhatsappJobIntake={() => {
						...
						<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
							<span className="text-xs text-(--cl-text-muted)">
								{total === 0
									? "No batches"
									: `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} batch${total !== 1 ? "es" : ""} (Page ${page} of ${totalPages})`}
							</span>
					...
					{/* Delete Dialog */}
					<Dialog
						open={deleteBatchNo !== null}
						onOpenChange={(open) => {
							if (!open && !deleting) setDeleteBatchNo(null);
						}}
					>
						<DialogContent
							aria-describedby={undefined}
							className="sm:max-w-sm bg-white dark:bg-zinc-950 text-(--cl-text)"
						>
							<DialogHeader>
								<DialogTitle>Delete Batch #{deleteBatchNo}</DialogTitle>
							</DialogHeader>
							<p className="text-sm text-(--cl-text-muted)">
								This will permanently delete all {deleteJobCount} job{deleteJobCount !== 1 ? "s" : ""}{" "}
								in this batch. This action cannot be undone.
							</p>
							<DialogFooter>
								<Button disabled={deleting} variant="outline" onClick={() => setDeleteBatchNo(null)}>
									Cancel
								</Button>
					...
					>
						...
						>
							<DialogHeader>
								<DialogTitle>Delete Job #{deleteJobNo}</DialogTitle>
							</DialogHeader>
							{deleteJobCount <= 2 ? (
								<p className="text-sm text-red-500">
									Cannot delete this job. A batch must have at least 2 jobs. This batch currently has{" "}
									{deleteJobCount} job{deleteJobCount !== 1 ? "s" : ""}.
								</p>
							) : (
								<p className="text-sm text-(--cl-text-muted)">
							...
							<DialogFooter>
								<Button variant="outline" disabled={deleting} onClick={() => setDeleteJobId(null)}>
									Cancel
								</Button>
								<Button
									variant="destructive"
									disabled={deleting || deleteJobCount <= 2}
									onClick={() => void handleDeleteSingleJob()}
								>
									{deleting && <Loader2 className="mr-1.5 h-4 w-4 animate-spin" />}
									Delete
								</Button>
						...
						>
							<DialogHeader>
								<DialogTitle className="flex items-center gap-2">
									...
									Attach Files
									{postSaveBatchNo && (
										<span className="text-sm font-normal text-(--cl-text-muted)">
											— Batch #{postSaveBatchNo}
										</span>
									)}
								</DialogTitle>
							</DialogHeader>
								...
								{(postSaveJobs ?? []).map((job) => (
									<div key={job.jobId} className="flex flex-col gap-2">
										<div className="flex items-center gap-2">
											<span className="font-mono text-sm font-bold text-(--cl-accent)">
												{job.jobNo}
											</span>
						...
						{attachJobId !== null && (
							<JobImageUpload
								...
								onFileCountChange={(count: number) => {
									...
									setJobs((prev) =>
										prev.map((j) => (j.id === attachJobId ? { ...j, file_count: count } : j)),
									);
									setRefreshTrigger((k) => k + 1);
									if (branchId) void loadData(Number(branchId), searchQ, page);
								}}
			...
			<PdfPreviewModal
				...
				onPrintCopiesChange={(n) => {
					...
					if (pendingPrintRef.current) {
						...
						setPdfPreviewUrl(
							getBatchJobSheetBlobUrl(batchJobs, division ?? null, branchCode, n, {
								clientName,
								buName: currentBu?.name ?? null,
								trackJobUrl,
								termsAndConditions: jobTermsAndConditions,
				...
				<td className={`${tdClass} sticky right-0 z-20 !bg-(--cl-surface-2) group`}>
					<div className="flex items-center justify-center">
						<DropdownMenu>
							...
							<DropdownMenuContent
								align="end"
								className="w-[160px] bg-white dark:bg-zinc-950 border-(--cl-border) shadow-[0_10px_30px_rgba(0,0,0,0.2)] z-50"
							>
								<DropdownMenuItem
									className="flex items-center gap-2 cursor-pointer text-blue-500 focus:bg-blue-500/10 focus:text-blue-600"
									onClick={onView}
								>
								...
								<DropdownMenuItem
									className="flex items-center gap-2 cursor-pointer text-indigo-500 focus:bg-indigo-500/10 focus:text-indigo-600"
									onClick={onPrint}
								>
								...
								<DropdownMenuItem
									...
									title={
										!isBatchDeletable ? "Cannot delete: one or more jobs have activity" : undefined
									}
									onClick={() => {
										if (isBatchDeletable) onDelete();
									}}
								>
				...
				<tr key={job.id} className="group transition-colors hover:bg-(--cl-accent)/5">
					...
					<td className={tdClass}>
						<div className="flex flex-col gap-0.5">
							...
							{job.purchase_date && (
								<span className="text-[11px] font-semibold text-(--cl-text-muted) w-fit">
					...
					<td className={`${tdClass} sticky right-0 z-10 bg-(--cl-surface) group-hover:bg-(--cl-surface-2)`}>
						<div className="flex items-center gap-1 justify-center">
							<Button
								className="h-7 w-7 p-0 text-violet-500 hover:bg-violet-500/10"
								variant="ghost"
								title="Attach Files"
								onClick={() => onAttachJob(job.id, job.job_no)}
							>
							...
							{batch.job_count > 2 && (
								<Button
									...
									onClick={() => {
										if (job.transaction_count <= 1)
											onDeleteJob(job.id, job.job_no, batch.batch_no, batch.job_count);
									}}
								>
