...
export function NewBatchJobForm({
	...
	const {
		...
		watch,
		register,
	} = useFormContext<BatchJobFormValues>();
	...
	return (
		...
		>
			<p className="text-[10px] font-black uppercase tracking-[0.15em] text-(--cl-text-muted)">Batch Details</p>
			...
			<Card className="border-(--cl-border) shadow-md bg-(--cl-surface) overflow-visible!">
				<CardContent className="py-2 px-3 flex flex-wrap lg:flex-nowrap gap-2 overflow-visible!">
					...
					<div className="space-y-0.5 shrink-0">
						...
						<Input
							readOnly
							className="h-8 w-20 bg-(--cl-surface-2) font-mono text-(--cl-accent) font-bold cursor-not-allowed opacity-80 text-xs"
					...
					<div className="space-y-0.5 shrink-0 w-44">
						<Label className={labelCls}>
							Receive Manner <span className="text-red-500">*</span>
						</Label>
						<select
			...
			<div className="flex items-center justify-between">
				...
				{fields.length < 2 && <span className="text-xs text-red-500 font-medium">Minimum 2 jobs required</span>}
			</div>
								...
								<Card className="border-(--cl-border) bg-(--cl-surface) border-l-4 border-l-(--cl-accent) overflow-visible">
									...
									<div className="grid grid-cols-[32px_1fr] md:grid-cols-[32px_minmax(240px,1fr)_130px_130px_100px_100px_60px_140px_70px_32px_32px] gap-x-2 items-center px-3 py-2">
										...
										<div className="flex items-center gap-1 min-w-0">
											<div className="flex-1 min-w-0">
												<SearchableCombobox<ModelRow>
													...
													selectedValue={
														watch(`rows.${idx}.product_brand_model_id`)?.toString() ?? ""
													}
													getDisplayValue={(m) =>
													...
													onSelect={(m) =>
														setValue(
															`rows.${idx}.product_brand_model_id`,
															m ? m.id : (undefined as unknown as number),
															{ shouldValidate: true },
														)
													}
													renderSelectedDisplay={(m) => (
														<div className="flex flex-col justify-center gap-px">
															<span className="text-xs font-semibold leading-tight text-(--cl-text) truncate">
																{m.brand_name}
															</span>
															<span className="text-[10px] leading-tight text-(--cl-text-muted) truncate">
										...
										<select
											...
											onChange={(e) => {
												...
												if (code !== "UNDER_WARRANTY")
													setValue(`rows.${idx}.warranty_card_no`, "");
										...
										<select
											...
											{...register(`rows.${idx}.job_receive_condition_id`, {
												valueAsNumber: true,
											})}
										>
										...
										{editBatchNo && watch(`rows.${idx}.id`) && (
											<button
												...
												onClick={() => {
													...
													if (jobId && editBatchNo)
														onAttachFiles?.(jobId, `Batch #${editBatchNo}`, idx);
												}}
											>
												<Paperclip className="h-3 w-3 text-slate-600" />
												<span>
													{(watch(`rows.${idx}.file_count`) ?? 0) > 0
														? `${watch(`rows.${idx}.file_count`)} File${watch(`rows.${idx}.file_count`) !== 1 ? "s" : ""}`
														: "Attach"}
												</span>
											</button>
										)}
											...
											>
												<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 px-3 pb-3 pt-1 border-t border-(--cl-border)/50">
													...
													<div className="space-y-1 md:hidden">
														...
														<select
															...
															onChange={(e) => {
																...
																if (code !== "UNDER_WARRANTY")
																	setValue(`rows.${idx}.warranty_card_no`, "");
																setValue(`rows.${idx}.job_type_id`, newId, {
																	shouldValidate: true,
																});
															}}
														>
													...
													<div className="space-y-1 sm:col-span-2">
														...
														<Textarea
															rows={2}
															className="bg-(--cl-surface-2) resize-none text-xs"
															placeholder="Describe the problem…"
															{...register(`rows.${idx}.problem_reported`)}
														/>
													</div>
													<div className="space-y-1">
														...
														<Input
															className="h-8 text-xs bg-(--cl-surface-2)"
															placeholder="Optional…"
															{...register(`rows.${idx}.alternate_job_no`)}
														/>
													</div>
													<div className="space-y-1">
														...
														{(() => {
															const rowIsWarranty =
																jobTypes.find(
																	(t) => t.id === watch(`rows.${idx}.job_type_id`),
																)?.code === "UNDER_WARRANTY";
															return (
																<Input
																	className="h-8 text-xs bg-(--cl-surface-2)"
