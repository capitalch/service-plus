...
export const StatusTransitionModal = ({ divisions, job, transition, technicians, onClose, onSubmit }: Props) => {
	...
	useEffect(() => {
		...
		apolloClient
			...
			.then((res) => {
				if (!cancelled) setHasReceipts((res.data?.genericQuery ?? []).length > 0);
			})
			.catch(() => {
				/* on failure, leave division editable; server has no guard */
			});
		return () => {
			cancelled = true;
		};
			...
			<DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					<div className="flex items-center gap-2 pr-8 flex-wrap">
						...
						<span
							className={`inline-flex items-center rounded-sm px-2 py-0.5 text-[11px] font-medium shrink-0 ${STATUS_COLORS[transition.targetCode] ?? "bg-gray-500 text-white"}`}
						>
					...
					<div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-foreground mt-1.5">
						{job.job_receive_manner_name && (
							<span>
								Receive:{" "}
								<span className="font-medium text-foreground">{job.job_receive_manner_name}</span>
							</span>
						)}
						{job.device_details && (
							<span>
								Device: <span className="font-medium text-foreground">{job.device_details}</span>
							</span>
						)}
						{job.job_receive_condition_name && (
							<span>
								Condition:{" "}
								<span className="font-medium text-foreground">{job.job_receive_condition_name}</span>
							</span>
						)}
					</div>
				</DialogHeader>
							...
							<div className="space-y-1.5">
								...
								{form.formState.errors.estimate_amount && (
									<p className="text-xs text-red-500">
										{form.formState.errors.estimate_amount.message}
									</p>
								)}
							</div>
						</div>
								...
								<div className="space-y-3">
									...
									<div className="space-y-1.5">
										...
										{form.formState.errors.technician_id && (
											<p className="text-xs text-red-500">
												{form.formState.errors.technician_id.message}
											</p>
										)}
									</div>
								</div>
