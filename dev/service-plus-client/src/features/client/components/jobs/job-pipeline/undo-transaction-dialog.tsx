		...
		<DialogContent className="sm:max-w-sm overflow-hidden bg-white dark:bg-zinc-950 border-(--cl-border)">
			...
			<div className="space-y-3 text-sm text-(--cl-text)">
				...
				<p className="rounded border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-400">
					The last transaction will be <span className="font-bold">permanently deleted</span> and the job
					restored to its previous status. This cannot be undone.
				</p>
			</div>
			<div className="flex justify-end gap-2 pt-2 border-t border-(--cl-border)">
				<Button className="h-8 px-4 text-xs" variant="ghost" disabled={submitting} onClick={onClose}>
					Cancel
				</Button>
				<Button
