...
export const JobPipelineStatusDrilldown = ({ status, technicians, onBack }: Props) => {
	...
	const { scrollWrapperRef, selectedRowId, setSelectedRowId, armRestore, disarmRestore } =
		useGridRowRetention(loading);
	...
	const loadData = useCallback(async () => {
		...
		try {
			...
			const countArgs = isAll ? baseArgs : { ...baseArgs, status_id: status.status_id };
	...
	return (
		...
		>
			...
			<div className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-(--cl-border) bg-(--cl-surface) py-2 px-4">
				...
				<span
					className={`inline-flex items-center rounded-sm px-3 py-1 text-sm font-semibold text-white ${badgeColorClass}`}
				>
					{status.status_name}
				</span>
							...
							<tbody className="divide-y divide-(--cl-border) bg-(--cl-surface)">
								{rows.map((row, idx) => {
									...
									const showCharges =
										...
										!NO_CHARGES_JOB_TYPES.has(row.job_type_code);
									...
									const isBatchStart =
										row.batch_no != null && (idx === 0 || rows[idx - 1].batch_no !== row.batch_no);
													...
													{row.file_count > 0 && (
														<button
															...
															onClick={(e) => {
																...
																setAttachJobId(row.id);
																setAttachJobNo(row.job_no);
															}}
														>
													...
													<div className="flex flex-wrap gap-1">
														...
														{(() => {
															const gst = isGstDivision(
																divisions.find((d) => d.id === row.division_id) ?? null,
															);
															return (
																<span
