...
export const JobPipelineLanding = ({ onStatusClick }: Props) => {
	...
	function renderRows(rows: JobBoardStatusCount[], startIdx: number) {
		return rows.map((s, i) => {
			...
			const colorParts = (STATUS_COLORS[s.status_code] ?? "bg-slate-400 hover:bg-slate-500 text-white")
				.trim()
				.split(/\s+/)
				.filter(Boolean);
			...
			const warrantySuffix =
				s.status_code === "COMPLETED_OK" || s.status_code === "COMPLETED_OK_FINAL"
			...
			return (
				<motion.button
					...
					onClick={() => {
						if (!isZero) onStatusClick(s);
					}}
				>
					<span
						className="w-44 shrink-0 truncate text-sm font-semibold text-(--cl-text)"
						title={`${s.status_name}${warrantySuffix}`}
					>
						{s.status_name}
						{warrantySuffix && (
							<span className="text-xs font-medium text-(--cl-text-muted)">{warrantySuffix}</span>
						)}
					</span>
					...
					<span
						className={`w-10 shrink-0 text-right text-sm font-bold tabular-nums ${isZero ? "text-(--cl-text-muted)" : "text-(--cl-text)"}`}
					>
						{count}
					</span>
				</motion.button>
	...
	return (
		...
		>
			...
			<div className="flex min-h-0 flex-1 flex-col overflow-y-auto py-4">
				{loading && statusCounts.length === 0 ? (
					...
					<div className="flex flex-col gap-1">
						...
						>
							...
							<span className="w-10 shrink-0 text-right text-sm font-bold tabular-nums text-(--cl-text)">
								{totalJobs}
							</span>
						</motion.button>
				...
				{!loading && statusCounts.length > 0 && (
					<p className="mt-6 text-center text-xs text-(--cl-text-muted)">Click a bar to view jobs</p>
				)}
			</div>
		</motion.div>
	);
