...
import {
	JobLookupCombobox,
	partUsedJobRestrictionReason,
} from "@/features/client/components/jobs/receipts/job-lookup-combobox";
...
const applyMarkup = (cost: number, pct: number) => Math.round(cost * (1 + pct / 100) * 100) / 100;

const thClass =
	"sticky top-0 z-20 text-xs font-extrabold uppercase tracking-widest text-(--cl-text) py-2 px-2 text-left border-b border-(--cl-border) bg-zinc-200/60 dark:bg-zinc-800/60";
...
export function NewPartUsedForm({ onJobSelect, form }: Props) {
	...
	const loadExistingLines = useCallback(
		async (jobId: number) => {
			if (!dbName || !schema) return;
			...
			try {
				...
				toast.error(MESSAGES.ERROR_PART_USED_LOAD_FAILED);
			} finally {
			...
			{!branchId ? (
				<div className="flex flex-col items-center justify-center py-20 rounded-xl border-2 border-dashed border-(--cl-border) text-center">
					...
					<p className="mt-1 text-xs text-(--cl-text-muted)">
						Select a branch from the global header to continue.
					</p>
				</div>
			) : (
				<>
					{/* Job Selection */}
					<div
						className={`rounded-lg border bg-(--cl-surface) p-4 shadow-sm transition-colors ${jobTouched && !selectedJob ? "border-red-400 dark:border-red-500" : "border-(--cl-border)"}`}
					>
						...
						{jobTouched && !selectedJob && (
							<p className="mt-1.5 text-xs text-red-500 font-medium">
								A valid job must be selected to save parts.
							</p>
						)}
						...
						{selectedJob && (
							<div className="mt-3 flex flex-wrap gap-x-6 gap-y-1 rounded-md border border-(--cl-border) bg-(--cl-surface-2)/50 px-4 py-2 text-sm">
								<span>
									<span className="text-xs font-bold uppercase text-(--cl-text-muted)">Job No </span>
									<span className="font-mono font-semibold">{selectedJob.job_no}</span>
								</span>
								<span>
									<span className="text-xs font-bold uppercase text-(--cl-text-muted)">Date </span>
									{selectedJob.job_date}
								</span>
								<span>
									<span className="text-xs font-bold uppercase text-(--cl-text-muted)">
										Customer{" "}
									</span>
									{selectedJob.customer_name}
								</span>
								<span>
									<span className="text-xs font-bold uppercase text-(--cl-text-muted)">Status </span>
									<span className="text-(--cl-accent)">{selectedJob.job_status_name}</span>
								</span>
							</div>
						)}
					</div>
					...
					{selectedJob && (
						<div className="rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
							<p className="px-4 pt-3 text-[10px] font-black uppercase tracking-[0.15em] text-(--cl-text-muted)">
							...
							{loadingExist ? (
								...
								<p className="px-4 py-3 text-sm text-(--cl-text-muted) italic">
									No parts logged yet for this job.
								</p>
							) : (
										...
										<tbody>
											{existingLines.map((line, idx) => (
												<tr key={line.id} className="hover:bg-(--cl-surface-2)/40">
													<td className={`${tdClass} px-2 text-xs text-(--cl-text-muted)`}>
														{idx + 1}
													</td>
													<td className={`${tdClass} px-2 font-mono font-medium`}>
														{line.part_code}
													</td>
													<td className={`${tdClass} px-2`}>{line.part_name}</td>
													<td className={`${tdClass} px-2 text-right font-medium`}>
														{Number(line.qty).toFixed(2)}
													</td>
													<td
														className={`${tdClass} px-2 text-xs text-(--cl-text-muted) italic`}
													>
														{line.remarks || "—"}
													</td>
							...
							<table className="min-w-[960px] w-full border-collapse text-sm">
								<thead>
									<tr>
										...
										<th className={thClass} style={{ width: "13%" }}>
											Remarks
										</th>
									...
									{fields.map((field, idx) => {
										...
										return (
											<tr key={field.id} className="hover:bg-(--cl-surface-2)/30">
												<td className={`${tdClass} pl-3 text-xs text-(--cl-text-muted)`}>
													{idx + 1}
												</td>
												...
												<td className={tdClass}>
													<PartCodeInput
														...
														onSelect={(part) => {
															...
															const effectiveSellingPrice = isWarranty
																? 0
																: markupPct > 0
												...
												<td className={`${tdClass} px-2 text-sm text-(--cl-text-muted)`}>
													{line?.part_name}
												</td>
												<td className={tdClass}>
													<Input
					...
					<div className="flex items-center gap-4 rounded-lg border border-(--cl-border) bg-(--cl-surface-2)/40 px-4 py-2 text-xs">
						<div className="flex items-center gap-1.5">
							<span className="font-black uppercase tracking-widest text-(--cl-text-muted)">
								New Lines
							</span>
