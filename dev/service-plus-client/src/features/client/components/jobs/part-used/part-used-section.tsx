...
import {
	partUsedFormSchema,
	type PartUsedFormValues,
	type ConsumptionRow,
	getPartUsedDefaultValues,
} from "./part-used-schema";
...
export const PartUsedSection = () => {
	...
	const linesValid =
		(formNewLines?.some((l) => l.part_id && (l.qty ?? 0) > 0) ?? false) || (formDeletedIds?.length ?? 0) > 0;
	...
	const loadData = useCallback(
		async (branchIdNum: number, q: string, pg: number) => {
			if (!dbName || !schema) return;
			setLoading(true);
			try {
		...
		>
				...
				<div className="flex items-center gap-3">
					...
					<div className="flex items-baseline gap-2">
						<h1 className="text-lg font-bold text-(--cl-text)">
							Part Used (Job)
							{mode === "new" && (
								<span className="ml-2 text-sm font-medium text-(--cl-text-muted)">— New</span>
							)}
							{mode === "view" && (
								<span className="ml-2 text-sm font-medium text-(--cl-text-muted)">— View</span>
							)}
						</h1>
						{mode === "view" && (
			...
			{mode === "new" ? (
				<div className="flex-1 overflow-y-auto">
					<FormProvider {...form}>
						<NewPartUsedForm key={resetKey} onJobSelect={setSelectedJob} form={form} />
					</FormProvider>
				</div>
			) : (
				<>
					...
					<div className="flex flex-wrap items-center gap-2 px-4 py-2 bg-(--cl-surface-2)/30">
						<div className="relative flex-1 sm:max-w-xs">
							...
							<Input
								...
								placeholder="Job no, alt job no, part code or part name…"
								value={search}
					...
					<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm mx-4">
								...
								<table className="min-w-full border-collapse">
									...
									<tbody className="bg-(--cl-surface)">
										{groupedRows.map((row, idx) => {
											...
											const groupBgHover = isEven
												? "hover:bg-(--cl-accent)/5"
												: "hover:bg-(--cl-accent)/10";
											const stickyBg = isEven
												? "bg-(--cl-surface)"
												: "bg-[color-mix(in_srgb,var(--cl-accent)_4%,var(--cl-surface))]";
											const stickyBgHover = isEven
												? "group-hover:bg-(--cl-surface-2)"
												: "group-hover:bg-(--cl-accent)/10";
											const topBorder =
												row.isFirstInGroup && idx > 0
													? "border-t-2 border-t-(--cl-border)"
													: "";
											...
											return (
												<tr
													key={row.id}
													className={`group transition-colors ${groupBg} ${groupBgHover} ${topBorder}`}
												>
													{/* Col 1: created_at */}
													<td className={`${tdClass} whitespace-nowrap`}>
														<span className="font-medium text-(--cl-text)">
															{createdDate}
														</span>
														{createdTime && (
															<span className="ml-1.5 text-xs text-(--cl-text-muted)">
																{createdTime}
															</span>
														)}
													</td>
													...
													<td className={`${tdClass} min-w-[180px]`}>
														{row.isFirstInGroup ? (
															<div className="flex flex-col gap-0.5">
																<span className="font-mono font-bold text-(--cl-accent)">
																	{row.job_no}
																</span>
																{row.alternate_job_no && (
																	<span className="w-fit font-mono text-[10px] font-semibold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40 rounded px-1.5 py-0.5">
																...
																<div className="flex items-center gap-1 flex-wrap">
																	...
																	/>
																	{row.is_final && (
																		<span className="text-[10px] font-semibold px-1.5 py-px rounded bg-violet-100 text-violet-700 dark:bg-violet-950/60 dark:text-violet-300">
																			Final
																		</span>
																	)}
																	{row.is_closed && (
																		<span className="text-[10px] font-semibold px-1.5 py-px rounded bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-300">
																			Closed
																		</span>
																	)}
																</div>
															...
															<span className="pl-2 text-(--cl-text-muted)/40 select-none text-xs">
																╰
															</span>
														)}
													</td>
													...
													<td className={tdClass}>
														<span className="rounded bg-(--cl-surface-3) px-2 py-0.5 text-xs font-semibold">
															{row.uom}
														</span>
													</td>
													...
													<td className={`${tdClass} text-xs text-(--cl-text-muted) italic`}>
														{row.remarks || "—"}
													</td>
													<td
														className={`${tdClass} sticky right-0 z-10 ${stickyBg} ${stickyBgHover}`}
													>
														<div className="flex items-center gap-0.5">
															...
															<Button
																...
																size="icon"
																title={
																	row.is_closed || row.is_final
																		? "Cannot edit — job is closed/finalised"
																		: "Edit"
																}
																variant="ghost"
															...
															<Button
																...
																title={
																	row.is_closed || row.is_final
																		? "Cannot delete — job is closed/finalised"
																		: "Delete"
																}
																variant="ghost"
						...
						<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
							<span className="text-xs text-(--cl-text-muted)">
								{total === 0
									? "No parts used"
									: `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} parts used (Page ${page} of ${totalPages})`}
							</span>
