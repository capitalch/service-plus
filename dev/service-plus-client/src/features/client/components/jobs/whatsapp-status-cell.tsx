...
export function WhatsappStatusCell({
	state,
				...
				<>
					...
					{(!historyCollapsed || !showToggle) && (
						<div className="flex w-fit flex-col gap-1 rounded-md border border-(--cl-border) bg-(--cl-surface-2) px-2 py-1.5">
							{attempts.map((a) => (
								<div
									key={`${a.attempt_no}-${a.sent_at}`}
									className="flex items-center gap-1.5 whitespace-nowrap"
								>
									<span className="min-w-4 shrink-0 text-right text-[10px] font-semibold text-(--cl-text-muted)">
										{a.attempt_no}.
									</span>
									<span className="text-[11px] tabular-nums text-(--cl-text)">
										{a.sent_at ? fmtLastTry(a.sent_at) : "\u2014"}
									</span>
									<span
