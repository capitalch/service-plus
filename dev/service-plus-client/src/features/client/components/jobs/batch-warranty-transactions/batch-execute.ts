		...
		for (const kind of order) {
			...
			if (stoppedForJob) {
				results.push({
					...
					kind,
					status: "skipped",
					message: "Skipped after an earlier failure for this job",
				});
				continue;
			}
			...
			if (!getLegalKinds(job).has(kind)) {
				results.push({
					...
					kind,
					status: "skipped",
					message: "Already at or past this stage",
				});
				continue;
			}
			...
			switch (kind) {
				case "COMPLETED_OK": {
					...
					if (r.ok && r.targetId !== undefined && r.targetCode !== undefined) {
						...
						job = {
							...job,
							job_status_id: r.targetId,
							job_status_code: r.targetCode,
							is_final: flags?.is_final ?? false,
							is_closed: flags?.is_closed ?? false,
						};
					}
