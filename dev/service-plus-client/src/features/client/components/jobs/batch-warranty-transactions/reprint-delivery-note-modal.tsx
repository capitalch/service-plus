...
export function ReprintDeliveryNoteModal({ branchId, branchName, availableDivisions, onClose }: Props) {
	...
	useEffect(() => {
		...
		const countPromise = apolloClient
			.query<GenericQueryData<{ total: number }>>({
				...
				variables: {
					...
					value: graphQlUtils.buildGenericQueryValue({
						sqlId: SQL_MAP.GET_DELIVERED_WARRANTY_JOB_GROUPS_COUNT,
						sqlArgs: args,
					}),
				},
			})
		...
		const rowsPromise = apolloClient
			.query<GenericQueryData<DeliveredWarrantyJobGroupRow>>({
				fetchPolicy: "network-only",
				query: GRAPHQL_MAP.genericQuery,
				variables: {
					db_name: dbName,
					schema,
					value: graphQlUtils.buildGenericQueryValue({
						sqlId: SQL_MAP.GET_DELIVERED_WARRANTY_JOB_GROUPS_BY_BRANCH,
						sqlArgs: args,
					}),
				},
			})
		...
		Promise.allSettled([countPromise, rowsPromise])
			.then((results) => {
				if (results.some((r) => r.status === "rejected"))
					toast.error("Failed to load delivered warranty jobs.");
			})
	...
	const groupedByDate = (() => {
		...
		groups.forEach((g) => {
			...
			if (list) list.push(g);
			else map.set(g.delivery_date, [g]);
		});
	...
	useEffect(() => {
		...
		setLoadingJobs(true);
		apolloClient
			.query<GenericQueryData<DeliveredWarrantyJobRow>>({
				fetchPolicy: "network-only",
				query: GRAPHQL_MAP.genericQuery,
				variables: {
					db_name: dbName,
					...
					value: graphQlUtils.buildGenericQueryValue({
						...
						sqlArgs: {
							customer_contact_id: selCustomerId,
							branch_id: branchId,
							delivery_date: selDeliveryDate,
						},
					}),
				},
			})
			...
			.catch(() => {
				if (!cancelled) toast.error("Failed to load jobs for this customer.");
			})
			.finally(() => {
	...
	async function handleCreate() {
		...
		try {
			...
			const noteJobs = await fetchDeliveryNoteJobsByIds(
				dbName,
				schema,
				chosenJobs.map((r) => r.id),
			);
			...
			const division = singleDivisionId
				? (availableDivisions.find((d) => d.id === singleDivisionId) ?? null)
				: null;
					...
					<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded border border-(--cl-surface-3)">
														...
														>
															...
															<td className="p-2">
																<div className="flex max-w-48 flex-col gap-0.5">
																	<span
																		className="truncate text-sm font-medium leading-5 text-(--cl-text)"
															...
															<td className="p-2">
																<div className="flex max-w-56 items-start gap-1.5">
																	...
																	<span
																		className="truncate font-mono text-xs leading-5 text-(--cl-text-muted)"
																		title={row.job_nos ?? undefined}
																	>
																		{row.job_nos ?? "—"}
																	</span>
																</div>
						...
						<div className="flex shrink-0 items-center justify-between border-t border-(--cl-surface-3) px-2 py-1.5">
							<span className="text-[11px] text-(--cl-text-muted)">
								{total === 0
									? "No groups"
									: `${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} (Page ${page}/${totalPages})`}
							</span>
							<div className="flex items-center gap-1">
								<Button
									...
									onClick={() => setPage(1)}
								>
									<ChevronsLeftIcon className="h-3.5 w-3.5 text-muted-foreground" />
								</Button>
								<Button
									className="h-6 w-6"
									disabled={page <= 1 || loading}
								...
								>
									<ChevronsRightIcon className="h-3.5 w-3.5 text-muted-foreground" />
								</Button>
							</div>
						</div>
					</div>
						...
						{!selectedGroup ? (
							...
							<>
								<div className="flex shrink-0 items-center justify-between border-b border-(--cl-surface-3) bg-(--cl-surface-2) px-2 py-1.5">
									<label className="flex cursor-pointer items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-(--cl-text-muted)">
										<span className="flex h-7 w-7 items-center justify-center">
											<input
												...
												ref={(el) => {
													if (el) el.indeterminate = someJobsChecked;
												}}
												type="checkbox"
												onChange={(e) =>
													setSelectedJobIds(
														e.target.checked
															? new Set(jobsInGroup.map((j) => j.id))
															: new Set(),
													)
												}
											/>
									...
									<span className="text-[11px] text-(--cl-text-muted)">
										{selectedJobIds.size} of {jobsInGroup.length} selected
									</span>
								</div>
								<div className="min-h-0 flex-1 overflow-y-auto">
									{loadingJobs ? (
										...
										<div className="p-4 text-center text-sm text-(--cl-text-muted)">
															...
															<td className="border-b border-(--cl-surface-3) p-2">
																<div className="flex flex-col gap-0.5">
																	<div className="flex items-center justify-between gap-1.5">
																		<span className="font-mono text-sm font-semibold text-(--cl-accent)">
																			#{job.job_no}
																		</span>
																	...
																	{job.device_details && (
																		<span
																			className="truncate text-xs leading-snug text-(--cl-text-muted)"
																			title={job.device_details}
																		>
																			{job.device_details}
																		</span>
																	)}
