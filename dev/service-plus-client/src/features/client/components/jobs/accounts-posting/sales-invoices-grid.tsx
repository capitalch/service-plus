...
import type { SalesInvoicePostingRow } from "./accounts-posting-schema";
...
export function SalesInvoicesGrid({ isPosted, selectedIds, onSelectionChange, onRowsLoaded, onTotalChange }: Props) {
    ...
    const [rows,    setRows]    = useState<SalesInvoicePostingRow[]>([]);
    ...
    const loadData = useCallback(async (bId: number, q: string, pg: number, posted: boolean) => {
        ...
        try {
            ...
            const [dataRes, countRes] = await Promise.all([
                apolloClient.query<GenericQueryData<SalesInvoicePostingRow>>({
                    fetchPolicy: "network-only",
                    query: GRAPHQL_MAP.genericQuery,
                    variables: {
                        db_name: dbName, schema,
                        value: graphQlUtils.buildGenericQueryValue({
                            sqlId: SQL_MAP.GET_SALES_INVOICES_FOR_POSTING_PAGED,
                ...
                apolloClient.query<GenericQueryData<{ total: number }>>({
                    ...
                    variables: {
                        db_name: dbName, schema,
                        value: graphQlUtils.buildGenericQueryValue({
                            sqlId: SQL_MAP.GET_SALES_INVOICES_FOR_POSTING_COUNT,
                            sqlArgs: args,
                        }),
                    },
                }),
    ...
    const handleTogglePosted = async (row: SalesInvoicePostingRow) => {
        if (!dbName || !schema) return;
        try {
            await apolloClient.mutate({
                ...
                variables: {
                    db_name: dbName, schema,
                    value: encodeObj({ tableName: "sales_invoice", xData: { id: row.id, is_posted: !row.is_posted } }),
                },
            });
                                ...
                                {rows.map((row, idx) => (
                                    <tr key={row.id} className={`group transition-colors hover:bg-(--cl-accent)/5 ${selectedIds.has(row.id) ? "bg-(--cl-accent)/10" : ""}`}>
                                        ...
                                        <td className={`${tdClass} font-medium`}>
                                            {row.invoice_no}
                                            {row.is_return && <span className="ml-1.5 rounded bg-red-100 px-1 py-0.5 text-[10px] font-semibold text-red-600">Return</span>}
                                        </td>
