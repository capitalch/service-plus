...
export function AccountsPostingSection() {
	...
	const postableTotal = divisions.reduce(
		(s, d) => s + num(d.money_receipts) + num(d.purchase_invoices) + num(d.job_invoices) + num(d.sales_invoices),
		0,
	);
	...
	const handlePostDataToTracePlus = async () => {
				...
				next: ({ data }) => {
					...
					if (ev)
						setProgress({
							...
							posted: ev.posted,
							failed: ev.failed,
							currentRef: ev.currentRef,
							currentDivision: ev.currentDivision,
						});
				},
				error: () => {
					/* progress is best-effort; the mutation result is authoritative */
				},
			});
		...
		try {
			...
			const postedCount =
				num(data?.postedMoneyReceipts) +
				num(data?.postedPurchaseInvoices) +
				num(data?.postedJobInvoices) +
				num(data?.postedSalesInvoices);
			...
			<div className="mx-auto w-full max-w-5xl px-6 py-8">
				{loadingCounts ? (
					...
					<p className="py-12 text-center text-sm text-(--cl-text-muted)">
						Select a branch to view unposted records.
					</p>
				) : divisions.length === 0 ? (
					<p className="py-12 text-center text-sm text-(--cl-text-muted)">
						No active divisions found for this branch.
					</p>
				) : (
					<div className="space-y-5">
								...
								<table className="w-full text-sm">
										...
										<tr className="text-[11px] uppercase tracking-wide text-(--cl-text-muted)">
											...
											{TYPE_COLUMNS.map((col) => {
												...
												return (
													<th
														key={col.key}
														className={`px-3 py-2.5 text-right font-medium ${col.posted ? "bg-(--cl-accent)/5" : ""}`}
													>
									...
									<tbody>
										{divisions.map((d) => (
											<tr
												key={d.division_id}
												className="border-t border-(--cl-border) transition-colors hover:bg-(--cl-surface-2)"
											>
												...
												{TYPE_COLUMNS.map((col) => {
													...
													return (
														<td
															key={col.key}
															className={`px-3 py-2.5 text-right tabular-nums ${col.posted ? "bg-(--cl-accent)/5" : ""} ${v > 0 ? "font-semibold text-(--cl-text)" : "text-(--cl-text-muted)"}`}
														>
															{v}
														</td>
													);
									...
									<tfoot>
										<tr className="border-t border-(--cl-border) font-semibold">
											...
											{TYPE_COLUMNS.map((col) => (
												<td
													key={col.key}
													className={`px-3 py-2.5 text-right tabular-nums text-(--cl-text) ${col.posted ? "bg-(--cl-accent)/5" : ""}`}
												>
						...
						<p className="text-center text-xs text-(--cl-text-muted)">
							Highlighted columns (Money Receipts, Purchase Invoices, Job Invoices, Sales Invoices) are
							posted to Trace Plus.
						</p>
									...
									return (
										<div className="w-full max-w-xs">
											...
											<div className="h-1.5 overflow-hidden rounded-full bg-(--cl-surface-2)">
												<div
													className="h-full rounded-full bg-(--cl-accent) transition-all duration-300"
													style={{ width: `${percent}%` }}
												/>
											</div>
											{progress.failed > 0 && (
								...
								<p className="text-sm">
									{grandTotal === 0 ? (
										<span className="flex items-center gap-1.5 font-medium text-emerald-600">
											<CheckCircle2 className="h-4 w-4 text-emerald-600" /> Everything is posted
										</span>
									) : (
										<span className="text-(--cl-text-muted)">
											<span className="font-bold text-(--cl-text)">{postableTotal}</span> record
											{postableTotal !== 1 ? "s" : ""} ready to post
										</span>
									)}
								</p>
							)}
							...
							>
								{isPosting ? (
									...
									<>
										...
										<span>Post data to Trace Plus</span>
									</>
								)}
							</button>
						</div>
