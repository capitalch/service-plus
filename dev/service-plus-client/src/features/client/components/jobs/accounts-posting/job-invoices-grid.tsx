...
import type { JobInvoicePostingRow } from "./accounts-posting-schema";
...
export function JobInvoicesGrid({ isPosted, selectedIds, onSelectionChange, onRowsLoaded, onTotalChange }: Props) {
    ...
    const [rows,    setRows]    = useState<JobInvoicePostingRow[]>([]);
    ...
    const loadData = useCallback(async (bId: number, q: string, pg: number, posted: boolean) => {
        ...
        try {
            ...
            const [dataRes, countRes] = await Promise.all([
                apolloClient.query<GenericQueryData<JobInvoicePostingRow>>({
                    fetchPolicy: "network-only",
                    query: GRAPHQL_MAP.genericQuery,
                    variables: {
                        db_name: dbName, schema,
                        value: graphQlUtils.buildGenericQueryValue({
                            sqlId: SQL_MAP.GET_JOB_INVOICES_FOR_POSTING_PAGED,
                ...
                apolloClient.query<GenericQueryData<{ total: number }>>({
                    ...
                    variables: {
                        db_name: dbName, schema,
                        value: graphQlUtils.buildGenericQueryValue({
                            sqlId: SQL_MAP.GET_JOB_INVOICES_FOR_POSTING_COUNT,
                            sqlArgs: args,
                        }),
                    },
                }),
            ...
            toast.error("Failed to load job invoices.");
        } finally {
            setLoading(false);
        }
    ...
    const handleTogglePosted = async (row: JobInvoicePostingRow) => {
        if (!dbName || !schema) return;
        try {
            await apolloClient.mutate({
                ...
                variables: {
                    db_name: dbName, schema,
                    value: encodeObj({ tableName: "job_invoice", xData: { id: row.id, is_posted: !row.is_posted } }),
                },
            });
            toast.success(row.is_posted ? "Invoice unposted." : "Invoice posted.");
            if (branchId) void loadData(branchId, searchQ, page, isPosted);
        } catch {
        ...
        <motion.div animate={{ opacity: 1 }} className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden" initial={{ opacity: 0 }} transition={{ duration: 0.2 }}>
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border)">
                <div className="flex-1 overflow-x-auto overflow-y-auto">
                    {loading ? (
                        <table className="min-w-full border-collapse"><tbody>{Array.from({ length: 10 }).map((_, i) => (
                            <tr key={i} className="animate-pulse">{Array.from({ length: 7 }).map((__, j) => (
                                <td key={j} className={tdClass}><div className="h-4 w-20 rounded bg-(--cl-border)" /></td>
                            ))}</tr>
                        ))}</tbody></table>
                    ) : rows.length === 0 ? (
                        <div className="flex h-32 items-center justify-center text-sm text-(--cl-text-muted)">No job invoices found.</div>
                    ) : (
                        <table className="min-w-full border-collapse">
                            <thead>
                                <tr>
                                    <th className={thClass}><input type="checkbox" className="cursor-pointer accent-(--cl-accent)" checked={allPageSelected} ref={el => { if (el) el.indeterminate = !allPageSelected && somePageSelected; }} onChange={toggleAllPage} /></th>
                            ...
                            <tbody>
                                {rows.map((row, idx) => (
                                    <tr key={row.id} className={`group transition-colors hover:bg-(--cl-accent)/5 ${selectedIds.has(row.id) ? "bg-(--cl-accent)/10" : ""}`}>
                                        <td className={tdClass}><input type="checkbox" className="cursor-pointer accent-(--cl-accent)" checked={selectedIds.has(row.id)} onChange={() => toggleRow(row.id)} /></td>
                                        ...
                                        {isPosted && (
                                            <td className={`${tdClass} sticky right-0 bg-(--cl-surface) group-hover:bg-(--cl-surface-2)`}>
                                                <DropdownMenu>
                                                    <DropdownMenuTrigger asChild><Button className="h-7 w-7 p-0" variant="ghost"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger>
                                                    <DropdownMenuContent align="end">
                                                        <DropdownMenuItem onClick={() => void handleTogglePosted(row)}><Undo2 className="mr-2 h-4 w-4" />Unpost</DropdownMenuItem>
                                                    </DropdownMenuContent>
                                                </DropdownMenu>
                                            </td>
                                        )}
