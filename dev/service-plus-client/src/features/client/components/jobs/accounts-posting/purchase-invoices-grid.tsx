...
import type { PurchaseInvoicePostingRow } from "./accounts-posting-schema";
...
export function PurchaseInvoicesGrid({ isPosted, selectedIds, onSelectionChange, onRowsLoaded, onTotalChange }: Props) {
    ...
    const [rows,    setRows]    = useState<PurchaseInvoicePostingRow[]>([]);
    ...
    const loadData = useCallback(async (bId: number, q: string, pg: number, posted: boolean) => {
        ...
        try {
            ...
            const [dataRes, countRes] = await Promise.all([
                apolloClient.query<GenericQueryData<PurchaseInvoicePostingRow>>({
                    fetchPolicy: "network-only",
                    query: GRAPHQL_MAP.genericQuery,
                    variables: {
                        db_name: dbName, schema,
                        value: graphQlUtils.buildGenericQueryValue({
                            sqlId: SQL_MAP.GET_PURCHASE_INVOICES_FOR_POSTING_PAGED,
                ...
                apolloClient.query<GenericQueryData<{ total: number }>>({
                    ...
                    variables: {
                        db_name: dbName, schema,
                        value: graphQlUtils.buildGenericQueryValue({
                            sqlId: SQL_MAP.GET_PURCHASE_INVOICES_FOR_POSTING_COUNT,
                            sqlArgs: args,
                        }),
                    },
                }),
    ...
    const handleTogglePosted = async (row: PurchaseInvoicePostingRow) => {
        if (!dbName || !schema) return;
        try {
            await apolloClient.mutate({
                ...
                variables: {
                    db_name: dbName, schema,
                    value: encodeObj({ tableName: "purchase_invoice", xData: { id: row.id, is_posted: !row.is_posted } }),
                },
            });
