...
function buildTrackInfoLine(
	printMeta: SingleJobSheetPrintMeta | undefined,
...
function buildSingleJobSheetDoc(
	job: JobDetailType,
	division: DivisionContextType | null,
	branchCode?: string,
	copies = 1,
	printMeta?: SingleJobSheetPrintMeta,
): jsPDF {
	...
	const trackLineFontSize = trackLineText
		? fitSingleLineFontSize(doc, trackLineText, pageWidth - 30 - 4, 11, 5.5, 0.25, true)
		: 0;
		...
		autoTable(doc, {
			body: [
				...
				[
					...
					{ content: String(job.qty), colSpan: 3 },
				],
...
export function getJobSheetBlobUrl(
	job: JobDetailType,
	division: DivisionContextType | null,
	branchCode?: string,
	copies = 1,
...
function buildBatchJobSheetDoc(
	jobs: JobDetailType[],
	division: DivisionContextType | null,
	branchCode?: string,
	copies = 1,
	...
	const trackLineFontSize = trackLineText
		? fitSingleLineFontSize(doc, trackLineText, pageWidth - 30 - 2.4, 11, 5.5, 0.25, true)
		: 0;
	...
	function drawContent(yOffset: number = 0): number {
		...
		if (division) {
			const line1 = [division.address_line1, division.address_line2, division.city, division.pincode]
				.filter(Boolean)
				.join(", ");
			const line2 = [
				division.phone && `Ph: ${division.phone}`,
				division.email && `Email: ${division.email}`,
				division.gstin && `GSTIN: ${division.gstin}`,
			]
				.filter(Boolean)
				.join(" | ");
				...
				body.push([
					{
						content: probRemText,
						colSpan: colCount,
						styles: { fontSize: 6, textColor: [80, 80, 80], cellPadding: 1 },
					},
				]);
			}
		}
...
export function getBatchJobSheetBlobUrl(
	jobs: JobDetailType[],
	division: DivisionContextType | null,
	branchCode?: string,
	copies = 1,
...
function buildJobInfoDoc({
	...
	const overviewRows: (string | { content: string; colSpan: number; styles: Record<string, unknown> })[][] = [
		...
		[
			"Status:",
			`${job.job_status_name}${job.is_final && !job.is_closed ? "  [FINAL]" : ""}`,
			"Job Type:",
			job.job_type_name,
		],
		...
		autoTable(doc, {
			...
			body: invoice.lines.map((l, i) =>
				gstEnabled
					...
					: [i + 1, l.description, Number(l.qty).toFixed(2), fmt(l.price), fmt(l.amount)],
			),
			startY: y,
			margin: { left: 14, right: 14 },
		...
		const totals: [string, string][] = gstEnabled
			? [
					...
					...(invoice.cgst_amount > 0
						? ([
								["CGST", fmt(invoice.cgst_amount)],
								["SGST", fmt(invoice.sgst_amount)],
							] as [string, string][])
						: []),
	...
	for (let i = 1; i <= totalPages; i++) {
		...
		doc.text(`Page ${i} of ${totalPages}`, pageWidth - 14, doc.internal.pageSize.getHeight() - 6, {
			align: "right",
		});
		doc.text(
			`Job #${job.job_no}  |  Generated ${new Date().toLocaleDateString()}`,
			14,
