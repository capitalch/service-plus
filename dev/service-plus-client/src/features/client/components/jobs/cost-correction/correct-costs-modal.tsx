...
const th =
	"sticky top-0 z-10 bg-(--cl-surface-2) px-2 py-1.5 text-left text-[11px] font-semibold uppercase tracking-wide text-(--cl-text-muted) border-b border-(--cl-border)/30 whitespace-nowrap";
...
export function CorrectCostsModal({ open, jobId, jobNo, branchId, onClose, onSaved }: Props) {
	...
	useEffect(() => {
		...
		apolloClient
			.query<GenericQueryData<CostLine>>({
	...
	function handleCostInput(lineTable: string, id: number, value: string) {
		setLines((prev) =>
			prev.map((l) => (l.line_table === lineTable && l.id === id ? { ...l, cost_input: value } : l)),
		);
	}
			...
			<DialogContent className="flex max-h-[90vh] w-full max-w-3xl flex-col sm:max-w-3xl">
				...
				{loading || error ? (
					<div className="flex h-32 items-center justify-center gap-2 text-sm text-(--cl-text-muted)">
						{error ? (
							<span>Failed to load cost lines.</span>
						) : (
							<>
										...
										<tr key={`${l.line_table}-${l.id}`}>
											...
											<td className={`${td} text-right`}>
												<Input
													...
													value={l.cost_input}
													onChange={(e) =>
														handleCostInput(l.line_table, l.id, e.target.value)
													}
					...
					<span className="text-xs text-(--cl-text-muted)">
						{missingCount > 0 ? (
							<span className="text-rose-600 dark:text-rose-400">
								{missingCount} line{missingCount !== 1 ? "s" : ""} still need a cost greater than zero.
							</span>
						) : hasChanges ? (
							"Unsaved changes."
						) : (
