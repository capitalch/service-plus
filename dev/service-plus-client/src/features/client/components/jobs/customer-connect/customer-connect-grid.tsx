...
import { ChevronsLeftIcon, ChevronLeftIcon, ChevronRightIcon, ChevronsRightIcon, Eye, Loader2 } from "lucide-react";
		...
		<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
							...
							{rows.map((row, idx) => {
								...
								return (
									...
									>
										...
										<td className={tdClass}>
											<WhatsappStatusCell
												state={row.whatsapp_notifications?.JOB_COMPLETION ?? null}
											/>
										</td>
										<td
			...
			<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
				...
				<div className="flex items-center gap-1">
					<button
						className="flex h-7 w-7 items-center justify-center rounded text-(--cl-text-muted) hover:bg-(--cl-accent)/10 hover:text-(--cl-accent) disabled:opacity-30 cursor-pointer"
