...
import { WhatsappLogGrid } from "./whatsapp-log-grid";
import { PAGE_SIZE } from "./customer-connect-helpers";
import type { CustomerConnectJobRow } from "./customer-connect-schema";

type GenericQueryData<T> = { genericQuery: T[] | null };

// Read-only log tab — Job Intake / Job Delivery (plans/plan.md). No
// selection, no send, no live subscription (see plan.md's "no live tracking"
// finding: the whatsappDeliveryStatus payload has no event_key, so a second
// concurrent subscriber couldn't safely tell which event a callback belongs
// to). Refresh is the only way this data updates — which is exactly what was
// asked for, not a shortcut.
//
// No header of its own — the active tab button already identifies which log
// this is, and the count is surfaced by the parent (customer-connect-section.tsx)
...
export function WhatsappLogSection({ eventKey, emptyMessage, onCountChange }: Props) {
    ...
    const loadData = useCallback(async (branch: number, q: string, pg: number) => {
        ...
        setLoading(true);
        try {
            const commonArgs = { branch_id: branch, event_key: eventKey, search: q };
            const [dataRes, countRes] = await Promise.all([
                apolloClient.query<GenericQueryData<CustomerConnectJobRow>>({
                    ...
                    variables: {
                        ...
                        value: graphQlUtils.buildGenericQueryValue({
                            sqlId: SQL_MAP.GET_WHATSAPP_EVENT_LOG_PAGED,
                ...
                apolloClient.query<GenericQueryData<{ total: number }>>({
                    ...
                    variables: {
                        db_name: dbName, schema,
                        value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_WHATSAPP_EVENT_LOG_COUNT, sqlArgs: commonArgs }),
                    },
                }),
            ]);
    ...
    }, [dbName, schema, eventKey, onCountChange]);
