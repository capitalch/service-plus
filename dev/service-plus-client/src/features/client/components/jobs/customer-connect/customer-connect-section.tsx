...
import {
	PAGE_SIZE,
	applyOutcomeToAttempts,
	getCompletionState,
	groupRowsByCustomer,
	hasAnyPriorAttempt,
	isRowSelectable,
	type GroupableJobRow,
} from "./customer-connect-helpers";
import type {
...
export function CustomerConnectSection() {
				...
				const [dataRes, countRes] = await Promise.all([
					...
					apolloClient.query<GenericQueryData<{ total: number }>>({
						...
						variables: {
							...
							schema,
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_WHATSAPP_ELIGIBLE_JOBS_COUNT,
								sqlArgs: commonArgs,
							}),
						},
					}),
	...
	const startBannerTracking = useCallback(
		(jobIds: number[]) => {
			stopBannerTracking();
		...
		>
			...
			<div className="grid grid-cols-1 items-center gap-3 sm:grid-cols-[1fr_auto_1fr]">
				<div className="flex items-center gap-2">
					...
					<span className="text-xs text-(--cl-text-muted) whitespace-nowrap">
						...
						{activeTab === "moneyReceipt" &&
							`${moneyReceiptTotal} logged message${moneyReceiptTotal !== 1 ? "s" : ""}`}
				...
				<div className="grid grid-cols-5 gap-2.5 rounded-xl border-2 border-(--cl-border) bg-(--cl-surface-2) p-1 shadow-md sm:mx-auto sm:flex sm:w-auto">
					...
					<button
						className={`h-9 gap-1.5 px-2 sm:px-4 text-xs sm:text-sm transition-transform duration-200 rounded-lg border-0 cursor-pointer inline-flex items-center justify-center ${
					...
					<button
						className={`h-9 px-2 sm:px-4 text-xs sm:text-sm transition-transform duration-200 rounded-lg border-0 cursor-pointer ${
							activeTab === "intake"
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						<div className="ml-auto flex items-center gap-2">
							...
							>
								...
								Send Messages{" "}
								{selectedIds.size > 0
									? `(${selectedIds.size} job${selectedIds.size !== 1 ? "s" : ""})`
									: ""}
							</Button>
						</div>
					</div>
