...
function modeBadgeClass(mode: string) {
	switch (mode) {
		...
		default:
			return "bg-(--cl-surface-3) text-(--cl-text-muted)";
	}
}
...
export const ReceiptsSection = () => {
	...
	const {
		sending: sendingWhatsappReceipt,
		send: sendWhatsappReceipt,
		ConfirmDialog: whatsappReceiptConfirmDialog,
	...
	const lastReceiptRef = useRef<{
		job: Parameters<typeof buildReceiptPdf>[0];
		division: Parameters<typeof buildReceiptPdf>[1];
	} | null>(null);
	...
	const loadData = useCallback(
		async (bid: number, from: string, to: string, q: string, pg: number) => {
		...
		try {
				...
				apolloClient.query<GenericQueryDataType<JobDetailType>>({
					...
					variables: {
						...
						value: graphQlUtils.buildGenericQueryValue({
							...
							sqlArgs: { id: row.job_id },
						}),
					},
				}),
				apolloClient.query<
					GenericQueryDataType<{
						id: number;
			...
			const division =
				availableDivisions.find(
					(d) => d.id === (job as unknown as { division_id: number | null }).division_id,
				) ?? null;
	...
	return (
		...
		>
			...
			<div className="flex flex-wrap items-center gap-x-4 gap-y-3 border-b border-(--cl-border) bg-(--cl-surface) px-4 py-2">
				<div className="flex items-center gap-3">
					...
					<div className="flex items-baseline gap-2">
						<h1 className="text-lg font-bold text-(--cl-text)">Receipts</h1>
						<span className="text-xs text-(--cl-text-muted)">{loading ? "Loading…" : `(${total})`}</span>
					</div>
				</div>
								...
								{rows.map((row, idx) => (
									...
									>
										...
										<td className={tdClass}>
											...
											{postDataToAccounts && (
												<div
													className={`mt-0.5 text-[10px] font-semibold ${row.is_posted ? "text-emerald-600" : "text-amber-600"}`}
												>
										...
										<td className={tdClass}>
											<div className="flex flex-col gap-0.5">
												<span className="font-mono font-semibold text-(--cl-accent)">
													...
													{row.is_opening_job && (
														<span className="ml-1.5 text-[10px] font-bold text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-950/40 rounded px-1 py-0.5">
															OPENING
														</span>
													)}
												</span>
												{row.alternate_job_no && (
													<span className="w-fit text-[10px] font-semibold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40 rounded px-1.5 py-0.5">
														Alt: {row.alternate_job_no}
													</span>
												)}
												{row.batch_no != null && (
													<span className="text-[10px] font-bold text-violet-600 dark:text-violet-400 w-fit bg-violet-50 dark:bg-violet-950/40 rounded px-1 py-0.5">
														Batch #{row.batch_no}
													</span>
												)}
												{row.division_id != null &&
													(() => {
														const dv = availableDivisions.find(
												...
												<div className="flex flex-wrap gap-1">
													...
													{(() => {
														const gst = isGstDivision(
															availableDivisions.find((d) => d.id === row.division_id) ??
																null,
														);
														return (
															<span
										...
										<td className={tdClass}>
											<span
												className={`rounded px-2 py-0.5 text-xs font-semibold ${modeBadgeClass(row.payment_mode)}`}
											>
												{row.payment_mode}
											</span>
										</td>
										...
										<td className={`${tdClass} text-xs text-(--cl-text-muted)`}>
											{row.reference_no || "—"}
										</td>
										<td
										...
										>
											<DropdownMenu>
												<DropdownMenuTrigger asChild>
													<Button
														className="h-7 w-7 p-0 text-(--cl-text-muted)"
														size="icon"
														variant="ghost"
													>
														{pdfLoading === row.id ? (
												...
												<DropdownMenuContent align="end" className="w-44">
													<DropdownMenuItem onClick={() => setViewJobId(row.job_id)}>
														<Eye className="mr-2 h-3.5 w-3.5 text-muted-foreground" /> View
														Job
													</DropdownMenuItem>
													...
													>
														<WhatsAppIcon className="mr-2 h-3.5 w-3.5" /> Send Receipt via
														WhatsApp
													</DropdownMenuItem>
													<DropdownMenuSeparator />
													<DropdownMenuItem
														...
														onClick={() => {
															if (!isReceiptJobRestricted(row)) handleEditReceipt(row);
														}}
													>
				...
				<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2 flex-wrap gap-2">
					<div className="flex items-center gap-4">
						<span className="text-xs text-(--cl-text-muted)">
							{total === 0
								? "No receipts"
								: `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} receipts (Page ${page} of ${totalPages})`}
						</span>
						{rows.length > 0 && (
			...
			<Dialog
				open={isDialogOpen}
				onOpenChange={(open) => {
					if (!open && !form.formState.isSubmitting) setIsDialogOpen(false);
				}}
			>
				<DialogContent
				...
				>
					<DialogHeader>
						<DialogTitle>{selectedReceipt?.id ? "Edit Receipt" : "New Receipt"}</DialogTitle>
					</DialogHeader>
			...
			>
				<AlertDialogContent className="max-w-sm">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Receipt cannot be deleted because the job is in a restricted status (closed, delivered,
							final, cancelled, disposed, on hold or return). No changes are allowed for such jobs.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
			...
			>
				<AlertDialogContent className="max-w-sm">
					<AlertDialogHeader>
						<AlertDialogTitle>Cannot Delete Receipt</AlertDialogTitle>
						<AlertDialogDescription>{MESSAGES.ERROR_RECEIPT_DELETE_IS_POSTED}</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel onClick={() => setDeleteRow(null)}>Close</AlertDialogCancel>
					</AlertDialogFooter>
				...
				<DialogContent aria-describedby={undefined} className="sm:max-w-sm">
					...
					<p className="text-sm text-(--cl-text-muted)">
						...
						<span className="font-semibold text-(--cl-text)">
							{deleteRow ? formatAmount(deleteRow.amount) : ""}
						</span>{" "}
						via <span className="font-semibold text-(--cl-text)">{deleteRow?.payment_mode}</span> for job{" "}
						<span className="font-mono font-semibold text-(--cl-accent)">#{deleteRow?.job_no}</span>? This
						action cannot be undone.
					</p>
					<DialogFooter>
						<Button disabled={deleting} variant="outline" onClick={() => setDeleteRow(null)}>
							Cancel
						</Button>
