...
import {
    downloadJobDetailPdf,
    getJobDetailPdfBlobUrl,
    type JobAdditionalChargeRow,
    type JobPartUsedRow,
} from "./job-detail-pdf";
...
export const JobPdfModal = ({ jobId, onClose }: Props) => {
    ...
    useEffect(() => {
        ...
        async function process() {
            setLoading(true);
            try {
                const [jobRes, tranRes, partsRes, chargesRes] = await Promise.all([
                ...
                const transactions = (tranRes.data?.genericQuery    ?? []) as JobTransactionRow[];
                const parts        = (partsRes.data?.genericQuery   ?? []) as JobPartUsedRow[];
                const charges      = (chargesRes.data?.genericQuery ?? []) as JobAdditionalChargeRow[];
                const division     = availableDivisions.find(d => d.id === job.division_id) ?? currentDivision ?? null;

                createdUrl = getJobDetailPdfBlobUrl(job, transactions, division, parts, charges);
                setData({ job, transactions, parts, charges });
                setPdfUrl(createdUrl);
            } catch {
                if (!cancelled) toast.error("Failed to load job details.");
            } finally {
    ...
    function handleDownload() {
        if (!data) return;
        downloadJobDetailPdf(data.job, data.transactions, division, data.parts, data.charges);
    }
            ...
            >
                <DialogHeader className="flex flex-row items-center justify-between border-b border-slate-200 px-5 py-3 pr-12 shrink-0">
                    <DialogTitle className="text-base font-bold text-slate-900">
                        {data ? `Job Report — #${data.job.job_no}` : "Job Report"}
                    </DialogTitle>
