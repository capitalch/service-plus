...
function buildJobDetailDoc(
	...
	if (division) {
		...
		y += 4;
		const contact = [division.phone && `Phone: ${division.phone}`, division.email && `Email: ${division.email}`]
			.filter(Boolean)
			.join(" | ");
		if (contact) {
	...
	const partsGstAmt = gstEnabled
		? parts.reduce((s, p) => s + (Number(p.selling_price ?? 0) * Number(p.qty) * partGstRate(p)) / 100, 0)
		: 0;
	const chargesGstAmt = gstEnabled
		? charges.reduce((s, c) => s + (Number(c.selling_price ?? 0) * chargeGstRate(c)) / 100, 0)
		: 0;
		...
		autoTable(doc, {
			body: [
				...
				...(gstEnabled && totalGstAmt > 0
					? forceIgst
						? [["IGST (incl. above)", fmtAmount(igstAmt)]]
						: [
								["CGST (incl. above)", fmtAmount(cgstAmt)],
								["SGST (incl. above)", fmtAmount(sgstAmt)],
							]
					: []),
