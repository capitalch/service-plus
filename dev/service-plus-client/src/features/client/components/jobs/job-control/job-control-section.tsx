...
type JobFilter = { group: "closed"; value: ClosedFilter } | { group: "status"; id: number | null };
...
const BATCH_COLORS = [
	{
		...
		badge: "text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40",
	},
	{
	...
	},
	{
		...
		badge: "text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/40",
	},
	{
		border: "border-l-teal-400 dark:border-l-teal-500",
		badge: "text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/40",
	},
	{
	...
	},
	{
		...
		badge: "text-fuchsia-600 dark:text-fuchsia-400 bg-fuchsia-50 dark:bg-fuchsia-950/40",
	},
];
...
export const JobControlSection = () => {
	...
	const [showPartsInInvoiceSetting, setShowPartsInInvoiceSetting] = useState<{
		show: boolean;
		text: string;
		hsn: number;
		gst_rate: number;
	...
	const loadData = useCallback(
		async (bId: number, q: string, pg: number, f: JobFilter) => {
	...
	useEffect(() => {
		...
		void gq<TechnicianRow>(SQL_MAP.GET_ALL_TECHNICIANS, { branch_id: branchId }).then((res) =>
			setTechnicians(res.data?.genericQuery ?? []),
		);
		void gq<DeliveryMannerRow>(SQL_MAP.GET_JOB_DELIVERY_MANNERS).then((res) =>
			setDeliveryManners(res.data?.genericQuery ?? []),
		);
		void gq<{ setting_value: unknown }>(SQL_MAP.GET_APP_SETTING_BY_KEY, {
			setting_key: "show_parts_in_job_invoice",
		}).then((res) => {
	...
	const handleClosedFilterChange = (value: ClosedFilter) => {
		setFilter({ group: "closed", value });
		setPage(1);
	};

	const handleStatusFilterChange = (id: number | null) => {
		setFilter({ group: "status", id });
		setPage(1);
	};
			...
			const [partsRes, chargesRes, jobRes] = await Promise.all([
				...
				apolloClient.query<GenericQueryData<AdditionalChargeRow>>({
					...
					variables: {
						...
						schema,
						value: graphQlUtils.buildGenericQueryValue({
							sqlId: SQL_MAP.GET_JOB_ADDITIONAL_CHARGES_BY_JOB,
							sqlArgs: { job_id: job.id },
						}),
					},
				}),
	...
	return (
		...
		>
				...
				<div className="overflow-x-auto border-b border-(--cl-border) bg-(--cl-surface) lg:overflow-x-visible">
					<div className="flex min-w-max items-center gap-2 px-3 py-1.5 lg:min-w-0 lg:flex-wrap">
						<Button
							...
							variant="outline"
							onClick={() => {
								setFilter({ group: "closed", value: null });
								setPage(1);
							}}
						>
						...
						<button
							disabled={loading}
							className={`shrink-0 px-3 h-7 rounded-full text-[11px] font-semibold transition-colors disabled:opacity-50 cursor-pointer border
                                ${
									filter.id === null
						...
						{jobStatuses
							...
							.map((s) => {
								const colorParts = (STATUS_COLORS[s.code] ?? "bg-slate-400 text-white")
									.trim()
									.split(/\s+/)
									.filter(Boolean);
								...
								return (
									<button
										key={s.id}
										disabled={loading}
										className={`shrink-0 px-3 h-7 rounded-full text-[11px] font-semibold transition-all disabled:opacity-30 cursor-pointer ${colorCls} ${isActive ? "opacity-100 ring-2 ring-offset-1 ring-white/70 shadow-md" : "opacity-50 hover:opacity-80"}`}
				...
				<div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-(--cl-border) bg-(--cl-surface) px-3 py-1.5">
					...
					<div className="flex items-center rounded border border-(--cl-border) overflow-hidden shrink-0">
						...
						<button
							...
							onClick={() => {
								setFilter({ group: "status", id: null });
								setPage(1);
							}}
						>
							Status
					...
					<Button
						className="ml-auto h-8 gap-1.5 px-3 text-[11px] font-semibold text-amber-600 dark:text-amber-400 border border-amber-400 dark:border-amber-500 hover:bg-amber-50 dark:hover:bg-amber-950/40 shrink-0"
						size="sm"
						variant="outline"
					...
					<Button
						className="h-8 gap-1.5 px-3 text-[11px] font-semibold text-sky-600 dark:text-sky-400 border border-sky-400 dark:border-sky-500 hover:bg-sky-50 dark:hover:bg-sky-950/40 shrink-0"
						size="sm"
						variant="outline"
			...
			<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
				...
				>
					{loading ? (
							...
							<thead>
								<tr className="bg-(--cl-surface-2)">
									{[
										...
										<th
											key={h}
											className={`${thClass} ${h === "Actions" ? "hidden md:table-cell" : ""}`}
										>
											{h}
										</th>
									))}
						...
						<div className="flex h-32 items-center justify-center text-xs text-(--cl-text-muted)">
						...
						<table className="min-w-full border-collapse">
							<thead className="sticky top-0 z-10">
								<tr>
									...
									<th
										className={`${thClass} sticky right-0 z-20 !bg-(--cl-surface-2) hidden md:table-cell`}
									>
										Actions
									</th>
								</tr>
							...
							<tbody className="divide-y divide-(--cl-border) bg-(--cl-surface)">
								{rows.map((job, idx) => {
									const batchColor =
										job.batch_no != null ? BATCH_COLORS[job.batch_no % BATCH_COLORS.length] : null;
									const isBatchStart =
										job.batch_no != null && (idx === 0 || rows[idx - 1].batch_no !== job.batch_no);
										...
										>
											...
											<td className={`${tdClass} whitespace-nowrap`}>
												<div className="flex flex-col gap-0.5">
													<span>
														{isDelivered ? (job.delivery_date ?? "—") : job.job_date}
													</span>
													{job.division_id &&
														(() => {
															const dv = divisions.find((d) => d.id === job.division_id);
															return dv ? (
																<span className="font-mono text-[9px] font-semibold text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/40 rounded px-1 py-0.5 w-fit">
																	{dv.code}
																</span>
															) : null;
														})()}
											...
											<td className={tdClass}>
												<div className="flex flex-col gap-0.5">
													...
													{job.is_opening_job && (
														<span className="text-[10.5px] font-bold text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-950/40 rounded px-1.5 py-0.5 w-fit">
															OPENING
														</span>
													)}
													{job.purchase_date && (
													...
													{job.batch_no != null && (
														<span
															className={`text-[10px] font-bold w-fit rounded px-1 py-0.5 ${batchColor?.badge}`}
														>
															Batch #{job.batch_no}
														</span>
													)}
													{job.file_count > 0 && (
														<button
															...
															onClick={(e) => {
																...
																setAttachJobId(job.id);
																setAttachJobNo(job.job_no);
															}}
														>
											...
											<td className={tdClass}>
												<div className="flex flex-col items-start gap-1">
													<StatusBadge
														code={job.job_status_code}
														name={job.job_status_name}
													/>
													<div className="flex flex-wrap gap-1">
														{job.is_final && !job.is_closed && (
															<span className="text-[10px] font-bold rounded px-1 py-0.5 text-indigo-600 dark:text-indigo-300 bg-indigo-100 dark:bg-indigo-950/40">
																FINAL
															</span>
														)}
														{(() => {
															const gst = isGstDivision(
																divisions.find((d) => d.id === job.division_id) ?? null,
															);
															return (
																<span
											...
											>
												{(() => {
													const isDelivered =
														job.job_status_code === "DELIVERED_OK" ||
														job.job_status_code === "DELIVERED_NOT_OK";
													if (isDelivered) {
														return (
															<div className="flex items-center justify-center">
																<DropdownMenu>
																	...
																	<DropdownMenuContent
																		align="end"
																		className="min-w-[200px] bg-white dark:bg-zinc-950 border-(--cl-border) shadow-lg rounded-lg p-1"
																	>
																		<DropdownMenuItem
																		...
																		<DropdownMenuItem
																			...
																			onClick={() =>
																				void deliveredActions.handleDeliveryNote(
																					job,
																				)
																			}
																		>
																		...
																		{job.invoice_is_posted !== null && (
																			<DropdownMenuItem
																				...
																				onClick={() =>
																					void deliveredActions.handleInvoiceReceipts(
																						job,
																					)
																				}
																			>
																		...
																		>
																			<IndianRupee className="h-3.5 w-3.5 shrink-0 text-slate-600" />{" "}
																			Correct Costs
																		</DropdownMenuItem>
																		...
																		<DropdownMenuItem
																			...
																			title={
																				job.invoice_is_posted === true
																					? "Cannot undo: invoice is already posted"
																					: undefined
																			}
																			onClick={() =>
																				deliveredActions.handleUndoDelivery(job)
																			}
																		>
													...
													return (
														<div className="flex items-center justify-center gap-1">
															...
															{(() => {
																...
																const isNoAction = NO_ACTION_CODES.has(
																	job.job_status_code,
																);
																const rowCanUndo = canUndo(job);
																const showCharges =
																	ADD_CHARGES_CODES.has(job.job_status_code) &&
																...
																return (
																	<DropdownMenu>
																		...
																		>
																			{!isNoAction && (
																				<>
																					<DropdownMenuLabel className="px-3 py-2 text-[9px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">
																						Move job to
																					</DropdownMenuLabel>
																					<DropdownMenuSeparator className="bg-zinc-100 dark:bg-zinc-800 mx-1" />
																			...
																			{showDeliverJob && (
																				<>
																					...
																					<DropdownMenuItem
																						...
																						disabled={
																							chargesReadonlyLoading ===
																							job.id
																						}
																						onClick={() =>
																							void handleOpenChargesReadonly(
																								job,
																							)
																						}
																					>
			...
			<AlertDialog
				open={!!undoFinalPendingJob}
				onOpenChange={(open) => {
					if (!open) setUndoFinalPendingJob(null);
				}}
			>
				<AlertDialogContent className="max-w-sm">
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction
							...
							onClick={() => {
								if (undoFinalPendingJob) void handleUndoFinalConfirm(undoFinalPendingJob);
							}}
						>
							Undo Final
						</AlertDialogAction>
