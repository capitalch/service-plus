...
function buildProformaDoc(
	...
	const infoLine = buildAddrLines([division?.code ?? null, division?.web_site ?? null]).join("   ");
	if (infoLine) {
		const maxW = midX - M - 5;
		doc.setFont("helvetica", "normal");
		doc.setFontSize(8);
	...
	doc.setFontSize(7);
	doc.setTextColor(120, 120, 120);
	doc.text("* This is a PROFORMA INVOICE only and NOT a Tax Invoice. Amounts are subject to change.", M, y, {
		maxWidth: pageW - M * 2,
	});
