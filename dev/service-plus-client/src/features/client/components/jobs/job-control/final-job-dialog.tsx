...
export function FinalJobDialog({ jobId, onClose, onFinalized }: Props) {
	...
	useEffect(() => {
		...
		void Promise.all([
			...
			gq<{ setting_value: unknown }>(SQL_MAP.GET_APP_SETTING_BY_KEY, {
	...
	const loadJobData = async () => {
		...
		try {
			...
			setSelectedRow({
				...
				mobile: job.mobile,
				device_details:
					[job.product_name, job.brand_name, job.model_name, job.serial_no].filter(Boolean).join(" ") || null,
	...
	async function handleDivisionChange(newDivisionId: number) {
		...
		try {
			...
			/* best-effort — don't toast, user is still editing */
		}
	}
