					...
					<Card className="border-(--cl-border) shadow-md bg-(--cl-surface) !overflow-visible">
						<CardContent className="pt-2 grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-x-3 gap-y-3 !overflow-visible">
							...
							<div className="space-y-1.5 md:col-span-3 lg:col-span-3">
								...
								/>
								{errors.alternate_job_no && (
									<p className="mt-1 text-xs text-red-500">{errors.alternate_job_no.message}</p>
								)}
							</div>
							...
							<div className="space-y-1.5 md:col-span-3 lg:col-span-3">
								...
								<Input
									readOnly
									className="bg-(--cl-surface-2) font-mono opacity-70 cursor-not-allowed"
									placeholder="—"
									value={watch("mobile") ?? ""}
								/>
							</div>
							...
							<div className="space-y-1.5 md:col-span-3 lg:col-span-3">
								...
								<select
									...
									onChange={(e) => {
										...
										if (newCode !== "UNDER_WARRANTY")
											setValue("warranty_card_no", "", { shouldValidate: false });
							...
							<div className="space-y-1.5 md:col-span-6 lg:col-span-6">
								<div className="flex items-end gap-2">
									<div className="flex-1">
										<SearchableCombobox<ModelRow>
											...
											onSelect={(m) =>
												setValue("model_id", m ? m.id : (undefined as unknown as number), {
													shouldValidate: true,
												})
											}
								...
								{errors.model_id && (
									<p className="mt-1 text-xs text-red-500">{errors.model_id.message}</p>
								)}
							</div>
						...
						<CardContent className="pt-2 grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-x-3 gap-y-3 !overflow-visible">
							...
							<div className="space-y-1.5 md:col-span-3 lg:col-span-3">
								...
								<select
									...
									onChange={(e) =>
										setValue("technician_id", e.target.value ? Number(e.target.value) : null, {
											shouldValidate: false,
										})
									}
								>
								...
								return (
									<div className="flex flex-wrap items-center gap-2 md:col-span-12 lg:col-span-12">
										...
										<span className="text-[10px] text-(--cl-text-muted)">
											(set automatically from Job Status)
										</span>
									</div>
								);
							})()}
