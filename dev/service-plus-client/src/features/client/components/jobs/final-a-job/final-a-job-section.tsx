...
} from "./job-charges-readonly-modal";
...
export const FinalAJobSection = ({ onBack, initialTab }: FinalAJobSectionProps = {}) => {
	...
	useEffect(() => {
		...
		const fetchMeta = async () => {
			try {
				const [brandsRes, txnRes, markupRes, additionalChargesRes, showPartsSettingRes] = await Promise.all([
					...
					apolloClient.query<GenericQueryData<AdditionalChargeMasterRow>>({
						...
						variables: {
							db_name: dbName,
							schema,
							value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_ALL_ADDITIONAL_CHARGES }),
						},
					}),
					apolloClient.query<GenericQueryData<{ setting_value: unknown }>>({
						fetchPolicy: "network-only",
				...
				const showPartsObj =
					rawShowParts != null && typeof rawShowParts === "object"
						? (rawShowParts as { show?: boolean })
						: null;
				setShowPartsInInvoice(showPartsObj?.show !== false);
				if (rawShowParts != null && typeof rawShowParts === "object")
					setDeliveryPartsInvoiceSetting(
						rawShowParts as { show: boolean; text: string; hsn: number; gst_rate: number },
					);
			} catch {
				/* silent */
			}
		...
		void fetchMeta();
		apolloClient
			.query<GenericQueryData<DeliveryMannerRow>>({
				fetchPolicy: "network-only",
				query: GRAPHQL_MAP.genericQuery,
				variables: {
					db_name: dbName,
					schema,
					value: graphQlUtils.buildGenericQueryValue({ sqlId: SQL_MAP.GET_JOB_DELIVERY_MANNERS }),
				},
			})
			.then((res) => setDeliveryManners(res.data?.genericQuery ?? []))
			.catch(() => {});
	}, [dbName, schema]);

	// ── Load list ───────────────────────────────────────────────────────────
	const loadData = useCallback(
		async (bid: number, q: string, pg: number, divisionId: number | null = null) => {
			if (!dbName || !schema) return;
			setLoading(true);
			const commonArgs = { branch_id: bid, division_id: divisionId, search: q };

			const rowsPromise = apolloClient
				.query({
					fetchPolicy: "network-only",
					query: GRAPHQL_MAP.genericQuery,
					variables: {
			...
			const countPromise = apolloClient
				.query({
					fetchPolicy: "network-only",
					query: GRAPHQL_MAP.genericQuery,
					variables: {
						...
						schema,
						value: graphQlUtils.buildGenericQueryValue({
							sqlId: SQL_MAP.GET_COMPLETED_JOBS_COUNT,
							sqlArgs: commonArgs,
						}),
					},
				})
				.then((res) =>
					setTotal(Number((res.data as GenericQueryData<{ total: number }>).genericQuery?.[0]?.total ?? 0)),
				);
	...
	function handleFinalizedSearchChange(value: string) {
		...
		finalizedDebounceRef.current = setTimeout(() => {
			setFinalizedPage(1);
			setFinalizedSearchQ(value);
		}, SEARCH_DEBOUNCE_MS);
	}
		...
		const countPromise = apolloClient
			.query({
				...
				variables: {
					...
					schema,
					value: graphQlUtils.buildGenericQueryValue({
						sqlId: SQL_MAP.GET_DELIVERABLE_JOBS_COUNT,
						sqlArgs: args,
					}),
				},
			})
			.then((res) =>
				setFinalizedTotal(
					Number((res.data as GenericQueryData<{ total: number }>).genericQuery?.[0]?.total ?? 0),
				),
			);
		...
		try {
			...
			setChargeLines(
				charges.map((c) => ({
					...
					sale_pr_gst: (loadedIsGst
						? c.selling_price * (1 + (c.gst_rate ?? 0) / 100)
						: c.selling_price
					).toFixed(2),
	...
	const [deliveryPartsInvoiceSetting, setDeliveryPartsInvoiceSetting] = useState<{
		show: boolean;
		text: string;
		hsn: number;
		gst_rate: number;
	...
	return (
		<>
			...
			>
				...
				{activeTab === "pending" && (
					<PendingJobsGrid
						...
						onOpenAttach={(id, jobNo) => {
							setAttachSource("pending");
							setAttachJobId(id);
				...
				<JobAttachDialog
					...
					onFilesChanged={(count) => {
						...
						if (attachSource === "finalized") {
							setFinalizedRows((prev) =>
								prev.map((r) => (r.id === id ? { ...r, file_count: count } : r)),
							);
						} else {
			...
			<AlertDialog
				open={!!undoConfirmRow}
				onOpenChange={(open) => {
					if (!open) setUndoConfirmRow(null);
				}}
			>
				<AlertDialogContent className="max-w-sm">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Job <span className="font-medium text-foreground">#{undoConfirmRow?.job_no}</span> will be
							moved back to pending. Are you sure?
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						...
						<AlertDialogAction
							onClick={() => {
								if (undoConfirmRow) {
									void handleUndoFinal(undoConfirmRow);
