...
export async function finalizeJobSave(args: FinalizeJobSaveArgs): Promise<boolean> {
	...
	if (hasTargetFinal && !hasLinesFinal) {
		toast.error(
			"Target amount cannot be set without any parts or charges. Please add at least one part or charge, or clear the target amount.",
		);
		return false;
	}
	...
	try {
		...
		const invalidParts = partLines.some(
			(l) =>
				l.part_id &&
				(l.qty <= 0 || (parseFloat(l.cost_price) || 0) < 0 || (parseFloat(l.selling_price) || 0) < 0),
		);
		const invalidCharges = chargeLines.some(
			(c) =>
				c.charge_name.trim() &&
				((parseFloat(c.qty) || 0) <= 0 ||
					(parseFloat(c.cost_price) || 0) < 0 ||
					(parseFloat(c.selling_price) || 0) < 0),
		);
		if (invalidParts || invalidCharges) {
			toast.error(
				"Qty must be greater than 0 and Cost/Sale prices cannot be negative. Please fix the highlighted rows before finalizing.",
			);
			return false;
		}
		...
		if (zeroCostParts.length > 0 || zeroCostCharges.length > 0) {
			const where = [
				zeroCostParts.length
					? `${zeroCostParts.length} part row${zeroCostParts.length !== 1 ? "s" : ""}`
					: null,
				zeroCostCharges.length
					? `${zeroCostCharges.length} spare/parts charge row${zeroCostCharges.length !== 1 ? "s" : ""}`
					: null,
			]
				.filter(Boolean)
				.join(" and ");
			toast.error(
				`Cost must be greater than 0 on every part row and every spare/parts charge — ${where} still have a cost of 0. Enter the cost on the highlighted rows before finalizing.`,
			);
			return false;
		}
			...
			variables: {
				...
				value: encodeObj({
					...
					xData: {
						...
						amount,
						to_show_parts_in_job_invoice: showPartsInInvoice,
