...
const thRo = "sticky top-0 z-10 bg-(--cl-surface-2) px-2 py-1.5 text-left text-xs font-semibold uppercase tracking-wide text-(--cl-text-muted) border-b border-(--cl-border)/30 whitespace-nowrap";
            ...
            {rows.map(r => {
                ...
                return (
                    <Fragment key={r.label}>
                        {r.dividerBefore && <div className="col-span-3 my-1.5 h-0.5 rounded-full bg-gray-300 dark:bg-gray-600" />}
                        <span className="pr-3 text-[10px] font-medium uppercase tracking-wide text-(--cl-text-muted)">{r.label}</span>
                        <span className={`justify-self-end tabular-nums text-sm font-semibold ${colorClass}`}>
                            {r.negative ? "-" : ""}{r.currency !== false ? "₹" : ""}{whole}
                        </span>
...
export function JobChargesReadonlyPanel({
    onClose, jobNo, isGst, isWarranty, forceIgst, amount, parts, charges, maxWidthClassName = "max-w-5xl",
}: PanelProps) {
    // ── Totals ──────────────────────────────────────────────────────────────
                            ...
                            <SummaryRows
                                rows={[
                                    ...
                                    ...(isGst && !isWarranty
                                        ...
                                        : []),
                                    { amount: isWarranty ? partsCostTotal : partsTotal, dividerBefore: true, label: "Parts" },
