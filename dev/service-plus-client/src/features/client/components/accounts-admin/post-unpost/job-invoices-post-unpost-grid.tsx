...
import type { JobInvoicePostUnpostRow, PostUnpostStats } from "./post-unpost-schema";
...
export function JobInvoicesPostUnpostGrid({
	pendingChanges,
	onChangeToggle,
	onStatsLoaded,
	onRowsLoaded,
	...
	const [rows, setRows] = useState<JobInvoicePostUnpostRow[]>([]);
		...
		async (bId: number, q: string, pg: number) => {
			...
			try {
				...
				const [dataRes, countRes, statsRes] = await Promise.all([
					apolloClient.query<GenericQueryData<JobInvoicePostUnpostRow>>({
						...
						variables: {
							...
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_JOB_INVOICES_POST_UNPOST_PAGED,
					...
					apolloClient.query<GenericQueryData<{ total: number }>>({
						...
						variables: {
							...
							schema,
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_JOB_INVOICES_POST_UNPOST_COUNT,
								sqlArgs: args,
							}),
						},
					}),
					apolloClient.query<GenericQueryData<PostUnpostStats>>({
						...
						variables: {
							...
							schema,
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_JOB_INVOICES_POST_UNPOST_STATS,
								sqlArgs: args,
							}),
						},
					}),
	...
	const allChecked =
		rows.length > 0 && rows.every((r) => (pendingChanges.has(r.id) ? pendingChanges.get(r.id)! : r.is_posted));
	const toggleTarget = !allChecked;
	const affectedCount = rows.filter(
		(r) => (pendingChanges.has(r.id) ? pendingChanges.get(r.id)! : r.is_posted) !== toggleTarget,
	).length;
			...
			<div className="flex flex-wrap items-center gap-2 py-2">
				<div className="relative flex-1 sm:max-w-md">
					...
					<Input
						...
						placeholder="Invoice no, job no, alt job no, customer"
						value={search}
						onChange={(e) => handleSearchChange(e.target.value)}
					/>
					{search && (
						<button
							className="absolute right-2.5 top-1/2 flex h-4 w-4 -translate-y-1/2 items-center justify-center rounded-full bg-(--cl-text-muted) text-(--cl-surface)"
							type="button"
									...
									<th className={thClass}>
										<label className="flex cursor-pointer items-center gap-1.5">
											<span className="flex h-7 w-7 items-center justify-center">
												<input
													type="checkbox"
													className="h-4.5 w-4.5 cursor-pointer accent-(--cl-accent)"
													checked={allChecked}
													onChange={() => setConfirmAllOpen(true)}
												/>
											</span>
											Posted
										</label>
							...
							<tbody>
								{rows.map((row, idx) => {
									...
									return (
										<tr
											key={row.id}
											className={`transition-colors hover:bg-(--cl-accent)/5 ${pending ? "bg-amber-50 dark:bg-amber-900/10" : ""}`}
										>
											...
											<td className={tdClass}>
												<span
													className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${row.gst_type === "GST" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300"}`}
												>
													{row.gst_type}
												</span>
											</td>
											...
											<td className={tdClass}>
												<label className="flex h-7 w-7 cursor-pointer items-center justify-center">
													<input
														...
														className="h-4.5 w-4.5 cursor-pointer accent-(--cl-accent)"
														checked={displayed}
														onChange={() => onChangeToggle(row.id, row.is_posted)}
													/>
												</label>
											</td>
										</tr>
