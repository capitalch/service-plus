...
export function PostUnpostSection() {
	...
	const handleChangeToggle = useCallback(
		(tab: InnerTab) => (id: number, currentDbValue: boolean) => {
			setPending(tab)((prev) => {
		...
		>
			...
			<div className="flex flex-wrap items-center gap-3 px-4">
				<div className="flex items-center gap-1 rounded-lg bg-(--cl-surface-2) p-1">
					<TabBtn
						active={innerTab === "receipts"}
						label="Money Receipts"
						onClick={() => setInnerTab("receipts")}
					/>
					<TabBtn
						active={innerTab === "purchase"}
						label="Purchase Invoices"
						onClick={() => setInnerTab("purchase")}
					/>
					...
					>
						{saving ? (
							...
							<>
								...
								<span>Save{activePending.size > 0 ? ` (${activePending.size})` : ""}</span>
							</>
						)}
					</button>
				</div>
			...
			<div className="flex min-h-0 flex-1 flex-col overflow-hidden px-4 pb-4">
				<div
					className={`flex min-h-0 flex-1 flex-col overflow-hidden ${innerTab !== "receipts" ? "hidden" : ""}`}
				>
				...
				<div
					className={`flex min-h-0 flex-1 flex-col overflow-hidden ${innerTab !== "purchase" ? "hidden" : ""}`}
				>
