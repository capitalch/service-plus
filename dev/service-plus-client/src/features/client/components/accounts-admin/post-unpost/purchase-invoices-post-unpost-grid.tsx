...
import type { PurchaseInvoicePostUnpostRow, PostUnpostStats } from "./post-unpost-schema";
...
export function PurchaseInvoicesPostUnpostGrid({
	pendingChanges,
	onChangeToggle,
	onStatsLoaded,
	onRowsLoaded,
	...
	const [rows, setRows] = useState<PurchaseInvoicePostUnpostRow[]>([]);
		...
		async (bId: number, q: string, pg: number) => {
			...
			try {
				...
				const [dataRes, countRes, statsRes] = await Promise.all([
					apolloClient.query<GenericQueryData<PurchaseInvoicePostUnpostRow>>({
						...
						variables: {
							...
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_PURCHASE_INVOICES_POST_UNPOST_PAGED,
					...
					apolloClient.query<GenericQueryData<{ total: number }>>({
						...
						variables: {
							...
							schema,
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_PURCHASE_INVOICES_POST_UNPOST_COUNT,
								sqlArgs: args,
							}),
						},
					}),
					apolloClient.query<GenericQueryData<PostUnpostStats>>({
						...
						variables: {
							...
							schema,
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_PURCHASE_INVOICES_POST_UNPOST_STATS,
								sqlArgs: args,
							}),
						},
					}),
