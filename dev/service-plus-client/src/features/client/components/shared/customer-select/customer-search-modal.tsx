				...
				>
					...
					<div className="relative flex items-center gap-3 pr-8">
						...
						<div className="flex-1 min-w-0">
							...
							<p className="mt-0.5 text-[11px] text-white/70">Search by name, mobile, GSTIN or address</p>
						</div>
					</div>
				</DialogHeader>
						...
						<span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2">
							{loading ? (
								<Loader2
									...
									style={{ color: "var(--cl-accent, #007acc)" }}
								/>
							) : (
				...
				<div className="overflow-y-auto flex-1 min-h-[200px]">
					...
					{results.map((row) => (
						...
						>
							{/* Hover accent strip */}
							<span
								className="absolute left-0 inset-y-0 w-[3px] scale-y-0 group-hover:scale-y-100 transition-transform origin-center rounded-r"
								style={{ background: "var(--cl-accent, #007acc)" }}
							/>
							...
							<div className="flex items-start justify-between gap-3 mb-1">
								<span className="font-semibold text-sm text-(--cl-text) leading-snug">
									{row.full_name ?? <span className="italic text-zinc-400 font-normal">No name</span>}
								</span>
							...
							<div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 text-xs text-zinc-500">
								...
								{row.state_name && <span className="text-zinc-400">{row.state_name}</span>}
							</div>
