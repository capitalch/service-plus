			...
			<div className={`flex flex-col gap-0.5 py-1${className ? ` ${className}` : ""}`}>
				<Popover open={dropdownOpen && results.length > 0} onOpenChange={setDropdownOpen}>
					...
					<PopoverContent
						...
						style={{
							width: dropdownWidth > 0 ? `${dropdownWidth}px` : "var(--radix-popover-anchor-width)",
						}}
					...
					>
						...
						{results.map((row) => {
							...
							return (
								...
								>
									<div
										className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold ${AVATAR_COLORS[colorIdx]}`}
									>
										{initial}
									</div>
									<div className="flex-1 min-w-0">
										<div className="flex items-baseline justify-between gap-2">
											<span className="truncate text-sm font-medium text-(--cl-text)">
												{row.full_name ?? (
													<span className="italic text-(--cl-text-muted)">No name</span>
												)}
											</span>
											<span className="shrink-0 font-mono text-xs text-(--cl-text-muted)">
												{row.mobile}
											</span>
										</div>
