			...
			<PopoverAnchor asChild>
				<div ref={anchorRef} className="relative">
					<Input
						...
						onFocus={() => {
							if (showOnEmpty || inputText.trim()) setOpen(true);
						}}
						onClick={() => {
							if (showOnEmpty || inputText.trim()) setOpen(true);
						}}
				...
				{filtered.length === 0 ? (
					...
					filtered.map((item) => {
						...
						return (
							...
							>
								<div
									className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${AVATAR_COLORS[colorIdx]}`}
								>
									{initial}
								</div>
								<div className="flex-1 min-w-0">
									<div className="flex items-center justify-between gap-2">
										<span
											className={`truncate text-sm ${isSelected ? "font-semibold text-blue-700" : "font-medium text-(--cl-text)"}`}
										>
											{label}
										</span>
