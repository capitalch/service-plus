...
function SectionLabel({
	...
	return (
		<div className="flex items-center gap-2 mb-1">
			<span
				className={`flex h-6 w-6 items-center justify-center rounded ${iconClass ?? "bg-(--cl-accent)/10 text-(--cl-accent)"}`}
			>
				{icon}
			</span>
			<p className="text-xs font-semibold uppercase tracking-wider text-(--cl-text)">{children}</p>
		</div>
	);
}
...
function buildAccountSetting(division: DivisionType) {
	...
	return {
		...
		salesInvoice: as.salesInvoice ? { ...as.salesInvoice } : { ...DEFAULT_SALE_INVOICE },
		jobInvoice: as.jobInvoice ? { ...as.jobInvoice } : { ...DEFAULT_SALE_INVOICE },
	};
}

// ─── Component ────────────────────────────────────────────────────────────────

export const EditDivisionDialog = ({ division, onOpenChange, onSuccess, open }: EditDivisionDialogPropsType) => {
	...
	useEffect(() => {
		if (!debouncedName || !dbName || !schema || !currentBranch) {
			setNameTaken(null);
			return;
		}
		...
		if (debouncedName === division.name) {
			setNameTaken(false);
			return;
		}
		...
		apolloClient
			...
			.then((res) => {
				...
				setNameTaken(exists);
				if (exists)
					form.setError("name", { message: MESSAGES.ERROR_DIVISION_NAME_EXISTS_EDIT, type: "manual" });
	...
	useEffect(() => {
		if (!debouncedCode || !dbName || !schema || !currentBranch) {
			setCodeTaken(null);
			return;
		}
		if (form.getFieldState("code").invalid) {
			setCodeTaken(null);
			return;
		}
		if (debouncedCode.toUpperCase() === division.code.toUpperCase()) {
			setCodeTaken(false);
			return;
		}
		...
		apolloClient
			...
			.then((res) => {
				...
				setCodeTaken(exists);
				if (exists)
					form.setError("code", { message: MESSAGES.ERROR_DIVISION_CODE_EXISTS_EDIT, type: "manual" });
	...
	async function onSubmit(data: DivisionFormValues) {
		...
		const accountSettingValue =
			postDataToAccounts && as?.clientCode
				? {
						clientCode: as.clientCode,
						buCode: as.buCode,
	...
	const submitDisabled =
		checkingName || checkingCode || nameTaken === true || codeTaken === true || form.formState.isSubmitting;
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				<DialogHeader className="shrink-0">
					<DialogTitle className="text-base font-semibold text-foreground">Edit Division</DialogTitle>
				</DialogHeader>
						...
						<TabsContent value="details">
							<div className="flex flex-col gap-3 pt-1 overflow-y-auto min-h-0 flex-1 pr-1">
								...
								<div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
									<p className="mb-3 text-[11px] font-bold uppercase tracking-widest text-blue-600">
										Address
									</p>
									<div className="flex flex-col gap-3">
										<div className="flex flex-col gap-1.5">
										...
										<div className="grid grid-cols-2 gap-3">
											<div className="flex flex-col gap-1.5">
												...
												<Select
													...
													onValueChange={(v) =>
														form.setValue("state_id", Number(v), { shouldValidate: true })
													}
												>
								...
								<div className="flex flex-col gap-4 pt-1 overflow-y-auto min-h-0 flex-1 pr-1">
									...
									<div className="rounded-lg border border-sky-200 bg-sky-50 px-4 py-3">
										...
										<div className="grid grid-cols-3 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_client_code" className="text-xs">
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_bu_code" className="text-xs">
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_branch_id" className="text-xs">
									...
									<div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 shadow-sm">
										<SectionLabel
											icon={<Receipt className="h-3.5 w-3.5 text-green-600" />}
										...
										<div className="mt-3 grid grid-cols-2 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_r_debit" className="text-xs">
													Debit A/c ID
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_r_credit" className="text-xs">
													Credit A/c ID
												</Label>
												<Input
													autoComplete="off"
									...
									<div className="rounded-lg border border-violet-200 bg-violet-50 px-4 py-3 shadow-sm">
										<SectionLabel
											icon={<ShoppingCart className="h-3.5 w-3.5 text-green-600" />}
										...
										<div className="mt-3 grid grid-cols-2 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_pi_debit" className="text-xs">
													Debit A/c ID
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_pi_credit" className="text-xs">
													Credit A/c ID
												</Label>
												<Input
													autoComplete="off"
													...
													onFocus={(e) => e.target.select()}
													{...form.register(
														"account_setting.purchaseInvoice.creditAccountId",
													)}
												/>
											</div>
										</div>
										<div className="mt-3 grid grid-cols-3 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_pi_prod" className="text-xs">
													Product Id
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_pi_hsn" className="text-xs">
													Default HSN
												</Label>
												<Input
													autoComplete="off"
													...
													onFocus={(e) => e.target.select()}
													{...form.register(
														"account_setting.purchaseInvoice.defaultProductHsn",
													)}
												/>
											</div>
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_pi_gst" className="text-xs">
													Default GST Rate %
												</Label>
												<Input
													autoComplete="off"
									...
									<div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 shadow-sm">
										<SectionLabel
											icon={<Tag className="h-3.5 w-3.5 text-slate-600" />}
										...
										<div className="mt-3 grid grid-cols-2 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_si_debit" className="text-xs">
													Debit A/c ID
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_si_credit" className="text-xs">
													Credit A/c ID
												</Label>
												<Input
													autoComplete="off"
										...
										<div className="mt-3 grid grid-cols-3 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_si_prod" className="text-xs">
													Product Id
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_si_hsn" className="text-xs">
													Default HSN
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_si_gst" className="text-xs">
													Default GST Rate %
												</Label>
												<Input
													autoComplete="off"
										...
										<div className="mt-3 grid grid-cols-3 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_si_contacts" className="text-xs">
													Contacts ID
												</Label>
												<Input
													autoComplete="off"
									...
									<div className="rounded-lg border border-teal-200 bg-teal-50 px-4 py-3 shadow-sm">
										...
										<div className="mt-3 grid grid-cols-2 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_ji_debit" className="text-xs">
													Debit A/c ID
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_ji_credit" className="text-xs">
													Credit A/c ID
												</Label>
												<Input
													autoComplete="off"
										...
										<div className="mt-3 grid grid-cols-3 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_ji_prod" className="text-xs">
													Product Id
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_ji_hsn" className="text-xs">
													Default HSN
												</Label>
												<Input
													autoComplete="off"
											...
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_ji_gst" className="text-xs">
													Default GST Rate %
												</Label>
												<Input
													autoComplete="off"
										...
										<div className="mt-3 grid grid-cols-3 gap-3">
											<div className="flex flex-col gap-1.5">
												<Label htmlFor="edv_ji_contacts" className="text-xs">
													Contacts ID
												</Label>
												<Input
													autoComplete="off"
