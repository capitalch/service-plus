...
export const DocumentSequenceSection = () => {
	...
	const submitDisabled =
		Object.keys(errors).length > 0 || form.formState.isSubmitting || loading || !currentBranch?.id;

	const activeDivision =
		typeof activeTab === "number" ? (availableDivisions.find((d) => d.id === activeTab) ?? null) : null;
						...
						<div className="flex flex-col divide-y divide-(--cl-border)">
							{fields.map((field, index) => {
								...
								return (
									<div
										key={field.id}
										className="grid grid-cols-12 items-center gap-4 p-4 hover:bg-(--cl-surface-2)/30"
									>
										...
										<div className="col-span-3">
											...
											/>
											{err?.prefix && (
												<p className="mt-1 text-[10px] text-red-500">{err.prefix.message}</p>
											)}
										</div>
										<div className="col-span-1">
											...
											/>
											{err?.separator && (
												<p className="mt-1 text-[10px] text-red-500">{err.separator.message}</p>
											)}
										</div>
										<div className="col-span-2">
											<Input
												...
												className="h-8 shadow-none"
												type="number"
												{...form.register(`sequences.${index}.padding`, {
													valueAsNumber: true,
												})}
											/>
											{err?.padding && (
												<p className="mt-1 text-[10px] text-red-500">{err.padding.message}</p>
											)}
										</div>
										<div className="col-span-2">
											<Input
												...
												className="h-8 shadow-none"
												type="number"
												{...form.register(`sequences.${index}.next_number`, {
													valueAsNumber: true,
												})}
											/>
											{err?.next_number && (
												<p className="mt-1 text-[10px] text-red-500">
													{err.next_number.message}
												</p>
											)}
										</div>
									</div>
