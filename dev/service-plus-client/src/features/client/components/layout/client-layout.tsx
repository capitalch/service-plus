...
type ClientSelectionContextType = {
	onSelect: (label: string, group?: string) => void;
	selected: string;
...
export const ClientLayout = ({ children }: ClientLayoutProps) => {
	...
	useEffect(() => {
		...
		void apolloClient
			...
			.then((res) => {
				...
				const rawSvcHsn = settings.find(
					(s) => s.setting_key === "default_hsn_for_service_charge",
				)?.setting_value;
				...
				const rawInvCopies = settings.find(
					(s) => s.setting_key === "no_of_job_invoices_per_print",
				)?.setting_value;
				...
				const rawRcptCopies = settings.find(
					(s) => s.setting_key === "no_of_job_receipts_per_print",
				)?.setting_value;
	...
	const displayTitle = [SECTION_LABELS[activeSection], selectedGroup, selected].filter(Boolean).join(" > ");
				...
				<HelpContext.Provider value={{ helpOpen, openHelp, closeHelp }}>
					<ClientSelectionContext.Provider value={{ onSelect, selected, selectedGroup }}>
						<PortalContainerContext.Provider value={themeRoot}>
							<div
								ref={setThemeRoot}
								className="client-theme relative h-full bg-(--cl-bg) text-(--cl-text)"
										...
										{currentDivision && (
											<div className="flex items-center gap-2 rounded-md bg-indigo-50 px-2.5 py-1.5 leading-none dark:bg-indigo-950/40">
												...
												>
													...
													<span
														className={`text-[10px] font-bold uppercase tracking-tighter ${isGstMode ? "text-emerald-700" : "text-amber-700"}`}
													>
