...
const ACTIVITY_ITEMS: ActivityItem[] = [
	...
	{ color: "text-sky-600", icon: BookOpen, section: "masters", to: ROUTES.client.masters, title: "Masters" },
	{
		...
		to: ROUTES.client.configurations,
			...
			<div className="flex w-full flex-col items-center gap-6">
				{activityItems.map(({ color, icon: Icon, section, to, title }) => {
					...
					return (
						...
						>
							<Icon
								className={isActive ? "h-5 w-5" : `h-5 w-5 ${color}`}
			...
			<div className="mt-auto flex w-full flex-col items-center gap-4 pb-4">
				...
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<button
							className="cursor-pointer text-(--cl-text-muted) transition-all hover:text-(--cl-text)"
							title="Account"
						>
					...
					<DropdownMenuContent align="end" side="right" className="w-60">
						...
						<div className="space-y-1 px-1.5 py-1 text-xs text-slate-500">
							{user?.username && (
								<p>
									Username: <span className="text-slate-700">{user.username}</span>
								</p>
							)}
							{user?.roleName && (
								<p>
									Role: <span className="text-slate-700">{user.roleName}</span>
								</p>
							)}
							{currentBu?.name && (
								<p>
									BU: <span className="text-slate-700">{currentBu.name}</span>
								</p>
							)}
							{user?.mobile && (
								<p>
									Mobile: <span className="text-slate-700">{user.mobile}</span>
								</p>
							)}
							{user?.clientCode && (
								<p>
									Client: <span className="text-slate-700">{user.clientCode}</span>
								</p>
							)}
						</div>
						<DropdownMenuSeparator />
