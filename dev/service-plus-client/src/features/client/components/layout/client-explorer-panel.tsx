...
function TreeItem({ disabled, icon: Icon, iconColor, label, title, helpArticleId }: TreeItemProps) {
	...
	return (
		<div className="flex items-center gap-1">
			<div
				onClick={() => {
					if (!disabled) onSelect(label, group || undefined);
				}}
			...
			<div className="space-y-1">
				<CollapsibleGroup label="New Job">
					<TreeItem
						...
						label="Single Job"
						helpArticleId="create-job"
					/>
					<TreeItem
						icon={PlusCircle}
						iconColor="text-emerald-600"
						label="Batch Jobs"
						helpArticleId="batch-jobs"
					/>
				</CollapsibleGroup>
				<TreeItem
					icon={ClipboardList}
...
export const ClientExplorerPanel = ({ activeSection }: Props) => {
	...
	return (
		<aside
			className={`fixed top-12 z-30 flex h-[calc(100%-4.5rem)] w-64 flex-col border-r border-(--cl-border) bg-(--cl-surface) transition-transform duration-200 ease-in-out left-0 md:left-16 ${translateClass}`}
		>
