...
export const DeletePartLocationDialog = ({
    ...
    const deleteEnabled = inUse === false && !checkingInUse && confirmValue.toLowerCase() === location.location.toLowerCase();
    ...
    async function handleDelete() {
        ...
        try {
            ...
            toast.success(MESSAGES.SUCCESS_PART_LOCATION_DELETED);
            onSuccess();
            onOpenChange(false);
        } catch {
            toast.error(MESSAGES.ERROR_PART_LOCATION_DELETE_FAILED);
        } finally {
            setSubmitting(false);
        }
    }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle>Delete Part Location</DialogTitle>
                    <DialogDescription>
                        Permanently delete{" "}
                        <span className="font-semibold text-slate-800">{location.location}</span>?
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    {inUse === true && (
                        <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
                            ...
                            <p className="text-sm text-amber-800">{MESSAGES.ERROR_PART_LOCATION_DELETE_IN_USE}</p>
                        </div>
                    )}

                    {inUse !== true && (
                        <div className="flex flex-col gap-1.5">
                            <Label htmlFor="confirm_part_location">
                                Type{" "}
                                <span className="font-semibold text-slate-800">{location.location}</span>{" "}
                                to confirm
                            </Label>
