...
export const StockLedgerSection = () => {
	...
	useEffect(() => {
		const timer = setTimeout(() => {
			if (partCode.trim()) void resolvePart();
		}, 600);
	...
	const COLUMNS: ReportColumnType<DisplayRowType>[] = [
		...
		{
			align: "right",
			cell: (r) => <span className="font-bold text-(--cl-accent-text)">{formatNumber(r.balance)}</span>,
			header: "Balance",
			id: "b",
			value: (r) => r.balance,
			width: "100px",
		...
		{
			align: "right",
			cell: (r) => (r.unit_cost != null ? formatInr(Number(r.unit_cost)) : "—"),
			header: "Unit Cost",
			id: "uc",
			value: (r) => Number(r.unit_cost ?? 0),
			width: "110px",
		},
	...
	return (
		<ReportSection>
			...
			<ChartCard
				description="Running balance is computed client-side"
				title={resolvedPart ? `${resolvedPart.part_code} — ${resolvedPart.part_name}` : "Pick a part"}
			>
				{!resolvedPart ? (
					...
					<ReportTable columns={COLUMNS} rowKey={(r) => r.id} rows={rows} stickyHeader={false} />
				)}
			</ChartCard>
		</ReportSection>
	);
