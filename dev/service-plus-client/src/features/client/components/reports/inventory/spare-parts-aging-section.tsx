...
export const SparePartsAgingSection = ({ initialFilterOnly1Yr = false, title = "Spare Parts Aging" }: Props = {}) => {
	...
	const totals = useMemo(() => {
		...
		for (const r of filtered) {
			...
			if (Number(r.age_days) > 365) {
				aged += Number(r.stock_qty);
				agedV += Number(r.stock_value);
			}
		}
	...
	const COLUMNS: ReportColumnType<RowType>[] = [
		...
		{
			align: "right",
			cell: (r) => formatNumber(Number(r.stock_qty)),
			footer: (rs) => formatNumber(rs.reduce((s, r) => s + Number(r.stock_qty), 0)),
			header: "Stock Qty",
			id: "q",
			value: (r) => Number(r.stock_qty),
			width: "100px",
		},
		{
			align: "right",
			cell: (r) => formatInr(Number(r.stock_value)),
			footer: (rs) => formatInr(rs.reduce((s, r) => s + Number(r.stock_value), 0)),
			header: "Stock ₹",
			id: "v",
			value: (r) => Number(r.stock_value),
			width: "120px",
		},
		{
			cell: (r) => (r.last_in_date ? formatDateShort(r.last_in_date) : "—"),
			header: "Last In",
			id: "in",
			value: (r) => r.last_in_date ?? "",
			width: "110px",
		},
		{
			align: "center",
	...
	return (
		<ReportSection>
			...
			<KpiGrid columns={4}>
				...
				<KpiCard
					label="Aged > 1yr Value"
					loading={q.loading}
					value={formatInr(totals.agedV)}
