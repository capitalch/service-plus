...
export const WarrantyJobDetailDialog = ({ jobId, jobNo, onClose }: Props) => {
	...
	useEffect(() => {
		...
		apolloClient
			.query<{ genericQuery: WarrantyDetailLineType[] | null }>({
		...
		>
			<DialogContent className="sm:max-w-3xl">
				<DialogHeader>
					...
					<DialogDescription>Parts consumed against this in-warranty job.</DialogDescription>
				</DialogHeader>
				...
				<div>
					...
					{!loading && !error && rows.length === 0 && (
						<ReportEmpty message="No parts consumed for this job." />
					)}
					{!loading && !error && rows.length > 0 && (
						<ReportTable
							columns={COLUMNS}
