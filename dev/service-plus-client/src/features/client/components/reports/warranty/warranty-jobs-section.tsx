	...
	return (
		<ReportSection>
			<ReportToolbar
				...
				title="Warranty Jobs"
			>
				<WarrantyRangeTabs fyStartMonth={fyStartMonth} onChange={setRange} range={range} />
			</ReportToolbar>
			...
			<KpiGrid columns={4}>
				<KpiCard
					...
					subValue={
						summary
							? `Repaired ${summary.repaired_count} · Delivered ${summary.delivered_count}`
							: undefined
					}
			...
			<ChartCard
				description="Per-part roll-up across all in-warranty jobs in range"
				title="Parts Consumption — Breakdown"
			>
				{partsQ.loading ? (
					<ReportLoading lines={4} />
				) : partsQ.data.length === 0 ? (
