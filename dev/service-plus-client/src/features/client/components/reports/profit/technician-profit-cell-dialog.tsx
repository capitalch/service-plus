...
export const TechnicianProfitCellDialog = ({ cell, onClose }: Props) => {
	...
	const columns: ReportColumnType<CellJobType>[] = [
		...
		},
		{
			align: "right",
			cell: (r) => (
				<span className="font-light text-amber-600 dark:text-amber-400">
					{formatNumber(Number(r.total_charges))}
				</span>
			),
				...
				>
					...
					<div className="min-w-0">
						...
						{!loading && !error && rows.length === 0 && (
							<ReportEmpty message="No delivered jobs in this month." />
						)}
						{!loading && !error && rows.length > 0 && (
							<>
								<div className="mb-2 text-xs text-(--cl-text-muted)">{rows.length} job(s)</div>
								<ReportTable
									columns={columns}
									maxHeight="60vh"
									rowKey={(r) => r.id}
