...
const MONTH_LABELS: string[] = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
...
function getRelativeFiscalQuarterBounds(
	today: Date,
	fyStart: number,
): { lastQuarter: DateRangeType; thisQuarter: DateRangeType } {
	const fyStartIdx = clampMonth(fyStart - 1);
	...
	return {
		lastQuarter: {
			...quarterBoundsFromOffset(thisQOffset - 3),
			key: "lastQuarter",
			label: RANGE_LABELS.lastQuarter,
		},
...
export function getRange(
	...
	if (key === "custom" && custom) {
		return {
			...
			key,
			label: formatRangeLabel(custom.from, custom.to),
			to: endOfDay(custom.to),
		};
	}
	switch (key) {
		case "today":
			return { from: startOfDay(today), key, label: RANGE_LABELS.today, to: endOfDay(today) };
		case "yesterday": {
		...
		case "q2":
			return getFiscalQuarterBounds(2, today, fyStart);
		case "q3":
			return getFiscalQuarterBounds(3, today, fyStart);
		case "q4":
			return getFiscalQuarterBounds(4, today, fyStart);
		case "thisQuarter":
			return getRelativeFiscalQuarterBounds(today, fyStart).thisQuarter;
		case "lastQuarter":
			return getRelativeFiscalQuarterBounds(today, fyStart).lastQuarter;
		case "ytd": {
			const fy = getCurrentFiscalYearBounds(today, fyStart);
			return { from: fy.from, key, label: RANGE_LABELS.ytd, to: endOfDay(today) };
		}
		case "lastYear":
			return getPreviousFiscalYearBounds(today, fyStart);
		case "custom":
			return { from: startOfDay(today), key, label: RANGE_LABELS.custom, to: endOfDay(today) };
	}
}
