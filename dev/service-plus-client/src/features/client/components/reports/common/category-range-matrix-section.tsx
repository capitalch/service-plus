...
const DEFAULT_MATRIX_DESCRIPTION =
	"Product category vs. standard date buckets. Ranges overlap (e.g. Today is included in This Week, Q1, and YTD), so the Total row sums each bucket down its column (across categories), not across buckets. A category with no jobs in any bucket is not listed.";
...
export const CategoryRangeMatrixSection = ({
	description,
	drillDownSqlId,
	fileSlug,
	financialCaveat,
	...
	rowLabel = "Category",
	rowOrder,
	showFinancialsInDrillDown = false,
	showFinancialToggles = false,
	sqlId,
	title,
}: Props) => {
				...
				return (
					<button
						className={cn(
							"w-full",
							clickable &&
								"cursor-pointer rounded hover:ring-2 hover:ring-(--cl-accent) hover:ring-inset",
						)}
						disabled={!clickable}
						type="button"
						onClick={
			...
			exportReportPdf({
				...
				rows: matrix.rows.map((r) => {
					...
					CATEGORY_BUCKET_COLUMNS.forEach((b) => {
						row[b.field] = formatWarrantySplit(r[b.field]);
					});
					return row;
				}),
				title,
				totalsRow: (() => {
					...
					CATEGORY_BUCKET_COLUMNS.forEach((b) => {
						row[b.field] = formatWarrantySplit(sumSplit(matrix.rows, b.field));
					});
					return row;
				})(),
			});
				...
				sheets: [
					{
						...
						rows: [
							...matrix.rows.map((r) => {
								const row: Record<string, number | string> = { Category: r.category };
								CATEGORY_BUCKET_COLUMNS.forEach((b) => {
									row[b.label] = formatWarrantySplit(r[b.field]);
								});
								return row;
							}),
							(() => {
								const row: Record<string, number | string> = { Category: "TOTAL" };
								CATEGORY_BUCKET_COLUMNS.forEach((b) => {
									row[b.label] = formatWarrantySplit(sumSplit(matrix.rows, b.field));
								});
								return row;
							})(),
						],
			...
			<ChartCard description={matrixDescription ?? DEFAULT_MATRIX_DESCRIPTION}>
				{(showSplit || showRevenue || showProfit) && (
					<div className="mb-2 flex flex-wrap items-center gap-4 text-[11px] font-semibold text-(--cl-text-muted)">
						...
						{showRevenue && (
							<span className="inline-flex items-center gap-1.5">Revenue shown in (parentheses)</span>
						)}
					</div>
				)}
