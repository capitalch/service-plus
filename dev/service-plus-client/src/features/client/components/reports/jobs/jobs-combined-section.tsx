...
const STAGE_DETAIL: Record<JobStageKeyType, { reportTitle: string; showFinancials: boolean; sqlId: string }> = {
	delivered: {
		reportTitle: "Jobs Delivered (OK)",
		showFinancials: true,
		sqlId: SQL_MAP.GET_JOBS_DELIVERED_OK_DETAIL,
	},
	...
	function buildExportRows(categoryKey: string, stageKey: string): Record<string, number | string>[] {
		return rows.flatMap((r) => {
			const base: Record<string, number | string> = {
				[categoryKey]: r.isFirstOfGroup ? r.category : "",
				[stageKey]: r.stageLabel,
			};
			CATEGORY_BUCKET_COLUMNS.forEach((b) => {
				base[b.field] = formatWarrantySplit(r.cells[b.field]);
			});
			const extra: Record<string, number | string>[] = [];
			if (r.stage === "delivered" && showProfit) {
				const row: Record<string, number | string> = { [categoryKey]: "", [stageKey]: "  Profit" };
				CATEGORY_BUCKET_COLUMNS.forEach((b) => {
					row[b.field] = formatInr(r.cells[b.field].profit_amount);
				});
				extra.push(row);
			}
			if (r.stage === "delivered" && showRevenue) {
				...
				CATEGORY_BUCKET_COLUMNS.forEach((b) => {
					row[b.field] = formatInr(r.cells[b.field].revenue_amount);
				});
				extra.push(row);
			}
			return [base, ...extra];
			...
			<ChartCard description={MATRIX_DESCRIPTION}>
				...
				{matrix.loading ? (
					...
					<ReportTable
						columns={columns}
						rowClassName={(r) =>
							r.isTotal ? "bg-(--cl-surface-3)" : r.isFirstOfGroup ? "border-t border-(--cl-border)" : ""
						}
