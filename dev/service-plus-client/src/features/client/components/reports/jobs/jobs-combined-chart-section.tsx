	...
	function handleXlsxExport() {
		try {
			...
			exportReportXlsx({
				...
				sheets: [
					{
						columns: columns("Period", "period"),
						name: "By Period",
						rows: buildExportRows(periodData, "period"),
					},
					{
						columns: columns("Category", "category"),
						name: `By Category - ${bucketLabel}`,
						rows: buildExportRows(categoryData, "category"),
					},
				],
			});
			toast.success(MESSAGES.SUCCESS_REPORTS_EXPORTED);
		...
		<ReportSection>
					...
					<Select onValueChange={(v) => setBucket(v as CategoryBucketFieldType)} value={bucket}>
						...
						<SelectContent>
							{CATEGORY_BUCKET_COLUMNS.map((b) => (
								<SelectItem key={b.field} value={b.field}>
									{b.label}
								</SelectItem>
							))}
						</SelectContent>
			...
			<KpiGrid columns={4}>
				...
				<KpiCard
					...
					icon={Wrench}
					label={`Repaired — ${bucketLabel}`}
					loading={matrix.loading}
					subValue={
						received ? `${(((kpi?.repaired ?? 0) / received) * 100).toFixed(0)}% of received` : undefined
					}
					value={formatNumber(kpi?.repaired ?? 0)}
				/>
				<KpiCard
					accentClassName="text-emerald-500"
					icon={Truck}
					label={`Delivered — ${bucketLabel}`}
					loading={matrix.loading}
					subValue={received ? `${((delivered / received) * 100).toFixed(0)}% of received` : undefined}
					value={formatNumber(delivered)}
				/>
				<KpiCard
					accentClassName="text-teal-500"
					icon={IndianRupee}
					label={`Profit — ${bucketLabel}`}
					loading={matrix.loading}
					subValue={`Revenue ${formatInr(kpi?.revenue ?? 0)}`}
					value={formatInr(kpi?.profit ?? 0)}
				/>
			</KpiGrid>
			...
			>
				{matrix.loading ? (
					...
					<StageChart data={periodData} showProfit={showProfit} showRevenue={showRevenue} xLabel="Period" />
				)}
			</ChartCard>
