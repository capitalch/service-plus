...
export const DashboardSection = () => {
	...
	const todayRange = useMemo(
		() => getRange(selectedRange.key, new Date(), fyStartMonth),
		[selectedRange.key, fyStartMonth],
	);
	...
	const rangeDescription = todayArgs.from === todayArgs.to ? todayArgs.from : `${todayArgs.from} – ${todayArgs.to}`;
		...
		<ReportSection>
			...
			<KpiGrid columns={4}>
				<KpiCard
					...
					subValue={
						kpis
							? `W ${formatNumber(kpis.jobs_received_warranty)} / OOW ${formatNumber(kpis.jobs_received_oow)}`
							: undefined
					}
				...
				<KpiCard
					...
					subValue={
						kpis
							? `W ${formatNumber(kpis.jobs_open_warranty)} / OOW ${formatNumber(kpis.jobs_open_oow)}`
							: undefined
					}
			...
			<div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
				...
				<ChartCard description="Open jobs needing follow-up" title="Alerts">
					{overdueQ.loading ? <ReportLoading lines={3} /> : <DashboardAlertsPanel overdue={overdueQ.data} />}
				</ChartCard>
			</div>
