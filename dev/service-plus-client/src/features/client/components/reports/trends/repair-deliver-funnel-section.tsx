		...
		<ReportSection>
			...
			<KpiGrid columns={4}>
				...
				<KpiCard
					...
					label="Repaired"
					loading={q.loading}
					value={formatNumber(repaired)}
					subValue={received ? `${((repaired / received) * 100).toFixed(0)}%` : undefined}
				/>
				<KpiCard
					...
					subValue={received ? `${((delivered / received) * 100).toFixed(0)}%` : undefined}
				/>
				<KpiCard
					label="Conversion"
					loading={q.loading}
					value={received ? `${((delivered / received) * 100).toFixed(1)}%` : "0%"}
				/>
			</KpiGrid>
				...
				{q.loading ? (
					...
					<div className="flex flex-col gap-3">
						<div className="space-y-1">
							<p className="text-[10px] font-bold uppercase tracking-wider text-(--cl-text-muted)">
								Received
							</p>
