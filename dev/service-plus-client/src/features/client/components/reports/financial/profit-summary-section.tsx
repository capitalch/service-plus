...
export const ProfitSummarySection = () => {
	...
	return (
		<ReportSection>
			...
			<ChartCard
				description="Profit = net invoice value (excl. GST) − parts cost − charges cost. Ranges overlap (e.g. Today is included in This Week, Q1, and YTD), so rows are not additive — no grand total is shown."
				title="Profit by Range"
			>
				{loading ? (
					...
					<ReportTable columns={COLUMNS} rowKey={(r) => r.key} rows={rows} stickyHeader={false} />
				)}
			</ChartCard>
		</ReportSection>
	);
