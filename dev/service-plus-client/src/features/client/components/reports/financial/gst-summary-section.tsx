...
const COLUMNS: ReportColumnType<RowType>[] = [
	...
	{
		align: "right",
		cell: (r) => formatInr(Number(r.aggregate)),
		footer: (rs) => formatInr(rs.reduce((s, r) => s + Number(r.aggregate), 0)),
		header: "Aggregate",
		id: "agg",
		value: (r) => Number(r.aggregate),
	},
	{
		align: "right",
		cell: (r) => formatInr(Number(r.cgst)),
		footer: (rs) => formatInr(rs.reduce((s, r) => s + Number(r.cgst), 0)),
		header: "CGST",
		id: "cgst",
		value: (r) => Number(r.cgst),
	},
	{
		align: "right",
		cell: (r) => formatInr(Number(r.sgst)),
		footer: (rs) => formatInr(rs.reduce((s, r) => s + Number(r.sgst), 0)),
		header: "SGST",
		id: "sgst",
		value: (r) => Number(r.sgst),
	},
	{
		align: "right",
		cell: (r) => formatInr(Number(r.igst)),
		footer: (rs) => formatInr(rs.reduce((s, r) => s + Number(r.igst), 0)),
		header: "IGST",
		id: "igst",
		value: (r) => Number(r.igst),
	},
	{
		align: "right",
		cell: (r) => <span className="font-bold text-(--cl-accent-text)">{formatInr(Number(r.total_gst))}</span>,
		footer: (rs) => formatInr(rs.reduce((s, r) => s + Number(r.total_gst), 0)),
		header: "Total GST",
		id: "tot",
		value: (r) => Number(r.total_gst),
	},
];
