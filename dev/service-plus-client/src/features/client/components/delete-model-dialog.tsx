...
export const DeleteModelDialog = ({
    ...
    const deleteEnabled = inUse === false && !checkingInUse && confirmValue.toLowerCase() === model.model_name.toLowerCase();
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Permanently delete{" "}
                        <span className="font-semibold text-slate-800">{model.model_name}</span>?
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    {inUse === true && (
                        <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
                            ...
                            <p className="text-sm text-amber-800">
                                This model cannot be deleted as it is assigned to existing jobs.
                            </p>
                        </div>
                    )}
                    ...
                    {inUse !== true && (
                        <div className="flex flex-col gap-1.5">
                            <Label htmlFor="confirm_model_name">
                                Type{" "}
                                <span className="font-semibold text-slate-800">{model.model_name}</span>{" "}
                                to confirm
                            </Label>
