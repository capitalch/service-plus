...
export const HELP_ARTICLES: HelpArticle[] = [
...

    {
        ...
        summary: "Two WhatsApp messages today — a Job Intake Notice on drop-off and a completion notice from Customer Connect — both tracked to real delivery by Meta, not just accepted by the server.",
        tags: ["whatsapp", "whatsapp integration", "customer connect", "job intake", "job creation", "job completion", "mobile number", "delivery status", "status link", "job slip", "whatsapp_notifications", "switch off", "disable"],
        content: [
            { type: "para", text: "Service+ sends two kinds of WhatsApp message: a Job Intake Notice the moment a job (or batch) is created — 'we've received your device' — and a completion notice from Jobs → Customer Connect once it's finalized — 'your device is ready for collection.' An earlier version of this product also sent an invoice on delivery and a receipt on payment; those two PDF-carrying sends are still removed and are not part of either message below. There is no SMS or email option — WhatsApp is the only channel." },
            ...
            { type: "note", text: "Status only ever moves forward along this list — a later, lower-ranked update (e.g. a delayed 'Sent' arriving after 'Delivered' already showed) is ignored, never shown as a regression. Both message types share this same tracking, shown as their own status pill wherever each appears." },
            { type: "heading", text: "Job Intake Notice — sent on creation, not on completion" },
            { type: "para", text: "See 'Job Intake Notice' in this Help Center for the full walkthrough. In short: it fires right after a single job or a batch is created, with no status filter — unlike the completion notice, a job doesn't need to be finished for this one. The message carries two buttons, 'Check Repair Status' and 'Download Job Slip,' that open a page/PDF with no login required — a customer can bookmark that link and it keeps working even after the job is delivered." },
            { type: "heading", text: "Completion notice — Customer Connect" },
            { type: "para", text: "Only jobs at status COMPLETED_OK that have been finalized are eligible for the completion notice. A RETURN job (device could not be fixed) never appears here — there is nothing to tell that customer is 'ready for pickup.'" },
            { type: "heading", text: "Grouping" },
            { type: "para", text: "For either message, every eligible job you select is grouped by customer and sent as one message per customer, covering all of that customer's selected jobs — never one message per job. A batch is just several jobs for one customer, so it already gets exactly one intake message." },
            { type: "heading", text: "Access" },
            { type: "para", text: "The Customer Connect menu item has its own access right, separate from the rest of Jobs — see 'Roles' in Access Management for exactly who has it. Sending a Job Intake Notice carries no separate access right of its own: if you can create the job, you can send (or resend) its intake message." },
            { type: "heading", text: "Turning an event off entirely" },
            { type: "para", text: "Configurations → App Settings → whatsapp_notifications has one on/off switch per event — Job Intake Message and Job Completed today (a third, Job Delivery, is reserved for a future release and does nothing yet). Only Job Completed is on by default. When an event is off, clicking Send doesn't fail or error — it simply doesn't go out, and you'll see a message saying that event is currently switched off." },
        ],
        faqs: [
            { q: "Can I send an SMS or email instead of WhatsApp?", a: "No — WhatsApp is the only messaging channel." },
            { q: "What happened to the WhatsApp buttons that used to send a job sheet, invoice, or receipt?", a: "The invoice-on-delivery and receipt-on-payment sends are still removed. The job-sheet-on-creation one is back, redesigned as the Job Intake Notice — a status link and a downloadable job slip instead of the PDF attached directly to the message. See 'Job Intake Notice' for details." },
...

    {
        ...
        summary: "Send the 'job ready for pickup' WhatsApp message to many finalized customers at once, grouped one message per customer, with delivery tracked by Meta.",
        tags: ["customer connect", "whatsapp", "bulk", "completion message", "delivery status", "select all"],
        content: [
            { type: "para", text: "Jobs → Customer Connect lists every job at status COMPLETED_OK that has been finalized — in other words, every job you can tell a customer 'your device is ready for pickup.' Select the ones you want to notify and click Send Messages." },
            ...
            { type: "table", headers: ["Column", "Meaning"], rows: [
                ...
                ["Whatsapp",      "One column, stacked: how many sends have succeeded/failed for this job, when the last attempt went out, and its current delivery status (Accepted, Sent, Delivered, Read, or Failed) — hover a Failed status for the error"],
            ]},
        ...
        faqs: [
            ...
            { q: "A job shows Failed in the Whatsapp column — will it retry automatically?", a: "No. Failed is terminal for that attempt — select the row again and click Send Messages to retry." },
            { q: "Does this screen cover the job-creation or delivery WhatsApp messages too?", a: "No. Job creation has its own message — the Job Intake Notice, sent from New Job/Batch Job or the job detail view, not from here. Delivery and receipt WhatsApp sends remain removed. See 'Job Intake Notice' and 'WhatsApp Integration' for the full picture." },
        ],
    },
...

    {
        ...
        content: [
            ...
            { type: "table", headers: ["Setting", "What It Controls"], rows: [
                ...
                ["whatsapp_notifications",                 "Turns outbound WhatsApp messages on or off, one switch per event (Job Intake Message, Job Completed, Job Delivery) — see 'Turning WhatsApp messages on or off' below"],
            ]},
            { type: "heading", text: "Turning WhatsApp messages on or off" },
            { type: "para", text: "The whatsapp_notifications setting opens its own dialog instead of the usual text/JSON editor — a toggle for each event: Job Intake Message, Job Completed, and Job Delivery. By default only Job Completed is switched on. Job Delivery's toggle has no effect yet — there is no WhatsApp message sent on delivery in this version of the app, so it's shown for when that feature arrives." },
