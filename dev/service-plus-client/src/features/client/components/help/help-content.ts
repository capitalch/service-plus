...
export const HELP_ARTICLES: HelpArticle[] = [
...

    {
        ...
        summary: "One send point — Customer Connect — with delivery confirmed by Meta, not just accepted by the server.",
        tags: ["whatsapp", "whatsapp integration", "customer connect", "job completion", "mobile number", "delivery status"],
        content: [
            { type: "para", text: "Service+ sends exactly one kind of WhatsApp message: the completion notice ('your device is ready for collection'), from Jobs → Customer Connect. Earlier versions also sent a job sheet on creation, an invoice on delivery, and a receipt on payment — those three PDF-carrying sends have been removed; Customer Connect is now the only WhatsApp send point in the product. There is no SMS or email option." },
            { type: "heading", text: "What counts as \"sent\"" },
            { type: "para", text: "A click on Send Messages only means the message was handed to Meta — it does not yet mean the customer received it. Every send is tracked through Meta's own delivery webhook to a real status, and only Delivered (or Read) means it actually reached the customer's phone." },
            ...
            { type: "note", text: "Status only ever moves forward along this list — a later, lower-ranked update (e.g. a delayed 'Sent' arriving after 'Delivered' already showed) is ignored, never shown as a regression." },
            { type: "heading", text: "Eligibility" },
            { type: "para", text: "Only jobs at status COMPLETED_OK that have been finalized are eligible. A RETURN job (device could not be fixed) never appears here — there is nothing to tell that customer is 'ready for pickup.'" },
            { type: "heading", text: "Grouping" },
            { type: "para", text: "Every eligible job you select is grouped by customer and sent as one message per customer, covering all of that customer's selected jobs — never one message per job." },
            { type: "heading", text: "Access" },
            { type: "para", text: "The Customer Connect menu item has its own access right, separate from the rest of Jobs. See 'Roles' in Access Management for exactly who has it." },
        ],
        faqs: [
            { q: "Can I send an SMS or email instead of WhatsApp?", a: "No — WhatsApp is the only messaging channel, and Customer Connect is the only screen that sends it." },
            { q: "Where did the Whatsapp buttons on New Job, Finalize, Deliver Job, and Receipts go?", a: "Removed. Those sent a PDF (job sheet, invoice, or receipt) with a short message; only the completion notice from Customer Connect remains." },
...

    {
        ...
        content: [
            ...
            { type: "table", headers: ["Column", "Meaning"], rows: [
                ...
                ["Delivery",      "Meta's own delivery status for the last send attempt on this job: Accepted, Sent, Delivered, Read, or Failed — hover a Failed badge for the error"],
            ]},
            ...
            { type: "bullets", items: [
                ...
                "A row with any prior send attempt — successful or failed — starts unchecked instead; resending is always a deliberate click.",
            ...
            { type: "para", text: "A banner above the grid stays up until you dismiss it — it doesn't auto-disappear like a toast. It starts by reporting what was dispatched: '12 dispatched to 9 customers. Awaiting delivery confirmation…' As Meta's delivery webhooks arrive, the banner and the grid's Delivery column update live — no need to refresh. This live tracking runs for a couple of minutes after a send (or until every message has settled, whichever comes first); after that, use the Refresh button to check again." },
        ...
        faqs: [
            ...
            { q: "A job shows Failed in the Delivery column — will it retry automatically?", a: "No. Failed is terminal for that attempt — select the row again and click Send Messages to retry." },
            { q: "Does this screen cover the job-creation or delivery WhatsApp messages too?", a: "There's nothing left to cover — job creation, delivery, and receipt WhatsApp sends have been removed entirely. Customer Connect's completion message is the only WhatsApp send point in the product." },
        ],
    },
...

    {
        ...
        summary: "Customer-facing parts catalog per branch — optionally linked to a Parts Master entry, with its own photos.",
        tags: ["spare parts web", "web catalogue", "web parts", "customer catalog", "brand", "part code", "photos", "price"],
        content: [
            ...
            { type: "para", text: "Delete asks you to type a confirmation value before proceeding, prioritized in this order so it's never blank: Part Code (if linked), otherwise Model, otherwise Part Name. The dialog states exactly which field to type." },
        ],
        faqs: [
