...
import {
	...
	getPurchaseInvoiceDefaultValues,
	...
	return (
		...
		>
			...
			<div className="flex flex-wrap items-center gap-x-4 gap-y-3 border-b border-(--cl-border) bg-(--cl-surface) px-4 py-1">
					...
					<div className="flex items-baseline gap-2 overflow-hidden">
						...
						{mode === "new" && (
							...
							>
								...
								<span
									className={`text-[10.5px] font-bold uppercase tracking-tighter ${
				...
				<label
					className={`flex items-center gap-1.5 select-none px-3 py-1.5 rounded-lg border-2 font-black text-[12px] uppercase tracking-[0.1em] transition-all shadow-sm ${
						mode !== "new"
			...
			{mode === "new" ? (
					...
					<NewPurchaseInvoice
						...
						onSaveSuccess={() => {
							if (editInvoice) {
								...
								setMode("view");
								if (branchId)
									void loadData(Number(branchId), fromDate, toDate, searchQ, 1, viewDivisionId);
							}
						}}
					/>
				</FormProvider>
				...
				<>
								...
								<table className="min-w-full border-collapse">
									<thead className="sticky top-0 z-30">
										<tr className="bg-(--cl-surface-2)">
											...
											<th className={thClass} style={{ width: "25%" }}>
												Supplier
											</th>
								...
								<table className="min-w-full border-collapse">
									<thead>
										<tr>
											...
											<th className={`${thClass} sticky top-0 right-0 z-30 !bg-(--cl-surface-2)`}>
												Actions
											</th>
										</tr>
									</thead>
									<tbody className="divide-y divide-(--cl-border) bg-(--cl-surface)">
										{invoices.map((inv, idx) => (
											<tr
												key={inv.id}
												className="group transition-colors hover:bg-(--cl-accent)/5 dark:hover:bg-white/[0.03]"
											>
												<td
					...
					<PurchaseInvoicePdfPreviewDialog
						branch={
							pdfPreviewInvoice
								? (branches.find((b) => b.id === pdfPreviewInvoice.branch_id) ?? null)
								: null
						}
						division={
							pdfPreviewInvoice
								? (availableDivisions.find((d) => d.id === pdfPreviewInvoice.division_id) ?? null)
								: null
						}
						...
						vendor={
							pdfPreviewInvoice
								? (vendors.find((v) => v.id === pdfPreviewInvoice.supplier_id) ?? null)
								: null
						}
					/>
