...
export const MasterDataDiffModal = ({ isOpen, onClose, onConfirm, submitting, masterDiffLines }: Props) => {
	return (
		<Dialog
			open={isOpen}
			onOpenChange={(open) => {
		...
		>
			<DialogContent aria-describedby={undefined} className="sm:max-w-2xl">
				...
				<p className="text-sm text-(--cl-text-muted)">
					The following values differ from what is currently stored in the part master. Proceeding will
					overwrite the master with the entered values.
				</p>
					...
					<table className="w-full text-sm">
						...
						<tbody className="divide-y divide-(--cl-border)">
							{masterDiffLines.flatMap((line) => {
									...
									rows.push(
										<tr key={`${line._key}-price`} className="bg-(--cl-surface)">
											...
											<td className="px-3 py-1.5 text-right font-mono">
												{formatNumber(line._orig_cost_price)}
											</td>
											<td className="px-3 py-1.5 text-right font-mono text-amber-600">
								...
								if (
									line._orig_gst_rate != null &&
									line.gst_rate > 0 &&
									line.gst_rate !== line._orig_gst_rate
								) {
									rows.push(
