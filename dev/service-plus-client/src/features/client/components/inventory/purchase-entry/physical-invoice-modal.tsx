...
export function PhysicalInvoiceModal({
	...
	const validateAndSubmit = handleSubmit((vals) => {
		...
		if (Math.abs(vals.qty - totals.qty) >= 0.001) {
			setError("qty", { type: "manual" });
			ok = false;
		}
		if (!isIgst && !check(vals.cgst, totals.cgst, taxPct, taxMin)) {
			setError("cgst", { type: "manual" });
			ok = false;
		}
		if (!isIgst && !check(vals.sgst, totals.sgst, taxPct, taxMin)) {
			setError("sgst", { type: "manual" });
			ok = false;
		}
		if (isIgst && !check(vals.igst, totals.igst, taxPct, taxMin)) {
			setError("igst", { type: "manual" });
			ok = false;
		}
		if (!check(vals.total, totals.total, totalPct)) {
			setError("total", { type: "manual" });
			ok = false;
		}
	...
	return (
		...
		>
			<DialogContent
				...
				className="sm:max-w-lg !bg-white !text-zinc-900 border-zinc-200"
			>
				<DialogHeader>
				...
				<div className="flex items-start gap-2 rounded-md border border-amber-400/30 bg-amber-400/10 px-3 py-2.5 text-xs text-amber-700">
					...
					<span>
						<strong>Read values directly from your physical invoice.</strong> Do not copy the figures shown
						on screen — values must be entered independently to ensure accuracy.
					</span>
				</div>
					...
					<table className="w-full text-sm">
						<thead>
							<tr className="bg-(--cl-surface-2)/60 border-b border-(--cl-border)">
								<th className="py-2 px-3 text-left text-[11px] font-bold uppercase tracking-wider text-(--cl-text-muted)">
									Field
								</th>
								<th className="py-2 px-3 text-right text-[11px] font-bold uppercase tracking-wider text-(--cl-text-muted)">
									Your Entry
								</th>
								<th className="py-2 px-2 text-center text-[11px] font-bold uppercase tracking-wider text-(--cl-text-muted)">
									Status
								</th>
							</tr>
						</thead>
							...
							<tr className={rowCls("qty")}>
								...
								<td className="py-1 px-2">
									<Input
										...
										onChange={(e) => {
											clearErrors("qty");
											setValue("qty", e.target.value === "" ? 0 : Number(e.target.value));
										}}
									/>
								</td>
								...
								<tr className={rowCls("igst")}>
									...
									<td className="py-1 px-2">
										<Input
											...
											onChange={(e) => {
												clearErrors("igst");
												setValue("igst", e.target.value === "" ? 0 : Number(e.target.value));
											}}
										/>
									</td>
							...
							<tr className={`font-semibold ${rowCls("total")}`}>
								...
								<td className="py-1 px-2">
									<Input
										...
										onChange={(e) => {
											clearErrors("total");
											setValue("total", e.target.value === "" ? 0 : Number(e.target.value));
										}}
									/>
								</td>
				...
				{isSubmitted && (
					...
					>
						{allValid ? (
							...
							<>
								<AlertTriangle className="h-4 w-4 text-amber-600" /> Validation failed. Please correct
