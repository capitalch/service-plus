...
export const generatePurchaseInvoicePdf = (
	...
	const drawSummaryRow = (
		label: string,
		value: number | string,
		opts?: { bold?: boolean; isGrand?: boolean; color?: [number, number, number] },
	) => {
		...
		const valStr = typeof value === "number" ? `Rs. ${formatCurrency(value).replace("₹", "").trim()}` : value;
	...
	if (Math.abs(diffAmount) > 0.01) {
		...
		drawSummaryRow(
			"Adjustment/Diff",
			`${sign} Rs. ${formatCurrency(Math.abs(diffAmount)).replace("₹", "").trim()}`,
			{ color: [150, 80, 0] },
		);
	}
