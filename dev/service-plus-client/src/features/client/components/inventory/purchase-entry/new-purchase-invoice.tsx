...
import {
	...
	selectEffectiveGstStateCode,
	selectDefaultGstRate,
	selectSchema,
} from "@/store/context-slice";
import type { VendorType } from "@/features/client/types/vendor";
import type {
	PurchaseInvoiceType,
	PurchaseLineFormItem,
	StockTransactionTypeRow,
} from "@/features/client/types/purchase";
...
const thClass =
	"sticky top-0 z-20 text-xs font-extrabold uppercase tracking-widest text-(--cl-text) py-2 px-2 text-left border-b border-(--cl-border) bg-zinc-200/60 dark:bg-zinc-800/60 backdrop-blur-sm shadow-[0_1px_0_var(--cl-border)]";
const tdClass = "p-0.5 border-b border-(--cl-border)";
const inputCls = "h-7 border-(--cl-border) bg-white text-sm px-2";

// ─── Component ────────────────────────────────────────────────────────────────

export const NewPurchaseInvoice = forwardRef<PurchaseInvoiceHandle, Props>(function NewPurchaseInvoice(
	{
		branchId,
		brandName,
		editInvoice,
	...
	useEffect(() => {
		...
		const vendorStateCode =
			vendor?.gst_state_code ?? (vendor?.gstin && vendor.gstin.length >= 2 ? vendor.gstin.substring(0, 2) : null);
	...
	useEffect(() => {
		...
		apolloClient
			.query<
				GenericQueryData<
					PurchaseInvoiceType & { lines: import("@/features/client/types/purchase").PurchaseLineType[] }
				>
			>({
	...
	const totals = useMemo(() => {
		...
		return {
			...
			total: aggregate + cgst + sgst + igst,
		};
	}, [fields]);
	...
	const executeSave = async (physical: PhysicalValues) => {
		...
		if (
			!purchaseTypeId ||
			(isReturn && !returnTypeId) ||
			(hasWarrantyLine && !isReturn && !warrantyTypeId) ||
			!branchId ||
			!dbName ||
			!schema ||
			!vendor_id ||
	...
	const handleConfirmedSubmit = async (physical: PhysicalValues) => {
		...
		const diffLines = isGstRegistered
			? lines.filter((line) => {
					if (!line.part_id) return false;
					const hsnConflict =
						line._orig_hsn_code != null &&
						line.hsn_code.trim() !== "" &&
						line.hsn_code.trim() !== line._orig_hsn_code;
					const priceConflict =
						line._orig_cost_price != null &&
						line.unit_price > 0 &&
						line.unit_price !== line._orig_cost_price;
					const gstConflict =
						line._orig_gst_rate != null && line.gst_rate > 0 && line.gst_rate !== line._orig_gst_rate;
		...
		>
			{!branchId ? (
				...
				<>
					...
					<Card
						className={`border-(--cl-border) shadow-md !overflow-visible bg-(--cl-surface) ${isReturn ? "border-l-4 border-l-orange-500" : ""}`}
					>
								...
								{availableDivisions.length > 0 && (
									<div className="space-y-2 md:col-span-3 lg:col-span-2">
										...
										<select
											...
											value={divisionId || ""}
											onChange={(e) =>
												handleDivisionChange(e.target.value ? Number(e.target.value) : 0)
											}
										>
					...
					{/* Line Items Table */}
					<p className="text-[10px] font-black uppercase tracking-[0.15em] text-(--cl-text-muted) my-2">
						Line Items
					</p>
					<Card
						className={`border-(--cl-border) shadow-sm flex min-h-[45vh] md:min-h-0 md:flex-1 flex-col relative bg-(--cl-surface) ${isReturn ? "border-l-4 border-l-orange-500" : ""}`}
					>
						<div
								...
								<thead>
									<tr className="bg-(--cl-surface-2)/50">
										...
										<th className={thClass} style={{ width: "20%" }}>
											Part <span className="text-red-500 ml-0.5">*</span>
										</th>
										<th
											...
											title="Warranty"
										>
											<ShieldOff className="h-3.5 w-3.5 mx-auto text-red-600" />
										</th>
										<th className={thClass} style={{ width: "8%" }}>
											HSN
										</th>
										<th className={`${thClass} text-right`} style={{ width: "6%" }}>
										...
										<th
											className={`${thClass} text-right border-l border-(--cl-border)`}
											style={{ width: "8%" }}
										>
											Subtotal
										</th>
										...
										{!isIgst ? (
											<>
												<th className={`${thClass} text-right`} style={{ width: "7%" }}>
													CGST
												</th>
												<th className={`${thClass} text-right`} style={{ width: "7%" }}>
													SGST
											...
											<th className={`${thClass} text-right`} style={{ width: "14%" }}>
												IGST
											</th>
										)}
										<th className={`${thClass} text-right`} style={{ width: "7%" }}>
								...
								<tbody className="bg-(--cl-surface)">
									{fields.map((line, idx) => {
										...
										return (
											<tr
												key={line._key}
												className="hover:bg-(--cl-surface-2)/30 group transition-colors align-middle"
											>
												<td
													className={`${tdClass} pl-4 text-xs font-medium text-(--cl-text-muted)`}
												>
													...
													<PartCodeInput
														...
														onSelect={(part) => {
															...
															if (isGstRegistered) {
																...
																const gstRateForCalc =
																	masterGstRate > 0
																		? masterGstRate
																		: Number(defaultGstRate) || 0;
																const grossedPrice =
																	Math.round(
																		baseCost * (1 + gstRateForCalc / 100) * 100,
																	) / 100;
																updateLine(idx, {
																	...common,
												...
												<td className={`${tdClass} text-center`}>
													<button
														...
														title={
															line.under_warranty
																? "Under Warranty — click to remove"
																: "Not under warranty — click to mark"
														}
														onClick={() => {
															...
															updateLine(
																...
																checked
																	? {
																			under_warranty: true,
																			unit_price: 0,
																			_pre_warranty_price: line.unit_price,
																		}
																	: {
																			under_warranty: false,
																			unit_price: line._pre_warranty_price ?? 0,
																			_pre_warranty_price: null,
																		},
															);
														}}
												...
												<td className={tdClass}>
													{(() => {
														...
														const priceChanged =
															...
															origCost != null &&
															line.unit_price > 0 &&
															Math.abs(line.unit_price - origCost) > 0.001;
														return (
												...
												{/* Subtotal (read-only) */}
												<td
													className={`${tdClass} px-2 text-right font-mono tabular-nums text-sm font-medium text-(--cl-text) border-l border-(--cl-border) bg-(--cl-surface-2)/40`}
												>
												...
												<td className={tdClass}>
													<Input
														...
														onChange={(e) =>
															updateLine(idx, { gst_rate: Number(e.target.value) })
														}
														onFocus={(e) => e.target.select()}
													/>
												</td>
												...
												{!isIgst ? (
													<>
														<td
															className={`${tdClass} px-2 text-right font-mono tabular-nums text-sm font-medium text-(--cl-text-muted) bg-(--cl-surface-2)/40`}
														>
															<div className="flex h-7 items-center justify-end">
												...
												{/* Total */}
												<td
													className={`${tdClass} px-2 text-right font-mono tabular-nums text-sm font-semibold text-(--cl-text) bg-(--cl-surface-2)/40`}
												>
													<div className="flex h-7 items-center justify-end">
					...
					<div
						ref={summaryRef}
						className={`rounded-lg border px-4 py-2.5 flex flex-wrap items-center gap-x-6 gap-y-1 justify-end ${isReturn ? "border-orange-500/30 bg-orange-500/5" : "border-(--cl-border) bg-(--cl-surface-2)/40"}`}
					>
						<div className="flex items-center gap-1.5">
							<span className="text-[10px] font-black uppercase tracking-widest text-(--cl-text-muted)">
								Lines
							</span>
						...
						{totals.cgst > 0 && (
							<div className="flex items-center gap-1.5">
								...
								<span className="font-mono font-semibold text-sm text-(--cl-text-muted)">
