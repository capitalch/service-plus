...
export const PurchaseInvoicePdfPreviewDialog = ({
	branch,
	division,
	invoice: propInvoice,
	onOpenChange,
	...
	useEffect(() => {
		...
		async function process() {
			...
			try {
				...
				const doc = generatePurchaseInvoicePdf(
					currentDetail,
					currentDetail.lines || [],
					companyName,
					branchName,
			...
			>
				...
				<div className="flex-1 w-full bg-zinc-100 flex items-center justify-center relative">
					{loading ? (
						...
						<iframe src={pdfUrl} className="w-full h-full border-none" title="Invoice PDF Preview" />
					) : (
