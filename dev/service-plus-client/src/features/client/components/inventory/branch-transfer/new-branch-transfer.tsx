...
const hdrCellCls =
	"text-[11px] font-extrabold uppercase tracking-widest text-(--cl-text) py-3 px-2 flex  border-b border-r border-(--cl-border) last:border-r-0 bg-zinc-200/50 dark:bg-zinc-800/50";
...
export function NewBranchTransfer({
	...
	const {
		...
		trigger,
		formState: { errors },
	} = useFormContext<BranchTransferFormValues>();
	...
	useEffect(() => {
		...
		apolloClient
			.query<
				GenericQueryData<
					StockBranchTransferType & {
						lines: import("@/features/client/types/branch-transfer").StockBranchTransferLineType[];
					}
				>
			>({
			...
			{!branchId ? (
				<div className="flex flex-col items-center justify-center py-20 bg-(--cl-surface-2)/30 rounded-xl border-2 border-dashed border-(--cl-border) text-center">
					<div className="bg-(--cl-accent)/5 p-5 rounded-full mb-4">
						<ArrowRightLeft className="h-12 w-12 opacity-40 text-muted-foreground" />
					</div>
				...
				<>
					...
					<p className="text-[10px] font-black uppercase tracking-[0.15em] text-(--cl-text-muted) px-1 mb-1">
						Transfer Details
						{editTransfer && (
							<span className="ml-2 px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 border border-amber-500/20">
								Edit
							</span>
						)}
					</p>
							...
							<div className="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-x-2 gap-y-2">
								...
								<div className="space-y-2 md:col-span-3 lg:col-span-3">
									...
									<Select
										value={toBranchId}
										onValueChange={(v) => {
											setValue("to_branch_id", v, { shouldValidate: true });
											void trigger();
										}}
									>
										<SelectTrigger
											className={`bg-(--cl-surface-2) ${!toBranchId ? "border-red-500" : ""}`}
										>
						...
						>
							<div className="min-w-[800px]">
								...
								<div className={`grid ${COLS} sticky top-0 z-20 backdrop-blur-md`}>
									...
									<div className={`${hdrCellCls} justify-end px-3`}>
										...
										>
											...
											<div className="p-1 border-r border-(--cl-border)/30">
												<PartCodeInput
													...
													onChange={(code) => {
														const patch: Partial<TransferLineFormValues> = {
															part_code: code,
														};
														if (!code.trim()) {
															patch.part_id = null;
					...
					<div
						ref={summaryRef}
						className="rounded-lg border border-(--cl-border) bg-(--cl-surface-2)/40 px-4 py-2.5 flex flex-wrap items-center gap-x-6 gap-y-1 justify-end"
					>
						...
						<div className="flex items-center gap-1.5 border-l border-(--cl-border) pl-4">
							...
							<span className="font-mono font-black text-base text-(--cl-accent)">{totalQty}</span>
						</div>
					</div>
				</>
			)}
