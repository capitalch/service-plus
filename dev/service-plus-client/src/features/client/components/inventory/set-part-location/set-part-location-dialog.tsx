...
const thCls =
	"text-xs font-semibold uppercase tracking-wide text-(--cl-text-muted) px-2 py-1.5 text-left border-b border-(--cl-border) bg-(--cl-surface-3)";
			...
			<td className={tdCls}>
				<div className="flex flex-col gap-0.5">
					<PartCodeInput
						...
						onChange={(code) =>
							onUpdate(line._key, { error: null, part_code: code, part_id: null, part_name: "" })
						}
						onClear={() =>
							onUpdate(line._key, { error: null, part_code: "", part_id: null, part_name: "" })
						}
						onSelect={(part) =>
							onUpdate(line._key, {
								error: null,
...
export const SetPartLocationDialog = ({ locations, onOpenChange, onSuccess, open }: Props) => {
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				<DialogHeader>
					<DialogTitle className="text-base font-semibold text-foreground">Set Part Location</DialogTitle>
				</DialogHeader>
				...
				<div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto py-1 pr-1">
					...
					<div className="flex items-center gap-3 rounded-lg border border-(--cl-border) bg-(--cl-surface-2) px-3 py-2">
						<span className="shrink-0 text-sm font-medium text-(--cl-text)">
							Apply location to all rows:
						</span>
						<Select
					...
					<Button className="self-start" size="sm" type="button" variant="outline" onClick={handleAddRow}>
