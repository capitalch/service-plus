...
export const SetLocationForSelectedDialog = ({ locations, onOpenChange, onSuccess, open, parts }: Props) => {
	...
	useEffect(() => {
		if (locationId > 0) {
			setRowLocationIds(Object.fromEntries(parts.map((p) => [p.part_id, locationId])));
		}
	}, [locationId, parts]);
							...
							<table className="w-full">
								...
								<tbody>
									{parts.map((p) => (
										<tr
											key={p.part_id}
											className="hover:bg-(--cl-surface-3) last:[&>td]:border-b-0"
										>
											<td className={`${tdCls} font-mono font-medium text-(--cl-text)`}>
												{p.part_code}
											</td>
											...
											<td className={tdCls}>
												<Select
													value={
														rowLocationIds[p.part_id]
															? String(rowLocationIds[p.part_id])
															: ""
													}
