...
export const StockOverviewSection = () => {
	...
	const loadStock = useCallback(
		async (branchId: number, brandId: number, q: string, pg: number) => {
	...
	const SortIcon = ({ col }: { col: string }) => {
		if (sortCol !== col)
			return <ArrowUpDownIcon className="ml-1 inline h-3 w-3 opacity-40 text-muted-foreground" />;
		return sortDir === "asc" ? (
		...
		>
			...
			<div className="flex flex-wrap items-center gap-3">
				...
				<RefreshButton
					...
					onClick={() => {
						if (selectedBranch)
							void loadStock(Number(selectedBranch), Number(selectedBrand), searchQ, page);
					}}
				/>
			</div>
							...
							<tbody>
								{Array.from({ length: 10 }).map((_, i) => (
									<tr key={i} className="animate-pulse">
										...
										<td className={tdClass} style={{ width: "20%" }}>
										...
										<td className={tdClass} style={{ width: "8%" }}>
											<div className="h-4 w-10 rounded bg-(--cl-border)" />
										</td>
									...
									<tr key={row.part_id} className="transition-colors hover:bg-(--cl-surface-2)/50">
										...
										<td className={`${tdClass} font-medium`} style={{ width: "20%" }}>
											{row.part_description
												? `${row.part_name} - ${row.part_description}`
												: row.part_name}
										</td>
										<td className={tdClass} style={{ width: "12%" }}>
				...
				<div className="sticky bottom-0 z-20 flex flex-wrap items-center justify-between gap-4 border-t border-(--cl-border) bg-(--cl-surface-2) px-4 py-2 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
					<div className="flex items-center gap-4 text-sm text-(--cl-text-muted)">
						...
						<span className="font-semibold text-(--cl-text)">
							...
							<span className="font-normal text-(--cl-text-muted) ml-0.5">Total Value</span>
						</span>
					</div>
