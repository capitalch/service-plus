...
export function NewStockAdjustment({
	...
	useEffect(() => {
		...
		apolloClient
			.query<
				GenericQueryData<
					StockAdjustmentType & {
						lines: import("@/features/client/types/stock-adjustment").StockAdjustmentLineType[];
					}
				>
			>({
	...
	const linesValid =
		lines.length > 0 && lines.every((l) => !!l.part_id && l.qty > 0 && (l.dr_cr === "D" || l.dr_cr === "C"));
								...
								<thead>
									<tr className="bg-(--cl-surface-2)/50">
										...
										<th className={thClass} style={{ width: "12%" }}>
											IN / OUT <span className="text-red-500 ml-0.5">*</span>
										</th>
											...
											>
												...
												<td className={tdClass}>
													<PartCodeInput
														...
														brandName={brandName}
														onChange={(code) => {
															const patch: Partial<StockAdjFormValues["lines"][number]> =
																{ part_code: code };
															if (!code.trim()) {
																patch.part_id = null;
																patch.part_name = "";
