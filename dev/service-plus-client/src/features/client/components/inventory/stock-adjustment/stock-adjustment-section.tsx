...
export const StockAdjustmentSection = () => {
	...
	useEffect(() => {
		...
		const fetchMeta = async () => {
			try {
				...
				toast.error(MESSAGES.ERROR_ADJUSTMENT_LOAD_FAILED);
			}
		};
		void fetchMeta();
	}, [dbName, schema]);

	// Load adjustments (paged)
		...
		async (bId: number, from: string, to: string, q: string, pg: number) => {
			...
			try {
				...
				const [dataRes, countRes] = await Promise.all([
					apolloClient.query<GenericQueryData<StockAdjustmentType>>({
						fetchPolicy: "network-only",
						query: GRAPHQL_MAP.genericQuery,
						variables: {
							db_name: dbName,
							...
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_STOCK_ADJUSTMENTS_PAGED,
					...
					apolloClient.query<GenericQueryData<{ total: number }>>({
						...
						variables: {
							...
							schema,
							value: graphQlUtils.buildGenericQueryValue({
								sqlId: SQL_MAP.GET_STOCK_ADJUSTMENTS_COUNT,
								sqlArgs: commonArgs,
							}),
						},
					}),
				]);
				setAdjustments(dataRes.data?.genericQuery ?? []);
	...
	const executeSave = async (values: StockAdjFormValues) => {
		...
		try {
			...
			toast.error(
				editAdjustment ? MESSAGES.ERROR_ADJUSTMENT_UPDATE_FAILED : MESSAGES.ERROR_ADJUSTMENT_CREATE_FAILED,
			);
		}
	};
	...
	const handleDelete = async () => {
		...
		try {
			...
			toast.success(MESSAGES.SUCCESS_ADJUSTMENT_DELETED);
			setDeleteId(null);
			void loadData(Number(branchId), fromDate, toDate, searchQ, page);
		} catch {
			toast.error(MESSAGES.ERROR_ADJUSTMENT_DELETE_FAILED);
		} finally {
			setDeleting(false);
		}
	};
					...
					<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
									...
									<tbody>
										{Array.from({ length: 15 }).map((_, i) => (
											<tr key={i} className="animate-pulse">
												...
												<td className={tdClass}>
													<div className="h-4 w-20 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="h-4 w-20 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="h-4 w-64 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="h-4 w-24 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="h-4 w-56 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
															...
															>
																<DropdownMenuItem
																	...
																	onClick={() => {
																		...
																		setSelectedBrand(String(adj.brand_id));
																		setMode("new");
																	}}
																>
						...
						<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
							<span className="text-xs text-(--cl-text-muted)">
								{total === 0
									? "No adjustments"
									: `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} adjustments (Page ${page} of ${totalPages})`}
							</span>
							<div className="flex items-center gap-1">
								<Button
									className="h-7 w-7"
					...
					>
						<DialogContent aria-describedby={undefined} className="sm:max-w-sm">
							...
							<p className="text-sm text-muted-foreground">
								This will permanently delete the adjustment and all associated stock transactions. This
