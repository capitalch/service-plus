	...
	return (
		<motion.div variants={cardVariants}>
			...
			>
				...
				{/* Part name */}
				<p className="line-clamp-2 text-sm font-semibold leading-snug text-(--cl-text)">{part.part_name}</p>

				{/* Brand + Category */}
				<div className="flex flex-wrap items-center gap-1.5">
					{part.brand_name && <span className="text-xs text-(--cl-text-muted)">{part.brand_name}</span>}
					{part.brand_name && part.category && <span className="text-xs text-(--cl-text-muted)">·</span>}
					{part.category && (
						<Badge
							className="border-(--cl-border) bg-(--cl-surface-3) text-(--cl-text-muted)"
							variant="outline"
						>
							{part.category}
						</Badge>
				...
				{part.primary_location && (
					<div className="flex items-center gap-1.5">
						...
						{part.location_count > 1 && (
							<Badge
								className="border-sky-200 bg-sky-50 text-sky-600 dark:border-sky-800 dark:bg-sky-950/30 dark:text-sky-400"
								variant="outline"
							>
...
export const PartFinderCard = ({ loading, onSelectPart, parts, selectedId }: Props) => {
		...
		return (
			<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{Array.from({ length: 6 }).map((_, i) => (
					<div
						key={i}
						className="h-52 animate-pulse rounded-xl border border-(--cl-border) bg-(--cl-surface-2)"
					/>
				))}
			</div>
		);
	...
	if (parts.length === 0) {
		return (
			<div className="flex flex-col items-center justify-center rounded-xl border border-(--cl-border) bg-(--cl-surface-2) py-16">
				...
				<p className="mt-1 text-xs text-(--cl-text-muted) opacity-70">
	...
	return (
		...
		>
			{parts.map((part) => (
				<PartCard isSelected={selectedId === part.id} key={part.id} onSelect={onSelectPart} part={part} />
			))}
		</motion.div>
	);
};
