...
export const PartFinderPage = () => {
	...
	const loadData = useCallback(
		async (q: string, pg: number, brandName: string, status: string) => {
			if (!dbName || !schema || !currentBranch) return;
			setLoading(true);
			try {
				const sqlArgs = {
	...
	return (
		...
		>
			...
			<div>
				...
				<p className="mt-1 text-sm text-(--cl-text-muted)">Search and locate parts across your inventory</p>
			</div>
			...
			<div className="flex flex-wrap items-center gap-3">
				...
				{searchQ && <RefreshButton loading={loading} onClick={handleRefresh} />}
			</div>
