...
import type {
	PartFinderResultType,
	PartFinderStockByLocationType,
	PartFinderStockSummaryType,
} from "@/features/client/types/part-finder";
...
export const PartFinderDetailPanel = ({ locations, onClose, onRefresh, part }: Props) => {
	...
	// Reset tab and clear cached data when the selected part changes
	useEffect(() => {
		if (part) {
			setTab("overview");
			setStockByLoc([]);
	...
	useEffect(() => {
		if (!part) return;
		if (tab === "stock" && stockByLoc.length === 0) void loadStockByLoc();
		if (tab === "history" && history.length === 0) void loadHistory();
	}, [tab, part]); // eslint-disable-line react-hooks/exhaustive-deps
					...
					<>
						...
						<div className="flex items-start justify-between border-b border-(--cl-border) px-4 py-3">
							<div className="min-w-0">
								...
								<p className="mt-0.5 line-clamp-2 text-sm font-semibold text-(--cl-text)">
									{part.part_name}
								</p>
							</div>
							<button
								...
								<div className="space-y-4">
									...
									<section>
										...
										<div className="divide-y divide-(--cl-border) rounded-lg border border-(--cl-border) bg-(--cl-surface-3) px-3">
											<InfoRow
												label="Part Code"
												value={<span className="font-mono">{part.part_code}</span>}
											/>
									...
									<section>
										...
										<div className="divide-y divide-(--cl-border) rounded-lg border border-(--cl-border) bg-(--cl-surface-3) px-3">
											<InfoRow
												label="Cost Price"
												value={
													part.cost_price != null ? `₹ ${part.cost_price.toFixed(2)}` : null
												}
											/>
											<InfoRow
												label="MRP"
												value={part.mrp != null ? `₹ ${part.mrp.toFixed(2)}` : null}
											/>
											<InfoRow label="HSN Code" value={part.hsn_code} />
											<InfoRow
												label="GST Rate"
												value={part.gst_rate != null ? `${part.gst_rate}%` : null}
											/>
										</div>
									</section>
											...
											<InfoRow
												...
												value={
													part.location_count > 0
														? `${part.location_count} location${part.location_count !== 1 ? "s" : ""}`
														: "Unassigned"
												}
											/>
										</div>
									...
									{loadingStock ? (
										<div className="space-y-2">
											{Array.from({ length: 4 }).map((_, i) => (
												<div
													key={i}
													className="h-8 animate-pulse rounded bg-(--cl-surface-3)"
												/>
											))}
										</div>
									) : (
												...
												{stockSummary?.last_snapshot_date ? (
													<>
														<div className="mb-2 flex items-center justify-between">
															<span className="font-semibold text-(--cl-text-muted)">
																Last Snapshot
															</span>
															<span className="font-mono text-(--cl-text)">
																{stockSummary.last_snapshot_date}
															</span>
														</div>
														<div className="mb-2 flex items-center justify-between border-b border-(--cl-border) pb-2">
															...
															<span className="font-bold tabular-nums text-(--cl-text)">
																{stockSummary.snapshot_closing.toFixed(3)}
															</span>
														</div>
														<p className="mb-1 font-semibold uppercase tracking-wide text-(--cl-accent-text)">
															Since snapshot
														</p>
														<div className="space-y-1">
															{stockSummary.purchase_in_since > 0 && (
																<div className="flex justify-between">
																	...
																	<span className="tabular-nums text-emerald-600">
																		+{stockSummary.purchase_in_since.toFixed(3)}
																	</span>
																</div>
															)}
															{stockSummary.purchase_out_since > 0 && (
																<div className="flex justify-between">
																	...
																	<span className="tabular-nums text-orange-600">
																		-{stockSummary.purchase_out_since.toFixed(3)}
																	</span>
																</div>
															)}
															{stockSummary.sales_in_since > 0 && (
																<div className="flex justify-between">
																	...
																	<span className="tabular-nums text-emerald-600">
																		+{stockSummary.sales_in_since.toFixed(3)}
																	</span>
																</div>
															)}
															{stockSummary.sales_out_since > 0 && (
																<div className="flex justify-between">
																	...
																	<span className="tabular-nums text-orange-600">
																		-{stockSummary.sales_out_since.toFixed(3)}
																	</span>
																</div>
															)}
															{stockSummary.adjust_in_since > 0 && (
																<div className="flex justify-between">
																	...
																	<span className="tabular-nums text-emerald-600">
																		+{stockSummary.adjust_in_since.toFixed(3)}
																	</span>
																</div>
															)}
															{stockSummary.adjust_out_since > 0 && (
																<div className="flex justify-between">
																	...
																	<span className="tabular-nums text-orange-600">
																		-{stockSummary.adjust_out_since.toFixed(3)}
																	</span>
																</div>
															)}
															<div className="flex justify-between border-t border-(--cl-border) pt-1 font-semibold">
																...
																<span
																	className={`tabular-nums ${stockSummary.net_since_snapshot >= 0 ? "text-emerald-600" : "text-orange-600"}`}
																>
																	{stockSummary.net_since_snapshot >= 0 ? "+" : ""}
																	{stockSummary.net_since_snapshot.toFixed(3)}
																</span>
															</div>
														</div>
													</>
												...
												<div className="overflow-hidden rounded-lg border border-(--cl-border)">
													<table className="w-full text-sm">
														<thead className="bg-(--cl-surface-3)">
															<tr>
																<th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-(--cl-text-muted)">
																	Location
																</th>
																<th className="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wide text-(--cl-text-muted)">
																	Qty
																</th>
															</tr>
														</thead>
														<tbody className="divide-y divide-(--cl-border)">
															{stockByLoc.map((loc) => (
																<tr
																	key={loc.location_id}
																	className="hover:bg-(--cl-surface-3)"
																>
									...
									{loadingHist ? (
										<div className="space-y-3">
											{Array.from({ length: 5 }).map((_, i) => (
												<div
													key={i}
													className="h-16 animate-pulse rounded-lg bg-(--cl-surface-3)"
												/>
											))}
										</div>
									) : history.length === 0 ? (
											...
											{history.map((h) => (
												<div key={h.id} className="relative">
													...
													<div className="rounded-lg border border-(--cl-border) bg-(--cl-surface-3) p-3">
														...
														{h.remarks && (
															<p className="mt-1 text-xs italic text-(--cl-text-muted)">
																{h.remarks}
															</p>
														)}
													</div>
