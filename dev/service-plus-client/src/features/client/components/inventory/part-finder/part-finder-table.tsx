...
export const PartFinderTable = ({ loading, onSelectPart, parts, selectedId }: Props) => {
	...
	function ColHead({ field, label }: { field: SortField; label: string }) {
		return (
			...
			>
				{label}
				<SortIcon dir={sortDir} field={field} sortField={sortField} />
			</TableHead>
		);
	}
								...
								<motion.tr
									...
									className={`cursor-pointer border-b transition-colors last:border-b-0 ${
										isActive
											...
											: "border-(--cl-border) hover:bg-(--cl-surface-3) " +
												(idx % 2 === 1 ? "bg-(--cl-surface-3)/40" : "")
									}`}
									initial="hidden"
									key={part.id}
										...
										{part.primary_location ? (
											<span className="flex items-center gap-1 text-sm text-(--cl-text-muted)">
												...
												{part.location_count > 1 && (
													<Badge
														className="ml-1 border-sky-200 bg-sky-50 text-sky-600 dark:border-sky-800 dark:bg-sky-950/30 dark:text-sky-400"
														variant="outline"
													>
														+{part.location_count - 1}
													</Badge>
