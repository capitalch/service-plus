...
const hdrCellCls =
	"text-[11px] font-extrabold uppercase tracking-widest text-(--cl-text) py-3 px-2 flex items-center justify-left border-b border-r border-(--cl-border) last:border-r-0 bg-zinc-200/50 dark:bg-zinc-800/50";
...
export function NewLoanEntry({
	...
	useEffect(() => {
		...
		apolloClient
			.query<GenericQueryData<StockLoanWithLines>>({
			...
			{!branchId ? (
				...
				<>
					...
					<p className="text-[10px] font-black uppercase tracking-[0.15em] text-(--cl-text-muted) px-1 mb-1">
						Loan Details
						{editLoan && (
							<span className="px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 border border-amber-500/20 ml-2">
								Edit
							</span>
						)}
					</p>
						...
						>
							<div className="min-w-[800px]">
								...
								<div className={`grid ${COLS} sticky top-0 z-20 backdrop-blur-md`}>
									...
									<div className={hdrCellCls}>
										Loan To <span className="text-red-500 ml-0.5">*</span>
									</div>
									<div className={cn(hdrCellCls, "justify-center")}>
										IN / OUT <span className="text-red-500 ml-0.5">*</span>
									</div>
									<div className={cn(hdrCellCls, "justify-end px-3")}>
