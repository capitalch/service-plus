                ...
                <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="flex items-baseline gap-2 overflow-hidden">
                        <h1 className="text-lg font-bold text-(--cl-text) truncate">
                            Loan Entry
                            {mode === "new" && !editLoan && <span className="ml-2 text-sm font-medium text-(--cl-text-muted) whitespace-nowrap">— New</span>}
                    ...
                    <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
                            ...
                            {loading ? (
                                ...
                                <table className="min-w-full border-collapse">
                                    ...
                                    <tbody className="divide-y divide-(--cl-border) bg-(--cl-surface)">
                                        {loans.map((loan, idx) => (
                                            <tr key={loan.id} className="group transition-colors hover:bg-(--cl-accent)/5 dark:hover:bg-white/[0.03]">
                        ...
                        <div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
                            <span className="text-xs text-(--cl-text-muted)">
                                {total === 0 ? "No loans" : `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} loans (Page ${page} of ${totalPages})`}
                            </span>
                            <div className="flex items-center gap-1">
                                <Button
                                    className="h-7 w-7"
