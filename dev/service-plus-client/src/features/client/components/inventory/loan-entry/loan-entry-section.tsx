...
import {
	loanEntryFormSchema,
	type LoanEntryFormValues,
	getLoanEntryDefaultValues,
	getInitialLoanLine,
...
export const LoanEntrySection = () => {
	...
	const executeSave = async (values: LoanEntryFormValues) => {
		...
		if (!selectedBrandId) {
			toast.error(MESSAGES.ERROR_LOAN_BRAND_REQUIRED);
			return;
		}
					...
					<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
								...
								<table className="min-w-full border-collapse">
									<thead className="sticky top-0 z-30">
										<tr className="bg-(--cl-surface-2)">
											...
											<th className={thClass} style={{ width: "18%" }}>
												Ref #
											</th>
											<th className={thClass} style={{ width: "43%" }}>
												Remarks
											</th>
									...
									<tbody>
										{Array.from({ length: 15 }).map((_, i) => (
											<tr key={i} className="animate-pulse">
												...
												<td className={tdClass}>
													<div className="h-4 w-32 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="h-4 w-96 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
														...
														<DropdownMenu>
															...
															>
																<DropdownMenuItem
																	...
																	onClick={() => {
																		setEditLoan(loan);
																		setSelectedBrand(String(loan.brand_id));
																		setMode("new");
																	}}
																>
						...
						<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
							<span className="text-xs text-(--cl-text-muted)">
								{total === 0
									? "No loans"
									: `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} loans (Page ${page} of ${totalPages})`}
							</span>
							<div className="flex items-center gap-1">
								<Button
									className="h-7 w-7"
					...
					>
						<DialogContent aria-describedby={undefined} className="sm:max-w-sm">
							...
							<p className="text-sm text-muted-foreground">
								This will permanently delete the loan entry and all associated stock transactions. This
