...
export const SalesEntrySection = () => {
	...
	const divisionCodeById = (id: number | null) => availableDivisions.find((d) => d.id === id)?.code ?? null;
	...
	const canSave =
		!!customerName.trim() && !!customerStateCode && !!selectedBrand && linesValid && isValidGstin(customerGstin);
	...
	const sinvSequence = docSequences.find((ds) => ds.document_type_code === "SALES_INVOICE" && ds.id != null) ?? null;
	...
	useEffect(() => {
		...
		const fetchMeta = async () => {
			try {
				...
				setMasterStates(
					(masterStateRes.data?.genericQuery ?? []).map((s) => ({
						id: s.id,
	...
	const loadData = useCallback(
		async (bId: number, from: string, to: string, q: string, pg: number, divisionId: number | null = null) => {
			if (!dbName || !schema) return;
			setLoading(true);
			try {
		...
		try {
			...
			const sheetData = [
				...
				...invLines.map((l, i) => [
					...
					Number(l.unit_price),
					Number(l.gst_rate),
					Number(l.cgst_amount),
					Number(l.sgst_amount),
					Number(l.igst_amount),
		...
		const sheetData = [
			...
			...invoices.map((inv) => {
				...
				return [
					...
					inv.invoice_no,
					inv.customer_name,
					s * Number(inv.aggregate_amount),
					s * Number(inv.cgst_amount),
					s * Number(inv.sgst_amount),
					s * Number(inv.igst_amount),
	...
	const handleDownloadAllPdf = () => {
		...
		// Returns are shown as negative figures so they net against sales.
		const sumSigned = (rows: typeof invoices) =>
			rows.reduce(
				(acc, inv) => {
					const s = inv.is_return ? -1 : 1;
					acc.aggregate += s * Number(inv.aggregate_amount);
	...
	return (
		...
		>
			...
			<div className="flex flex-wrap items-center gap-x-4 gap-y-3 border-b border-(--cl-border) bg-(--cl-surface) px-4 py-1">
				...
				<div className="flex items-center gap-3 overflow-hidden">
					...
					<div className="flex items-baseline gap-2 overflow-hidden">
						...
						{mode === "new" && (
							<div
								className={`flex items-center gap-1 px-1.5 py-1 rounded-sm border shadow-sm animate-in fade-in zoom-in duration-500 delay-150 ml-4 ${
									isGstMode
							...
							>
								...
								<span
									className={`text-[10.5px] font-bold uppercase tracking-tighter ${isGstMode ? "text-emerald-700" : "text-amber-700"}`}
								>
				...
				<label
					className={`flex items-center gap-1.5 cursor-pointer select-none px-3 py-1.5 rounded-lg border-2 font-black text-[12px] uppercase tracking-[0.1em] transition-all shadow-sm ${
						mode !== "new"
				...
				>
					...
					>
						<RefreshCw
							className={`h-3.5 w-3.5 text-blue-600 ${form.formState.isSubmitting ? "animate-spin" : ""}`}
						/>
						Reset
					</Button>
					<Button
			...
			{mode === "new" ? (
				...
				<>
					...
					<div className="flex flex-wrap items-center gap-2 px-4 py-2 bg-(--cl-surface-2)/30">
						...
						{availableDivisions.length > 0 && (
							<select
								...
								onChange={(e) => {
									setViewDivisionId(e.target.value ? Number(e.target.value) : null);
									setPage(1);
								}}
							>
					...
					<div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-(--cl-border) bg-(--cl-surface) shadow-sm">
								...
								<table className="min-w-full border-collapse">
									<thead className="sticky top-0 z-30">
										<tr className="bg-(--cl-surface-2)">
											...
											<th className={thClass} style={{ width: "26%" }}>
												Customer
											</th>
											<th className={`${thClass} text-right`} style={{ width: "10%" }}>
												Aggregate
											</th>
											<th className={`${thClass} text-right`} style={{ width: "8%" }}>
												CGST
											</th>
											<th className={`${thClass} text-right`} style={{ width: "8%" }}>
												SGST
											...
											<th className={`${thClass} text-right`} style={{ width: "12%" }}>
												Total
											</th>
											<th className={`${thClass} text-center`} style={{ width: "11%" }}>
												Actions
											</th>
										</tr>
									</thead>
										...
										{Array.from({ length: 15 }).map((_, i) => (
											<tr key={i} className="animate-pulse">
												<td className={tdClass}>
													<div className="h-4 w-4 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="h-4 w-16 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="h-4 w-24 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="h-4 w-40 rounded bg-(--cl-border)" />
												</td>
												<td className={`${tdClass} text-right`}>
													<div className="ml-auto h-4 w-16 rounded bg-(--cl-border)" />
												</td>
												<td className={`${tdClass} text-right`}>
													<div className="ml-auto h-4 w-12 rounded bg-(--cl-border)" />
												</td>
												<td className={`${tdClass} text-right`}>
													<div className="ml-auto h-4 w-12 rounded bg-(--cl-border)" />
												</td>
												...
												<td className={`${tdClass} text-right`}>
													<div className="ml-auto h-4 w-20 rounded bg-(--cl-border)" />
												</td>
												<td className={tdClass}>
													<div className="mx-auto h-4 w-8 rounded bg-(--cl-border)" />
												</td>
											</tr>
										))}
									</tbody>
								...
								<table className="min-w-full border-collapse">
									...
									<tbody className="divide-y divide-(--cl-border) bg-(--cl-surface)">
										{invoices.map((inv, idx) => (
											...
											>
												<td
													className={`${tdClass} text-(--cl-text-muted)`}
													style={{ width: "5%" }}
												>
												...
												>
													{inv.invoice_no}
													{inv.is_return && (
														<span className="ml-1.5 text-[10px] font-bold text-orange-600 bg-orange-100 dark:bg-orange-950/40 rounded px-1 py-0.5">
															Return
														</span>
													)}
													{(postDataToAccounts ||
														...
														<div className="mt-0.5 flex items-center gap-1.5">
															{postDataToAccounts && (
																<span
																	className={`text-[10px] font-semibold ${inv.is_posted ? "text-emerald-600" : "text-amber-600"}`}
																>
												...
												<td
													className={`${tdClass} text-right font-medium text-(--cl-accent)`}
													style={{ width: "12%" }}
												>
															...
															>
																...
																<DropdownMenuItem
																	...
																	onClick={() => {
																		...
																		setSelectedBrand(String(inv.brand_id ?? ""));
																		setEditInvoice(inv);
																		setMode("new");
																	}}
																>
									...
									<tfoot className="sticky bottom-0 z-10 bg-(--cl-surface-2) shadow-[0_-1px_0_var(--cl-border)] border-t border-t-(--cl-border)">
										<tr>
											...
											<td
												className={`${tdClass} text-right font-bold text-(--cl-accent) text-[13px]`}
											>
						...
						<div className="flex items-center justify-between border-t border-(--cl-border) px-4 py-2">
							<span className="text-xs text-(--cl-text-muted)">
								{total === 0
									? "No invoices"
									: `Showing ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, total)} of ${total} invoices (Page ${page} of ${totalPages})`}
							</span>
							<div className="flex items-center gap-1">
								<Button
									className="h-7 w-7"
					...
					<SalesInvoicePdfPreviewDialog
						division={
							pdfPreviewInvoice
								? (availableDivisions.find((d) => d.id === pdfPreviewInvoice.division_id) ??
									currentDivision)
								: null
						}
						...
						onOpenChange={(open) => {
							if (!open) setPdfPreviewInvoice(null);
						}}
					/>
					...
					>
						<DialogContent aria-describedby={undefined} className="sm:max-w-sm !bg-white text-(--cl-text)">
							...
							<p className="text-sm text-(--cl-text-muted)">
								This will permanently delete the invoice and all associated stock transactions. This
								action cannot be undone.
							</p>
							<DialogFooter>
								<Button disabled={deleting} variant="outline" onClick={() => setDeleteId(null)}>
									Cancel
								</Button>
			...
			<Dialog
				open={diffAlertMsg !== null}
				onOpenChange={(open) => {
					if (!open) setDiffAlertMsg(null);
				}}
			>
				<DialogContent aria-describedby={undefined} className="sm:max-w-md !bg-white text-(--cl-text)">
					...
					<DialogFooter>
						<Button className="cursor-pointer" onClick={() => setDiffAlertMsg(null)}>
							OK
						</Button>
					</DialogFooter>
				</DialogContent>
