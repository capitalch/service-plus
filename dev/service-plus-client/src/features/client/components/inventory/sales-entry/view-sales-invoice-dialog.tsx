...
const thClass =
	"text-xs font-extrabold uppercase tracking-widest text-zinc-500 p-2 text-left border-b border-zinc-200 bg-zinc-100/80";
const tdClass = "p-2 text-sm text-zinc-700 border-b border-zinc-100";

// ─── Component ────────────────────────────────────────────────────────────────

export const ViewSalesInvoiceDialog = ({ invoice, open, onOpenChange, onShowPdf }: Props) => {
	...
	useEffect(() => {
		...
		setLoading(true);
		apolloClient
			.query<GenericQueryData<DetailRow>>({
				...
				{loading ? (
					...
					<div className="space-y-4">
						...
						<div className="grid grid-cols-2 gap-x-6 gap-y-3 rounded-xl border border-zinc-200 bg-zinc-50/50 p-5 text-sm sm:grid-cols-4 shadow-sm">
							...
							<Field label="Customer" value={detail.customer_name} className="sm:col-span-2" />
							{detail.customer_gstin && <Field label="GSTIN" value={detail.customer_gstin} />}
							{detail.customer_state_code && (
								<Field label="State Code" value={detail.customer_state_code} />
							)}
							{detail.remarks && (
								<Field
									label="Remarks"
									value={detail.remarks}
									className="col-span-2 sm:col-span-4 border-t border-zinc-100 pt-2 mt-1"
								/>
							)}
						</div>
							...
							<table className="min-w-full border-collapse text-sm">
								...
								<tbody className="bg-white">
									{lines.map((line, idx) => (
										<tr key={line.id} className="hover:bg-zinc-50/80 transition-colors">
											...
											<td className={`${tdClass} text-right`}>
												{formatCurrency(line.aggregate_amount)}
											</td>
								...
								<tfoot>
									<tr className="bg-zinc-100/50 font-bold border-t-2 border-zinc-200">
										<td
											colSpan={6}
											className="p-2 text-right text-xs uppercase tracking-widest text-zinc-500"
										>
											Totals
										</td>
										<td className="p-2 text-right text-sm text-zinc-900 font-bold">
										...
										<td className="p-2 text-right text-sm text-(--cl-accent) font-extrabold">
						...
						<div className="flex justify-end">
							<div className="w-72 rounded-lg border border-zinc-200 bg-zinc-50 overflow-hidden text-sm">
								...
								{[
									...
									<div
										key={row.label}
										className="flex items-center justify-between px-4 py-1.5 border-b border-zinc-100 last:border-b-0"
									>
										<span
											className={`text-xs font-semibold uppercase tracking-wide ${row.muted ? "text-zinc-400" : "text-zinc-500"}`}
										>
											{row.label}
										</span>
										<span
											className={`font-mono tabular-nums ${row.muted ? "text-zinc-600" : "text-zinc-700 font-semibold"}`}
										>
								...
								<div className="flex items-center justify-between px-4 py-2 bg-zinc-100 border-t border-zinc-200">
									...
									<span className="font-mono tabular-nums font-bold text-zinc-700">
										{formatCurrency(computedTotal)}
									</span>
								</div>
								...
								<div
									className={`flex items-center justify-between px-4 py-1.5 border-t border-zinc-100 ${Math.abs(diffAmount) > 0.01 ? "bg-amber-50" : ""}`}
								>
									<span
										className={`text-xs font-semibold uppercase tracking-wide ${Math.abs(diffAmount) > 0.01 ? "text-amber-500" : "text-zinc-400"}`}
									>
										Difference
									</span>
									<span
										className={`font-mono tabular-nums font-semibold ${Math.abs(diffAmount) > 0.01 ? "text-amber-600" : "text-emerald-600"}`}
									>
										{diffAmount > 0 ? "+" : ""}
								...
								<div className="flex items-center justify-between px-4 py-2.5 bg-white border-t-2 border-zinc-300">
									...
									<span className="font-mono tabular-nums text-base font-extrabold text-(--cl-accent)">
