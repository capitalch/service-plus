...
export function NewSalesInvoice({
	...
	useEffect(() => {
		...
		apolloClient
			.query<GenericQueryData<SalesInvoiceType & { lines: SalesLineFormItem[] }>>({
			...
			.then((res) => {
				...
				const editDivisionGstStateCode =
					availableDivisions.find((d) => d.id === editDivisionId)?.gst_state_code ?? effectiveGstStateCode;
				const newIsIgst =
					!!detail.customer_state_code &&
					!!editDivisionGstStateCode &&
					detail.customer_state_code !== editDivisionGstStateCode;
				setIsIgst(newIsIgst);
				const loadedLines = ((detail as any).lines ?? []).map(
					(l: any) =>
						({
	...
	useEffect(() => {
		if (skipTargetSyncRef.current) {
			skipTargetSyncRef.current = false;
			return;
		}
		...
		>
			{!branchId ? (
				...
				<>
								...
								<div className="space-y-2 md:col-span-3 lg:col-span-3">
									...
									<div className="relative">
										<select
											...
											value={divisionId || ""}
											onChange={(e) =>
												onDivisionChange(e.target.value ? Number(e.target.value) : 0)
											}
										>
					...
					<Card
						className={`border-(--cl-border) shadow-sm relative bg-(--cl-surface) flex min-h-[45vh] lg:flex-1 flex-col ${isReturn ? "border-l-4 border-l-orange-500" : ""}`}
					>
							...
							<table className="min-w-[920px] w-full border-collapse text-sm sticky-header">
								<thead>
									<tr className="bg-(--cl-surface-2)/50">
										...
										<th className={thClass} style={{ width: "16%" }}>
											Part <span className="text-red-500 ml-0.5">*</span>
										</th>
										<th className={thClass} style={{ width: "7%" }}>
											HSN
										</th>
										<th className={`${thClass} text-right`} style={{ width: "5%" }}>
										...
										<th className={thClass} style={{ width: "11%" }}>
											Remarks
										</th>
								...
								<tbody className="bg-(--cl-surface)">
									{fields.map((field, idx) => {
										...
										return (
											<tr
												key={field.id}
												className="hover:bg-(--cl-surface-2)/30 group transition-colors"
											>
												<td
												...
												<td className={tdClass}>
													<PartCodeInput
														...
														brandName={brandName}
														onChange={(code) => {
															const patch: Partial<SalesLineFormItem> = {
																part_code: code,
															};
															if (!code.trim()) {
																patch.part_id = null;
															...
															if (isGstMode) {
																...
																const mrpExGst =
																	Number(part.mrp) > 0
																		? Math.round(
																				(Number(part.mrp) /
																					(1 + gstRateForCalc / 100)) *
																					100,
																			) / 100
																		: 0;
																const markupPrice =
																	Math.round(
																		baseCostPrice * (1 + markupPct / 100) * 100,
																	) / 100;
																unitPrice =
																	mrpExGst ||
																...
																const markupPrice =
																	Math.round(
																		computedCostPrice * (1 + markupPct / 100) * 100,
																	) / 100;
																unitPrice =
																	Number(part.mrp) || sellingWithGst || markupPrice;
															}
												...
												{/* Aggregate (read-only) */}
												<td
													className={`${tdClass} px-2 text-right pt-1 font-mono tabular-nums text-(--cl-text-muted) border-l border-(--cl-border) bg-(--cl-surface-2)/40`}
												>
												...
												{!isIgst ? (
													<>
														<td
															className={`${tdClass} px-2 text-right pt-1 font-mono tabular-nums text-(--cl-text-muted) bg-(--cl-surface-2)/40`}
														>
												...
												{isGstMode && (
													<GstPriceCell
														...
														onCommit={(newUnitPrice) =>
															updateLine(idx, { unit_price: newUnitPrice })
														}
													/>
												)}
												...
												{/* Total */}
												<td
													className={`${tdClass} px-2 text-right pt-1 font-mono font-semibold tabular-nums text-(--cl-text) bg-(--cl-surface-2)/40`}
												>
												...
												<td className={`${tdClass} relative`}>
													...
													{(() => {
														const profit =
															(line.unit_price - (line.cost_price ?? 0)) * line.qty;
														return (
															<span
																title="Line profit"
								...
								<tfoot>
									<tr className="bg-(--cl-surface-2)/60">
										...
										<td
											className="p-0.5 border-t border-(--cl-border) text-right px-1"
											title="Total profit"
										>
											<span
												className={`inline-flex items-center gap-0.5 rounded px-1 py-0.5 text-[10px] font-bold leading-none tabular-nums ${
													totals.profit < 0
														? "bg-red-500/10 text-red-600"
														: "bg-emerald-500/10 text-emerald-600"
												}`}
											>
												<span className="opacity-60">P</span>₹{formatNumber(totals.profit)}
											</span>
										</td>
										<td className="p-0.5 border-t border-(--cl-border)" />
									</tr>
					...
					<div
						className={`sticky bottom-0 z-30 w-full shrink-0 self-stretch lg:w-fit lg:self-end rounded-lg border-2 overflow-hidden bg-(--cl-surface) ${isReturn ? "border-orange-500/30" : "border-(--cl-accent)/30"}`}
					>
							...
							<div className="flex flex-wrap items-center justify-between gap-4">
								...
								<div className="flex flex-wrap items-center gap-4">
									...
									<div className="flex items-center gap-1.5">
										...
										<span className="tabular-nums text-sm font-semibold text-(--cl-text)">
											₹{formatNumber(totals.cgst + totals.sgst + totals.igst)}
										</span>
									</div>
									<div className="flex items-center gap-1.5">
										<span className="text-xs font-medium uppercase tracking-wide text-(--cl-text-muted)">
