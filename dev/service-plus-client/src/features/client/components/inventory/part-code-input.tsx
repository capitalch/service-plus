...
export const PartCodeInput = forwardRef<HTMLInputElement, PartCodeInputProps>(
	(
		...
		useEffect(() => {
			if (!partId) setInternalCostPrice(null);
		}, [partId]);

		// Track description of the last-selected part (cleared when part is deselected)
		const [internalPartDescription, setInternalPartDescription] = useState<string | null>(null);
		useEffect(() => {
			if (!partId) setInternalPartDescription(null);
		}, [partId]);
		...
		const effectiveCostPrice =
			costPrice !== undefined ? costPrice : (internalCostPrice ?? cached?.costPrice ?? null);
		const effectivePartDescription =
			partDescription !== undefined ? partDescription : (internalPartDescription ?? cached?.description ?? null);
		...
		useEffect(() => {
			...
			partDebounceRef.current = setTimeout(async () => {
				...
				try {
					const sqlId =
						partSearchMode === "code" ? SQL_MAP.GET_PARTS_BY_CODE_PREFIX : SQL_MAP.GET_PARTS_BY_KEYWORD;
					const sqlCountId =
						partSearchMode === "code"
							? SQL_MAP.GET_PARTS_BY_CODE_PREFIX_COUNT
							: SQL_MAP.GET_PARTS_BY_KEYWORD_COUNT;
			...
			<>
				<div className={`flex flex-col gap-0.5 py-1${className ? ` ${className}` : ""}`}>
							...
							onMouseDown={(e) => {
								...
								if (isScrollbar) {
									...
									document.addEventListener(
										"mouseup",
										() => {
											scrollbarMouseDownRef.current = false;
										},
										{ once: true },
									);
								} else {
								...
								>
									<div className="flex-1 min-w-0">
										<div className="flex items-baseline justify-between gap-2">
											<span className="font-mono text-sm font-bold text-(--cl-text)">
												{part.part_code}
											</span>
											<span className="shrink-0 text-xs text-(--cl-text-muted) truncate max-w-[140px]">
												{part.brand_name}
											</span>
										</div>
										...
										<div className="mt-1 flex flex-wrap items-center gap-1">
											...
											{part.cost_price != null && (
												<span className="bg-blue-50 border border-blue-200 px-1.5 py-0.5 rounded text-[10px] text-blue-700">
													Cost: {formatNumber(part.cost_price)}
												</span>
											)}
											{part.mrp != null && (
												<span className="bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded text-[10px] text-amber-700">
													MRP: {formatNumber(part.mrp)}
												</span>
											)}
										</div>
									</div>
					...
					{showName && partId && partName && (
						<div className="flex items-center gap-1.5 px-1 overflow-hidden h-3 animate-in fade-in slide-in-from-top-1 duration-200">
							<span
								className="shrink-0 max-w-[45%] truncate text-[10px] font-bold text-(--cl-accent,#007acc)/70 tracking-tight"
								title={partName}
							>
								{partName}
							</span>
							...
							{effectivePartDescription && (
								<span
									className="min-w-0 truncate text-[10px] font-medium text-slate-500 dark:text-slate-400"
									title={effectivePartDescription}
								>
									· {effectivePartDescription}
								</span>
							)}
				...
				>
					...
					>
						...
						<div
							className={`flex flex-col gap-1.5 rounded-lg border p-3 transition-colors ${partSearchMode === "code" ? "border-slate-400 bg-slate-50" : "border-slate-200 bg-white"}`}
						>
							...
							<div className="relative">
								...
								<Input
									...
									value={partCodeQuery}
									onChange={(e) => {
										setPartCodeQuery(e.target.value);
										setPartPage(1);
									}}
									onFocus={() => {
										if (partSearchMode !== "code") {
											setPartSearchMode("code");
											setPartResults([]);
											setPartPage(1);
											setPartTotal(0);
						...
						<div
							className={`flex flex-col gap-1.5 rounded-lg border p-3 transition-colors ${partSearchMode === "keyword" ? "border-slate-400 bg-slate-50" : "border-slate-200 bg-white"}`}
						>
							<label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest select-none">
							...
							<div className="relative">
								...
								<Input
									...
									onChange={(e) => {
										setPartKeywordQuery(e.target.value);
										setPartPage(1);
									}}
									onFocus={() => {
										if (partSearchMode !== "keyword") {
											setPartSearchMode("keyword");
											setPartResults([]);
											setPartPage(1);
											setPartTotal(0);
										}
						...
						{!partLoading && partTotal > 0 && (
							<p className="text-xs text-slate-500 text-right pr-1">
								...
								{Math.ceil(partTotal / PART_PICK_PAGE_SIZE) > 1 &&
									` · Page ${partPage} of ${Math.ceil(partTotal / PART_PICK_PAGE_SIZE)}`}
							</p>
						)}
									...
									>
										<div className="min-w-0 flex-1">
											...
											<p className="mt-1 text-xs inline-flex flex-wrap items-center gap-1.5">
												...
												{part.cost_price != null ? (
													<span className="bg-blue-50 border border-blue-200 px-1.5 py-0.5 rounded text-blue-700">
														Cost: {formatNumber(part.cost_price)}
													</span>
												) : null}
												{part.mrp != null ? (
													<span className="bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded text-amber-700">
														MRP: {formatNumber(part.mrp)}
													</span>
												) : null}
											</p>
								...
								<div className="flex items-center justify-between border-t border-slate-200 pt-2">
									...
									<div className="flex items-center gap-1">
										<Button
											...
											size="icon"
											className="h-7 w-7"
											disabled={partPage <= 1 || partLoading}
											onClick={() => setPartPage(1)}
										>
										...
										<Button
											...
											size="icon"
											className="h-7 w-7"
											disabled={partPage >= totalPages || partLoading}
											onClick={() => setPartPage((p) => p + 1)}
										>
										...
										<Button
											...
											className="h-7 w-7"
											disabled={partPage >= totalPages || partLoading}
											onClick={() => setPartPage(totalPages)}
										>
