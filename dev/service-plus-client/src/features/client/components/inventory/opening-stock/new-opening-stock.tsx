...
import {
	...
	type OpeningStockLineFormValues,
...
export function NewOpeningStock({
	...
	useEffect(() => {
		...
		apolloClient
			.query<GenericQueryData<OpeningStockType>>({
			...
			{!branchId ? (
				...
				<>
					...
					<p className="mb-1 flex items-center gap-2 px-1 text-[10px] font-black uppercase tracking-[0.15em] text-(--cl-text-muted)">
						Entry Details
						{editEntry && (
							<span className="rounded border border-amber-500/20 bg-amber-500/10 px-1.5 py-0.5 text-amber-600">
								Edit
							</span>
						)}
					</p>
					...
					{/* Section label */}
					<p className="mb-1 px-1 text-[10px] font-black uppercase tracking-[0.15em] text-(--cl-text-muted) my-2">
						Line Items
					</p>
								...
								<thead>
									<tr className="bg-(--cl-surface-2)/50">
										...
										<th className={thClass} style={{ width: "28%" }}>
											Part <span className="ml-0.5 text-red-500">*</span>
										</th>
								...
								<tbody className="bg-(--cl-surface)">
									{fields.map((field, idx) => {
										...
										return (
											<tr
												key={field.id}
												className="group transition-colors hover:bg-(--cl-surface-2)/30"
											>
												<td
													className={`${tdClass} pl-4 pt-1 text-xs font-medium text-(--cl-text-muted)`}
												>
													{idx + 1}
												</td>
												...
												<td className={tdClass}>
													<PartCodeInput
														...
														onChange={(code) => {
															const patch: Partial<OpeningStockLineFormValues> = {
																part_code: code,
															};
															if (!code.trim()) {
																patch.part_id = null;
					...
					<div
						ref={summaryRef}
						className="flex flex-wrap items-center justify-end gap-x-6 gap-y-1 rounded-lg border border-(--cl-border) bg-(--cl-surface-2)/40 px-4 py-2.5"
					>
