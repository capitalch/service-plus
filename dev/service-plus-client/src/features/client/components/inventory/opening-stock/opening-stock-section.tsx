...
export const OpeningStockSection = () => {
		...
		async (bId: number) => {
			...
			try {
				...
				if (!found) {
					form.reset({
						...getOpeningStockDefaultValues(),
						lines: [getInitialOpeningStockLine(selectedBrandId)],
					});
					setOriginalLineIds([]);
				}
			} catch {
	...
	const executeSave = async (values: OpeningStockFormValues) => {
		...
		try {
			...
			toast.error(
				existingEntry ? MESSAGES.ERROR_OPENING_STOCK_UPDATE_FAILED : MESSAGES.ERROR_OPENING_STOCK_CREATE_FAILED,
			);
		}
	};
			...
			>
				<DialogContent aria-describedby={undefined} className="sm:max-w-sm !bg-white text-(--cl-text)">
					...
					<p className="text-sm text-(--cl-text-muted)">
						This will permanently delete the opening stock entry and all associated stock transactions. This
						action cannot be undone.
					</p>
					<DialogFooter>
						<Button disabled={deleting} variant="outline" onClick={() => setDeleteConfirmOpen(false)}>
							Cancel
						</Button>
