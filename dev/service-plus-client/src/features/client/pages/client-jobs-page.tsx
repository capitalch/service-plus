	...
	return (
		<div className="flex flex-1 items-center justify-center rounded-lg border border-(--cl-border) bg-(--cl-surface-2) p-20">
			<div className="text-center">
				...
				<p className="mt-2 text-xs text-(--cl-text-muted)">This jobs feature is coming soon.</p>
			</div>
		</div>
	);
}
...
function JobsContent() {
	...
	const navigateToBatchEdit = useCallback(
		(batchNo: number) => {
	...
	switch (selected) {
		case "Single Job":
			return (
				<SingleJobSection
					onNavigateToBatchEdit={navigateToBatchEdit}
					forceView={forceSingleJobView}
					onViewModeApplied={() => setForceSingleJobView(false)}
				/>
			);
		case "Batch Jobs":
			return (
				<BatchJobSection
					initialEditBatchNo={pendingBatchNo}
					onEditBatchNoApplied={() => setPendingBatchNo(null)}
					onReturnToSingleJob={returnToSingleJob}
				/>
			);
		case "Opening Jobs":
