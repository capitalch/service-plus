...
const STYLES = {
	admin: {
		...
		trigger:
			"border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100 focus:ring-0 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700",
	},
	client: {
		...
		trigger:
			"border-(--cl-border) bg-(--cl-surface-2) text-(--cl-text) font-medium hover:bg-(--cl-hover) focus:ring-1 focus:ring-(--cl-accent)",
	},
} as const;
		...
		<div className={`flex items-center gap-2 ${isClient ? "rounded-md bg-(--cl-surface-2) px-2.5 py-1" : ""}`}>
			...
			<div className={`flex items-center gap-1.5 ${isClient ? "" : "flex-col gap-0.5 items-start"}`}>
				...
				{availableBus.length === 1 ? (
					...
					<Select value={currentBu ? String(currentBu.id) : undefined} onValueChange={handleBuChange}>
				...
				<>
					...
					<div className={`flex items-center gap-1.5 ${isClient ? "" : "flex-col gap-0.5 items-start"}`}>
						<span className={`flex shrink-0 items-center gap-1 font-medium ${s.labelSize} ${s.label}`}>
							...
							<span className={isClient ? "hidden lg:inline" : "inline"}>Branch</span>
						</span>
						{availableBranches.length === 1 ? (
							<span className={`flex h-7 items-center rounded-md border px-2.5 text-xs ${s.static}`}>
				...
				<>
					...
					<div className={`flex items-center gap-1.5 ${isClient ? "" : "flex-col gap-0.5 items-start"}`}>
						<span className={`flex shrink-0 items-center gap-1 font-medium ${s.labelSize} ${s.label}`}>
							...
							<span className={isClient ? "hidden lg:inline" : "inline"}>Division</span>
						</span>
						<Select
