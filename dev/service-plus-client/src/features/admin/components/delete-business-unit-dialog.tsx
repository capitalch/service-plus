...
export const DeleteBusinessUnitDialog = ({ bu, onOpenChange, onSuccess, open }: DeleteBusinessUnitDialogPropsType) => {
			...
			if (bu.schema_exists) {
				await apolloClient.mutate({
					...
					variables: {
						db_name: dbName,
						schema: "security",
						value: encodeURIComponent(JSON.stringify({ code: buCode, delete_bu_row: true })),
					},
				});
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						Permanently delete <span className="font-semibold text-slate-800">{bu.name}</span>?
					</DialogDescription>
				</DialogHeader>
				...
				<div className="flex flex-col gap-4">
					...
					{bu.schema_exists && (
						<div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
							...
							<p className="text-sm text-amber-800">
								<span className="font-semibold">Warning:</span> {MESSAGES.INFO_BU_SCHEMA_DROP_WARNING}{" "}
								The schema <span className="font-mono font-semibold">{buCode}</span> will be dropped.
							</p>
						</div>
					)}
					...
					<div className="flex flex-col gap-1.5">
						<Label htmlFor="confirm_bu_code">
							Type <span className="font-mono font-semibold text-slate-800">{buCode}</span> to confirm
						</Label>
						<Input
							autoComplete="off"
							className="font-mono"
