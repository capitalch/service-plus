		...
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent className="sm:max-w-sm">
				<DialogHeader>
					...
					<DialogDescription>
						Send a reset link to <span className="font-semibold text-slate-800">{user.full_name}</span> ·{" "}
						<span className="text-slate-500">{user.email}</span>
					</DialogDescription>
				</DialogHeader>
				<p className="text-sm text-slate-600">
					A reset link (valid for 48 hours) will be emailed to the user. No password is changed at this stage.
				</p>
				{alert && (
					<Alert className="relative pr-8" variant={alert.variant}>
						<AlertDescription>
							{alert.message}
							{alert.detail && <span className="mt-1 block text-xs opacity-75">{alert.detail}</span>}
						</AlertDescription>
						<button
							aria-label="Dismiss"
