...
export const OrphanBuSchemasDialog = ({ onOpenChange, open }: OrphanBuSchemasDialogPropsType) => {
		...
		try {
			await apolloClient.mutate({
				...
				variables: {
					db_name: dbName,
					schema: "security",
					value: encodeURIComponent(JSON.stringify({ code: schemaName, delete_bu_row: false })),
				},
			});
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>
						For every Business Unit, there should be a schema with the same name. The schema contains the
						data tables for the Business Unit. Orphaned schemas are those data tables which are not
						associated with any Business Unit.
					</DialogDescription>
					<DialogDescription>
				...
				<div className="flex flex-col gap-3">
					{loading ? (
						...
						<p className="py-6 text-center text-sm text-slate-400">No orphaned schemas found.</p>
					) : (
											...
											<div className="flex items-center justify-between gap-3">
												...
												{!rs.expanded && (
													<Button
														...
														size="sm"
														variant="ghost"
														onClick={() => setRowState(s.schema_name, { expanded: true })}
													>
				...
				<DialogFooter className="flex-wrap gap-2">
					...
					<div className="flex flex-1 justify-end gap-2">
						{selected.size > 0 && (
							...
							>
								{deletingSelected ? <Loader2Icon className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : null}
