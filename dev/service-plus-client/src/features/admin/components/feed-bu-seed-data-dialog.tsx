...
export const FeedBuSeedDataDialog = ({ bu, onOpenChange, onSuccess, open }: FeedBuSeedDataDialogPropsType) => {
	...
	useEffect(() => {
		...
		apolloClient
			...
			.catch((err) => {
				const msg = err?.errors?.[0]?.message ?? MESSAGES.ERROR_BU_SEED_FEED_FAILED;
				setFeedError(msg);
			});
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent aria-describedby={undefined} className="w-full sm:max-w-[400px]">
				<DialogHeader>
					<DialogTitle className="text-base font-semibold text-foreground">Add Seed Data</DialogTitle>
				</DialogHeader>
				...
				{done && (
					<div className="flex flex-col items-center gap-4 py-6 text-center">
						...
						<p className="text-sm text-slate-500">
							Seed data for <span className="font-medium">{bu.name}</span> ({bu.code}) has been added
							successfully.
						</p>
