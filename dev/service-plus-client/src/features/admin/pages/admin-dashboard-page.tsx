...
export const AdminDashboardPage = () => {
	...
	const { data, error, loading, refetch } = useQuery<AdminDashboardDataType>(GRAPHQL_MAP.adminDashboardStats, {
			...
			>
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">Welcome back, {user?.fullName ?? user?.username}.</p>
					</div>
					<Button
				...
				<div className="rounded-lg border border-teal-100 bg-teal-50 p-4 text-sm text-teal-700">
					...
					<p className="mt-0.5 text-teal-600">
						Use the sidebar to manage users and view audit logs for your client. Switch to Client Mode to
						access service management features.
					</p>
				</div>
			</motion.div>
		</AdminLayout>
