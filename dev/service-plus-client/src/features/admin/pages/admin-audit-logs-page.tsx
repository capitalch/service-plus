...
import type { AuditEntryType, AuditLogPageType, AuditStatsType } from "@/features/super-admin/types";
...
const EntryDetailDialog = ({ entry, onClose }: { entry: AuditEntryType | null; onClose: () => void }) => (
	<Dialog
		open={!!entry}
		onOpenChange={(o) => {
			if (!o) onClose();
...
export const AdminAuditLogsPage = () => {
	...
	const [fromDate, setFromDate] = useState<string>(getDateDaysAgo(90));
	...
	useEffect(() => {
		setPage(1);
	}, [fromDate, toDate, debouncedSearch, actionFilter, outcomeFilter]);
	...
	const topAction = stats?.actionCounts[0] ? formatAction(stats.actionCounts[0].action) : "—";
			...
			>
				...
				<div>
					...
					<p className="mt-1 text-sm text-slate-500">
						Track all system actions, user activity, and configuration changes (last 90 days default).
					</p>
				</div>
					...
					>
						<Card>
							<CardHeader className="pb-2 pt-4">
								<CardTitle className="text-sm font-medium text-slate-600">Events Over Time</CardTitle>
							</CardHeader>
							<CardContent className="pb-4">
								<ResponsiveContainer height={180} width="100%">
									<AreaChart data={stats.timeSeries}>
										<defs>
											<linearGradient id="colorCountAdmin" x1="0" x2="0" y1="0" y2="1">
						...
						<Card>
							<CardHeader className="pb-2 pt-4">
								<CardTitle className="text-sm font-medium text-slate-600">Actions Breakdown</CardTitle>
							</CardHeader>
							<CardContent className="pb-4">
								<ResponsiveContainer height={180} width="100%">
									<BarChart
				...
				>
					...
					<div className="ml-auto flex gap-2">
						...
						>
							<RefreshCwIcon
								className={`mr-1.5 h-3.5 w-3.5 text-blue-600 ${logsLoading ? "animate-spin" : ""}`}
							/>
							Refresh
						</Button>
						<Button
