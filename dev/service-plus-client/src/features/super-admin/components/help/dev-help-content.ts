...

    {
        ...
        content: [
            ...
            { type: "table", headers: ["File", "Role"], rows: [
                ...
                ["templates.py", "TEMPLATES holds exactly one entry: JOB_COMPLETION. TemplateSpec carries header_params and body_params (renamed from the old positional params) — the template uses Meta's named-parameter form (parameter_name per value), so a mismatch between param name and slot is impossible by construction, unlike the old positional array."],
            ...
            { type: "para", text: "Customer Connect doesn't poll for delivery outcomes — the webhook handler publishes to app/graphql/pubsub.py's in-memory PubSub (the same one accountsPostingProgress already used for its branch-scoped progress stream) right after a status update actually applies. The new whatsappDeliveryStatus(db_name: String!) subscription (app/graphql/resolvers/subscription.py) filters server-side by db_name and yields {db_name, job_id, status, error}; the client filters further, client-side, to just the job_ids it's currently tracking — same shape as accountsPostingProgress's branchId filtering. Reusing existing graphql-ws infra instead of a client-side setInterval poll was a mid-build call." },
            { type: "heading", text: "Status ladder — never moves backwards" },
            { type: "para", text: "PENDING(0) < ACCEPTED(1) < SENT(2) < DELIVERED(3) < READ(4), FAILED(9) terminal. A callback ranked at or below the currently stored value is a no-op (200, no write) — Meta reorders and retries callbacks, so out-of-order delivery is expected, not a bug to fix upstream." },
            { type: "heading", text: "job.whatsapp_notifications — written from two places" },
