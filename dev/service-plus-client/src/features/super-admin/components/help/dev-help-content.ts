...
export const DEV_HELP_ARTICLES: HelpArticle[] = [
...

    {
        ...
        content: [
            ...
            { type: "table", headers: ["pnpm script", "Source DB / schema", "Output file", "Env var it reads"], rows: [
                ...
                ["gen-types-client", "SERVICE_PLUS_CLIENT connection, public schema (the registry DB)", "src/types/db-schema-client.ts", "SERVICE_PLUS_CLIENT"],
            ]},
...

    {
        ...
        summary: "Five events on one send rail, Meta's own delivery webhook for tracking, tenant + event routing via biz_opaque_callback_data, token-gated public PDFs, and a per-attempt send history inside the same jsonb — still no document/media send path.",
        tags: ["whatsapp", "whatsapp integration", "whatsapp_notifications", "sendWhatsappCompletion", "sendWhatsappJobIntake", "sendWhatsappJobDelivery", "sendWhatsappMoneyReceipt", "sendWhatsappJobInvoice", "biz_opaque_callback_data", "customer connect", "job intake", "job delivery", "money receipt", "job invoice", "attempts", "send history", "meta cloud api", "webhook", "jsonb_set", "token.py", "job_intake_router"],
        content: [
            { type: "para", text: "This article is the as-built implementation summary for the shared WhatsApp rail, kept current across five builds: the original completion notice (Customer Connect), the Job Intake Notice (plans/plan-whatsapp.md), paperless Job Delivery (plan-wa-delivery.md), the Money Receipt send (plan-wa-money-receipt.md), and the Invoice resend (plan-wa-invoice-resend.md). Every one of those plan docs still carries a stale '(not implemented yet)' in its title — read the per-step Done markers inside them, not the heading. Two sibling articles go deeper where an event breaks the shared pattern: 'Paperless Job Delivery' (the OTP subsystem) and 'Money Receipt & Invoice Sends' (the array-shaped log and the second token pair). Everything on this page is what all five events share. Direct Meta WhatsApp Cloud API, one shared phone_number_id across all tenants, no BSP intermediary and no provider-registry abstraction." },
            ...
            { type: "table", headers: ["event_key", "Mutation", "Trigger screen", "Grouped?", "jsonb shape"], rows: [
                ["JOB_CREATION",      "sendWhatsappJobIntake",     "New Job / Batch Job, job detail view",     "Yes — by customer_contact_id, 35/message", "flat object"],
                ["JOB_COMPLETION",    "sendWhatsappCompletion",    "Customer Connect (the only sending tab)",  "Yes — same",                               "flat object"],
                ["JOB_DELIVERY",      "sendWhatsappJobDelivery",   "delivery-modal.tsx, batch warranty jobs",  "Yes — same, one shared OTP",               "flat object + otp_*/confirmed_* fields"],
                ["JOB_MONEY_RECEIPT", "sendWhatsappMoneyReceipt",  "receipts-section.tsx row actions",         "No — one job_payment row per send",        "ARRAY, one element per payment_id"],
                ["JOB_INVOICE",       "sendWhatsappJobInvoice",    "delivered-jobs-grid.tsx row actions",      "No — one job per send",                    "flat object"],
            ]},
            { type: "note", text: "JOB_MONEY_RECEIPT is the only event that does not fit the shared write/read stack — its array value needs its own SET_JOB_MONEY_RECEIPT_WHATSAPP_ATTEMPT and its own Customer Connect components. Every other event, including the two newest, reuses SET_JOB_WHATSAPP_ATTEMPT/_OUTCOME and GET_WHATSAPP_EVENT_LOG_PAGED unchanged, because those took %(event_key)s as a bind parameter from the start — a generalization that has now paid off three separate times." },
            ...
            { type: "table", headers: ["File", "Role"], rows: [
                ...
                ["templates.py", "TEMPLATES holds six TemplateSpec entries for five events: JOB_COMPLETION (job_completed_ready_for_pickup_v2), JOB_CREATION (job_intake_notice_v2), JOB_DELIVERY (job_delivery_notice_v1), JOB_DELIVERY_OTP (job_delivery_otp_v1 — the sixth entry, the only AUTHENTICATION-category one, and the reason the count is six not five), JOB_MONEY_RECEIPT (job_money_receipt_v1) and JOB_INVOICE (job_invoice_v1). TemplateSpec carries header_params/body_params (Meta's named-parameter form for UTILITY templates — a param-name/slot mismatch is impossible by construction; AUTHENTICATION is positional-only, Meta's rule not ours) plus button_params: one entry per dynamic-URL button, in button order — empty for JOB_COMPLETION, [\"token\", \"token\"] for JOB_CREATION and JOB_DELIVERY (both buttons carry the identical token), one entry for JOB_MONEY_RECEIPT and JOB_INVOICE. An approved template can't be edited — a wording change means a new _vN name and a fresh Meta review."],
                ["mobile.py",    "is_valid_mobile()/normalize_mobile() — same validation the client mirrors, shared by every event."],
                ["token.py",     "TWO independent signing pairs over the same HMAC-SHA256 scheme and helpers (_b64url/_signature). sign(db_name, schema, job_ids, ttl_days=730)/verify(token) — payload db_name|schema|job_ids|exp — backs the JOB_CREATION status page and job slip, both JOB_DELIVERY PDF buttons, and JOB_INVOICE (reused unchanged; it was written generic, never OTP-specific). sign_receipt(db_name, schema, job_id, payment_id)/verify_receipt(token) — payload db_name|schema|job_id|payment_id|exp — exists only because the first pair cannot name a single payment row, and job_ids was deliberately not overloaded with a second meaning. Neither is the delivery OTP, which is a separate secret entirely (see \u2018Paperless Job Delivery\u2019)."],
                ["otp.py",       "JOB_DELIVERY only \u2014 generate_otp() (4 digits, secrets.randbelow, never random) and the HMAC-SHA256 hash/compare used to store and check it. The code is never stored in plaintext, never logged, and never returned to the client."],
                ["sender.py",    "Every send path. The grouped events (JOB_CREATION/JOB_COMPLETION/JOB_DELIVERY): server-side re-filter (never trust the client's selection) \u2192 group by customer_contact_id \u2192 cap at MAX_JOBS_PER_WHATSAPP_MESSAGE (35) per customer, splitting into multiple messages above that \u2192 build biz_opaque_callback_data \u2192 send \u2192 _persist_attempt per job. The ungrouped ones (JOB_MONEY_RECEIPT/JOB_INVOICE) skip grouping and chunking entirely \u2014 one row in, one message out \u2014 and JOB_MONEY_RECEIPT persists through _persist_receipt_attempt instead, the array-shaped writer. _is_event_enabled() gates all five before any DB or Meta work happens. Also holds the delivery OTP lifecycle (OTP_TTL_MINUTES, OTP_MAX_ATTEMPTS, verify/manual-override)."],
            ]},
            { type: "heading", text: "Tenant + event routing: biz_opaque_callback_data, not a table" },
            { type: "para", text: "A status webhook carries only a wamid — no job, customer, tenant, or event. Meta's send API accepts an arbitrary string, biz_opaque_callback_data, echoed back verbatim on every status callback for that message. Current format is 4-part, pipe-delimited: db_name|schema|event_code|job_id,job_id,… — event_code is a 2-letter abbreviation (CC=JOB_COMPLETION, JC=JOB_CREATION, JD=JOB_DELIVERY, MR=JOB_MONEY_RECEIPT, JI=JOB_INVOICE) kept out of the full event-key vocabulary on the wire only, since every byte here subtracts from the 512-char cap available for job ids. The webhook decodes it back with _EVENT_KEY_BY_CODE before it goes near SQL, so event_key is always the full string by the time it reaches the database. A legacy 3-part payload (db_name|schema|job_ids, no event code — from before JOB_CREATION existed) is still decoded, treated as JOB_COMPLETION; both formats are accepted indefinitely since an old in-flight callback could arrive at any time." },
            ...
            { type: "para", text: "Every document a customer can reach is a plain URL button pointing back at this server — nothing is ever a WhatsApp document/media attachment. Three routers serve them, each 404ing plainly on a bad or expired token: job_intake_router.py (the no-login status page and the job-slip PDF, JOB_CREATION), job_delivery_router.py (the Delivery Note PDF and the Invoice PDF — the latter reused unchanged by JOB_INVOICE, since it verifies a token and queries by job_ids with no status filter at all), and job_money_receipt_router.py (one payment row's receipt PDF, behind verify_receipt). All PDFs are reportlab, built server-side from whitelisted public SQL — deliberately not ports of the client's jsPDF builders." },
            { type: "warning", text: "Each public prefix needs its own nginx location block in production. The SPA's catch-all intercepts anything not explicitly proxied first, which has already broken a release once — /job-intake/, /job-delivery/ and /job-money-receipt/ are all proxied today, and any future public prefix must be added there before its buttons will work outside dev." },
...

    {
        ...
        summary: "JOB_DELIVERY's two-template send, the hashed 4-digit code, batch-wide OTP writes, and the confirmation record — the one event on the WhatsApp rail with a completion state.",
        tags: ["job delivery", "paperless", "otp", "confirmation code", "verifyJobDeliveryOtp", "setJobDeliveryManualConfirmation", "sendWhatsappJobDelivery", "job_delivery_otp_v1", "job_delivery_notice_v1", "delivery note pdf", "invoice pdf", "confirmed_at", "manual_override", "whatsapp"],
        content: [
            { type: "para", text: "As-built summary of plans/plan-wa-delivery.md (all steps Done, despite the doc's stale title). Read 'WhatsApp Integration — Implementation' first — this page only covers what JOB_DELIVERY does differently. What makes it different is that it is the only event with a completion state: every other send is fire-and-forget, this one expects something back." },
            { type: "heading", text: "Two templates, one logical event" },
            { type: "para", text: "sendWhatsappJobDelivery sends twice. First job_delivery_notice_v1 (UTILITY) — job list, amount paid/balance, and two dynamic-URL buttons, 'Download Delivery Note' and 'Download Invoice', both carrying the identical token. Then job_delivery_otp_v1 (AUTHENTICATION) — the 4-digit code alone, in Meta's fixed wording, with the 'Copy Code' button that category mandates." },
            { type: "note", text: "This split is not a design preference. Meta's category classifier rejects a numeric confirmation code inside a UTILITY body no matter what surrounds it — a combined single-message v2 was submitted and refused (plan-wa-delivery.md, Step 3 v2). Don't try to merge them again." },
            { type: "warning", text: "Only the OTP send is persisted to whatsapp_notifications (event_key JOB_DELIVERY). The summary send is logged but never written there — deliberately, so a failed summary can never be misread as a missed code. The order matters: summary first, OTP second, because the OTP is the one being tracked." },
            { type: "heading", text: "The code" },
            { type: "table", headers: ["Property", "Value", "Why"], rows: [
                ["Length",      "4 digits (otp.py, secrets.randbelow — never random)",       "Read aloud at a counter; 10,000 codes is ATM-PIN order, and the lockout is what does the real work, not the keyspace"],
                ["Lifetime",    "OTP_TTL_MINUTES = 15",                                       "Long enough to find a phone, short enough that a stale code is worthless"],
                ["Lockout",     "OTP_MAX_ATTEMPTS = 5, then a resend is required",            "The actual security control given a 4-digit keyspace"],
                ["At rest",     "HMAC-SHA256 under whatsapp_delivery_otp_secret",             "Never plaintext, never logged, never returned to the client — staff cannot see the code anywhere in the app"],
                ["Resend",      "Re-run the send mutation; no separate resend mutation",      "A fresh code overwrites the hash, invalidating the old one for free"],
            ]},
            { type: "heading", text: "Batch writes: all jobs or none" },
            { type: "para", text: "A multi-job delivery is the normal case, not an edge case. One send covers every selected job with one shared code, and SET_JOB_DELIVERY_OTP writes the same otp_hash/otp_expires_at onto every job_id in one transaction via exec_sql_batch — a partial write must never leave some jobs on a new hash and others on a stale one. verifyJobDeliveryOtp then requires every job_id passed to share the same matching, unexpired hash before confirming any of them; it does not trust one representative job. A wrong guess increments otp_attempt_count on all of them together, so the 5-attempt lockout cannot drift apart within a batch." },
            { type: "note", text: "A delivery exceeding MAX_JOBS_PER_WHATSAPP_MESSAGE (35) splits into several messages, each with its own independent code. Known limitation, not handled in the UI — a handover that large is expected to use manual override instead." },
            { type: "heading", text: "The confirmation record" },
            { type: "code", language: "jsonc", text: '// job.whatsapp_notifications -> JOB_DELIVERY\n{\n  // ...the same flat ladder every event has (attempt_count, last_status, attempts[], ...)\n  "otp_hash": "...",             // HMAC-SHA256, never the code\n  "otp_expires_at": "...",\n  "otp_attempt_count": 0,        // lockout at 5\n  "confirmed_at": null,\n  "confirmation_method": null,   // "otp_verified" | "manual_override"\n  "confirmed_by_staff_id": null  // who verified or overrode\n}' },
            { type: "para", text: "confirmed_by_staff_id names the staff member who performed the verification, taken from the caller's existing authenticated session — no new auth plumbing. It is a materially better dispute record than retaining the code would be ('verified by this named employee' beats 'a correct-looking code was entered somewhere'), with no security downside since it isn't a secret." },
            { type: "warning", text: "These fields are written by dedicated queries (SET_JOB_DELIVERY_OTP, SET_JOB_DELIVERY_CONFIRMATION, INCREMENT_JOB_DELIVERY_OTP_ATTEMPT), never through SET_JOB_WHATSAPP_OUTCOME — that query's WHERE clause demands a Meta wamid and a status-ladder advance, neither of which an OTP verification has. Both SET_ queries use the same defensive jsonb_typeof(...) = 'object' guard as the shared writers, because the manual-override path can legitimately be the first write ever for this event on a job that was never messaged at all." },
            { type: "heading", text: "Manual override" },
            { type: "para", text: "setJobDeliveryManualConfirmation writes confirmed_at/confirmation_method='manual_override'/confirmed_by_staff_id with no code and no Meta send. Always available — the customer may have no valid mobile, no phone to hand, or the message may simply have failed. It exists so a real handover is never left unconfirmed just because WhatsApp didn't cooperate." },
            ...
            { type: "note", text: "GET_JOB_DELIVERY_OTP_PENDING is a single server-computed boolean per job precisely so the 'Verify Code' affordance can exist without shipping otp_hash or otp_expires_at to the browser." },
            { type: "heading", text: "Customer Connect's Job Delivery tab" },
            { type: "para", text: "GET_WHATSAPP_EVENT_LOG_PAGED carves out this one event: for JOB_DELIVERY it additionally requires confirmed_at IS NOT NULL. 'Delivery happened' means the customer confirmed, not merely that a message went out, so an unconfirmed send is not a log entry. WhatsappStatusCell also counts the confirmation itself as the success for this event rather than success_count — that counter is written only by Meta's DELIVERED webhook and measures transport to the handset, so leading every confirmed row with a raw '✓ 0' was reporting a success as a failure." },
        ],
        faqs: [
            { q: "Does confirmation gate is_closed, accounts posting, or stock?", a: "No. Delivery, invoicing and posting fire exactly as they did before; the confirmation is informational and nothing waits on it. That was a deliberate decision — making a paper-replacement flow able to block revenue posting would have been a much riskier feature." },
            { q: "Why 4 digits and not 6?", a: "It is read aloud across a counter. 10,000 codes is the same order as a bank PIN, and like a PIN its security comes from the 5-attempt lockout, not the keyspace. Six digits would have made the flow worse without changing the threat model, since anyone holding the customer's phone can read the code out either way — this proves physical presence with a staff witness, not identity." },
            { q: "Can staff look up the code to help a confused customer?", a: "No, by design. It is hashed at rest, never logged, and never returned to the client. If the customer can't produce it, resend or use manual override." },
            { q: "Why is there a separate secret for the OTP instead of reusing token.py's?", a: "Different lifetime and blast radius. token.py signs public document links that must stay valid for ~2 years; the OTP secret protects a 15-minute proof-of-handover. Rotating one should never invalidate the other." },
            { q: "A delivery is confirmed but doesn't appear in the Job Delivery log tab.", a: "Check confirmed_at is actually set — the tab filters on it. A send with no confirmation is intentionally absent; that tab is the record of collections, not of sends." },
        ],
    },

    {
        ...
        summary: "The two ungrouped, fire-and-forget events — why JOB_MONEY_RECEIPT needed its own array-shaped SQL, its own token pair and its own Customer Connect stack, while JOB_INVOICE needed none of it.",
        tags: ["money receipt", "job invoice", "sendWhatsappMoneyReceipt", "sendWhatsappJobInvoice", "job_money_receipt_v1", "job_invoice_v1", "sign_receipt", "verify_receipt", "jsonb array", "SET_JOB_MONEY_RECEIPT_WHATSAPP_ATTEMPT", "MoneyReceiptLogGrid", "delivered jobs", "whatsapp"],
        content: [
            ...
            { type: "table", headers: ["", "JOB_MONEY_RECEIPT", "JOB_INVOICE"], rows: [
                ["Scope",            "One job_payment row",                          "One job (one invoice)"],
                ["Trigger",          "receipts-section.tsx row actions",             "delivered-jobs-grid.tsx row actions, gated on row.invoice_no"],
                ["Template",         "job_money_receipt_v1 (UTILITY, 1 button)",     "job_invoice_v1 (UTILITY, 1 button)"],
                ["Token",            "sign_receipt/verify_receipt — NEW pair",       "token.py's original sign/verify — reused unchanged"],
                ["PDF route",        "job_money_receipt_router.py — NEW",            "GET /job-delivery/invoice/{token} — reused unchanged, no new route"],
                ["jsonb value",      "ARRAY, one element per payment_id",            "flat object, like JOB_CREATION/COMPLETION"],
                ["Write query",      "SET_JOB_MONEY_RECEIPT_WHATSAPP_ATTEMPT — NEW", "SET_JOB_WHATSAPP_ATTEMPT — reused unchanged"],
                ["Outcome webhook",  "Never applies (see below) — send status only",  "Applies normally, full ladder"],
                ["Customer Connect", "New sibling stack: MoneyReceiptLogSection/Grid + 2 new SQL", "WhatsappLogSection/Grid, eventKey widened — zero new components"],
            ]},
            { type: "heading", text: "Why the array, and what it costs" },
            { type: "para", text: "A job has one invoice but can have many payments, each needing its own independent attempt ladder. Encoding that as {\"<payment_id>\": {...}} would put a row id into a JSON key path; an array keeps payment_id as ordinary data inside each element and matches how this codebase already shapes per-row jsonb collections. The cost is that nothing generic works on it." },
            { type: "bullets", items: [
            ...
            { type: "para", text: "token.py's original sign(db_name, schema, job_ids) cannot name a single payment row. sign_receipt(db_name, schema, job_id, payment_id)/verify_receipt use the same HMAC-SHA256 scheme and the same _b64url/_signature helpers, with their own payload layout (db_name|schema|job_id|payment_id|exp). Overloading job_ids with a second meaning was rejected deliberately — a token that sometimes means 'these jobs' and sometimes 'this payment' is a decoding ambiguity waiting to become a security bug." },
            { type: "heading", text: "Where each can be triggered — and where it can't" },
            { type: "para", text: "Sending only ever happens from the screen that owns the record: receipts from the Receipts grid, invoices from Delivered Jobs. Customer Connect's Money Receipt and Invoice tabs are read-only logs; viewing is not a second way to send. The Invoice send lives on Delivered Jobs specifically because GET_DELIVERED_JOBS_PAGED (is_closed = true) is the only query that still lists a job after closure — delivery-modal.tsx is fed by a query filtered to is_closed = false, so a closed job can never be reopened there, which is exactly the gap this event was built to close." },
            { type: "warning", text: "job_money_receipt_router.py needs its own nginx location block in production, same as /job-intake/ and /job-delivery/. The SPA catch-all swallows any public prefix that isn't explicitly proxied, and that has already broken a release once." },
        ],
        faqs: [
            { q: "Can one send cover all of a job's receipts?", a: "No — one send is one job_payment row, matching the Receipts grid's own per-row PDF scope. If the customer wants the full picture, send the invoice instead: that PDF already lists every payment received against the job." },
            { q: "Why does the Money Receipt tab show the same job several times?", a: "It is one row per receipt send, produced by a lateral join over the array, not one row per job. That is the whole reason it couldn't reuse the generic log stack." },
            { q: "Does sending an invoice regenerate it or consume a number?", a: "No. It mints a token for the existing job and hands it to the existing invoice route. Nothing is regenerated, no document sequence advances." },
            { q: "Why is there no access-right guard on either mutation?", a: "Same precedent as every other send on this rail: the hosting screen's own right already gates the entry point. No new right was seeded, and that decision was reused rather than re-litigated per event." },
            { q: "Could JOB_INVOICE have reused JOB_DELIVERY's message instead of getting its own template?", a: "No — an approved Meta template can't be edited, and job_delivery_notice_v1 carries two buttons plus delivery-specific wording and a companion OTP send. job_invoice_v1 is a plain one-button Utility message with no code in it, which is also why it needed none of JOB_DELIVERY's category-classifier trouble." },
        ],
    },
