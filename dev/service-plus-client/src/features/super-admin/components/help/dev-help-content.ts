...
export const DEV_HELP_ARTICLES: HelpArticle[] = [
...

    {
        ...
        category: "Database & Schema",
        title: "Generated TypeScript Types (pg-to-ts)",
        summary: "How the three DB schemas become typed TypeScript — and how to regenerate them.",
        tags: ["pg-to-ts", "types", "codegen", "db-schema-service.ts", "db-schema-security.ts", "db-schema-client.ts"],
        content: [
            { type: "para", text: "Rather than hand-maintaining TypeScript types for every table, the client generates them directly from Postgres using pg-to-ts." },
            { type: "table", headers: ["Generated file", "Source schema"], rows: [
                ["src/types/db-schema-service.ts", "demo1 (business) schema, from service_plus_service DB"],
                ["src/types/db-schema-security.ts", "security schema"],
                ["src/types/db-schema-client.ts", "public schema of the registry DB (service_plus_client)"],
            ]},
            { type: "heading", text: "Regenerating" },
            { type: "para", text: "Uses pg-to-ts + dotenv-cli + cross-var + cross-env because three different schemas across two different databases need three different connection strings, driven by an .env file with SERVICE_PLUS_SERVICE and SERVICE_PLUS_CLIENT connection strings." },
            { type: "steps", items: [
                "pnpm install -D pg-to-ts dotenv-cli cross-var cross-env (already in devDependencies once set up).",
                "Set SERVICE_PLUS_SERVICE and SERVICE_PLUS_CLIENT in .env (postgresql://user:pwd@host:port/dbname).",
                "Run pnpm gen-types-all — runs gen-types-service, gen-types-security, and gen-types-client in sequence.",
            ]},
            { type: "warning", text: "Run this after any DDL change to the tenant or registry database, or the generated types silently drift from the real schema — GraphQL query/mutation typing per house convention should always reference these generated types, not hand-written ones." },
        ],
        faqs: [
            ...
            { q: "What do I do after adding a new column to a table?", a: "Update the DDL, then re-run pnpm gen-types-all before writing any client code that touches the new column." },
        ],
    },
...

    {
        ...
        content: [
            { type: "table", headers: ["Issue", "Fix"], rows: [
                ["Client TS types out of sync after a DB migration", "Re-run pnpm gen-types-all — see 'Generated TypeScript Types'"],
