...
export const DEV_HELP_ARTICLES: HelpArticle[] = [
...

    {
        ...
        summary: "One send point (Customer Connect), Meta's own delivery webhook for tracking, tenant routing via biz_opaque_callback_data, no PDF/document path.",
        tags: ["whatsapp", "whatsapp integration", "whatsapp_notifications", "sendWhatsappCompletion", "biz_opaque_callback_data", "customer connect", "meta cloud api", "webhook", "jsonb_set"],
        content: [
            { type: "para", text: "This article is the as-built implementation summary. Direct Meta WhatsApp Cloud API, one shared phone_number_id across all tenants, no BSP intermediary and no provider-registry abstraction — there is exactly one provider to abstract over. Sends are fire-and-forget accept calls; the outcome that actually matters (delivery) arrives later, asynchronously, via Meta's status webhook — this is the one send path this codebase has now. The earlier synchronous accepted-means-success model, and the three PDF-carrying send points that used it, have been removed entirely." },
            ...
            { type: "table", headers: ["File", "Role"], rows: [
                ["client.py",    "send_template() only — the single call that touches Meta's HTTP API. upload_media() and any document/header branch are gone; there is no PDF template any more."],
                ["templates.py", "TEMPLATES holds exactly one entry: JOB_COMPLETION, currently name=job_completed_ready_for_pickup_v2 (an approved template can't be edited — a wording change means a new _vN name and a fresh Meta review, not an in-place edit). TemplateSpec carries header_params and body_params (renamed from the old positional params) — the template uses Meta's named-parameter form (parameter_name per value), so a mismatch between param name and slot is impossible by construction, unlike the old positional array. Gotcha: business_unit appears in both header_params and body_params — Meta's approved _v2 wording references the BU name again in the body's closing line, so the same value has to be sent as two separate named params, one per component; body's copy is untruncated (1024-char budget) unlike the header's truncated one (60-char budget)."],
                ["mobile.py",    "is_valid_mobile()/normalize_mobile() — same validation the client mirrors."],
                ["sender.py",    "The send sequence: server-side re-filter (never trust the client's selection) → group by customer_contact_id → cap at 35 jobs/customer, splitting into multiple messages above that → set biz_opaque_callback_data → persist wamid + ACCEPTED on 200, FAILED on error."],
            ]},
            { type: "heading", text: "Tenant routing: biz_opaque_callback_data, not a table" },
            { type: "para", text: "A status webhook carries only a wamid — no job, customer, or tenant. Meta's send API accepts an arbitrary string, biz_opaque_callback_data, echoed back verbatim on every status callback for that message, confirmed empirically to survive on template sends (not just free-form messages, which is all Meta's docs describe). Format is locked as pipe-delimited: db_name|schema|job_id,job_id,… — not JSON, since both ends of the string are this codebase's own code and every byte saved is a byte of the 512-char cap available for more job ids. This removes the need for any routing table or lookup — the callback already knows its tenant and its jobs." },
            ...
            { type: "table", headers: ["Endpoint", "Role"], rows: [
                ...
                ["POST /api/webhooks/whatsapp", "The real traffic. Verifies X-Hub-Signature-256 (HMAC-SHA256 of the raw body with the App Secret) before trusting anything in the payload — biz_opaque_callback_data is attacker-controlled data otherwise. Parses entry[].changes[].value.statuses[] (Meta batches several per request), decodes biz_opaque_callback_data into db_name/schema/job_ids, applies the status ladder, and always returns 200 — even for an unrecognized wamid — since a 500 just buys an infinite Meta retry loop."],
            ]},
            { type: "heading", text: "Live push to the client: whatsappDeliveryStatus subscription" },
            { type: "para", text: "Customer Connect doesn't poll for delivery outcomes — the webhook handler publishes to app/graphql/pubsub.py's in-memory PubSub (the same one accountsPostingProgress already used for its branch-scoped progress stream) right after a status update actually applies. The new whatsappDeliveryStatus(db_name: String!) subscription (app/graphql/resolvers/subscription.py) filters server-side by db_name and yields {db_name, job_id, status, error}. Client-side (customer-connect-section.tsx), the subscription is mounted once for the screen's whole lifetime, not re-created per dispatch — every event patches the matching grid row if that job_id is on the current page and unchecks its checkbox live, with no job_id allowlist gating that. A separate, narrower job_id set (trackingJobIdsRef) is only used to roll counts into the post-dispatch banner, and that bookkeeping alone stops after ~2 minutes or once its batch settles — the grid keeps updating past that point regardless, which matters since Read in particular can arrive long after any reasonable dispatch-tracking window (it depends on the customer actually opening the message). Reusing existing graphql-ws infra instead of a client-side setInterval poll was a mid-build call." },
            { type: "heading", text: "Status ladder — never moves backwards, and gated on wamid" },
            { type: "para", text: "PENDING(0) < ACCEPTED(1) < SENT(2) < DELIVERED(3) < READ(4), FAILED(9) terminal. A callback ranked at or below the currently stored value is a no-op (200, no write) — Meta reorders and retries callbacks, so out-of-order delivery is expected, not a bug to fix upstream. SET_JOB_WHATSAPP_OUTCOME's WHERE clause also requires the callback's wamid to match the job's current last_wamid, not just the rank check — without that, a resend (which gets a fresh wamid) could still have a late/duplicate callback for its superseded wamid land afterward and clobber the new attempt's outcome, since FAILED outranks everything on the ladder alone." },
            { type: "heading", text: "job.whatsapp_notifications — written from two places, single-level jsonb_set only" },
            { type: "para", text: "One key, JOB_COMPLETION, replaces the old four-key shape (JOB_CREATION/COMPLETION/DELIVERY/RECEIPT) now that only completion is sent. sender.py records the attempt at send time (attempt_count, last_wamid, status ACCEPTED); the webhook settles the outcome later (success_count/fail_count, last_status, last_error) once Meta actually confirms delivery or failure. The counter increment is SQL-side jsonb_set (UPDATE ... SET whatsapp_notifications = jsonb_set(...) WHERE id = ...), not a Python read-modify-write — the old build_notification_update_args() approach was racy the moment a webhook and a send could touch the same job concurrently." },
            { type: "note", text: "jsonb_set cannot auto-vivify a path more than one level deep — jsonb_set('{}', '{JOB_COMPLETION,attempt_count}', ..., true) silently leaves the value unchanged instead of creating JOB_COMPLETION, because all earlier path steps must already exist. Both SET_JOB_WHATSAPP_ATTEMPT and SET_JOB_WHATSAPP_OUTCOME build JOB_COMPLETION as its own isolated object first (single-level jsonb_set calls only, reading the prior value via a jsonb_typeof(...) = 'object' guard that also self-heals any corrupted/non-object legacy value instead of erroring on it), then attach it with one single-level jsonb_set at the end. A version of this that instead threaded '{JOB_COMPLETION,<field>}' multi-level paths through the chain went unnoticed for a while — it silently no-ops on a job's first-ever send, and a separate variant using a top-level || merge actively corrupted existing data into an array shape. If completion messages ever stop persisting again, check this convention first before assuming the bug is elsewhere." },
            { type: "heading", text: "Access right" },
            { type: "para", text: "JOBS_CUSTOMER_CONNECT in seed_security_data.py's ACCESS_RIGHT_SEED_SQL, granted to MANAGER and RECEPTIONIST (not TECHNICIAN) — gates the Customer Connect menu item, the only surviving WhatsApp entry point. The sendWhatsappCompletion resolver itself carries no require_access_right guard of its own; gating is client-side, via the menu entry, same as before." },
            { type: "heading", text: "What was removed" },
            { type: "table", headers: ["Removed", "Reason"], rows: [
                ["app/routers/notifications/whatsapp_router.py", "The PDF-carrying REST endpoint (job creation/delivery/receipt sends) — no document path survives."],
                ["app/notifications/whatsapp_client.py / _helpers.py / _templates.py", "Superseded by app/whatsapp/{client,mobile,templates,sender}.py."],
                ["app/graphql/resolvers/jobs/whatsapp.py", "Rewritten inside app/whatsapp/sender.py."],
                ["src/lib/whatsapp-service.ts, src/features/client/components/jobs/use-whatsapp-send.ts", "The REST fetch wrapper and shared hook for the four PDF-carrying buttons — deleted outright, no callers were left."],
                ["GET_JOBS_FOR_WHATSAPP_SEND (sql_jobs.py)", "Its only caller was the deleted REST router."],
            ]},
            { type: "note", text: "job.whatsapp_notifications keeps its jsonb column; the three dead event keys (JOB_CREATION/DELIVERY/RECEIPT) may linger in existing rows from before this change — harmless, and not worth a cleanup migration. Nothing writes them any more." },
        ],
        faqs: [
            ...
            { q: "Why does sendWhatsappCompletion re-filter to COMPLETED_OK + is_final instead of trusting the job_ids the client sent?", a: "The client — especially Customer Connect's multi-select — is never trusted for authorization-relevant filtering. The resolver re-loads the jobs and re-checks eligibility itself before grouping and sending." },
            { q: "Is there an audit trail of every individual WhatsApp send attempt?", a: "No — only the latest state per job in job.whatsapp_notifications['JOB_COMPLETION']. attempt_count/success_count/fail_count are running counters, not a per-attempt log." },
            { q: "How does a status callback find its tenant database without a routing table?", a: "biz_opaque_callback_data, set at send time to db_name|schema|job_ids, is echoed back by Meta on every status callback for that message — decode it, no lookup needed. Confirmed empirically against live Meta traffic." },
            ...
            { q: "What happens if a customer has more than 35 eligible jobs?", a: "sender.py splits them into multiple messages, each independently grouped, sent, and tracked — same as an unrelated customer's message would be. 35 is the cap that keeps biz_opaque_callback_data under Meta's 512-char limit with real margin." },
        ],
    },
