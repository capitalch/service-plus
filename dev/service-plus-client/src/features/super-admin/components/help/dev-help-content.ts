...

    {
        ...
        content: [
            ...
            { type: "table", headers: ["Location", "Form"], rows: [
                ...
                ["jobs/charge-cost-rule.ts — the one client-side copy", "SPARE_CHARGE_PATTERN = /(spare|parts)/i, exposed as chargeNeedsCost()"],
            ]},
            { type: "para", text: "Three client sites consume charge-cost-rule.ts and none of them restate the regex: cost-correction-helpers.ts (needsCost / isMissingCost), finalize-job-save.ts (the zeroCostCharges filter that blocks Finalize) and final-job-form.tsx (isChargeCostMissing — the red Cost cell). The Cost column on the Additional Charges table deliberately carries no red asterisk, because the column is not mandatory on every row." },
            { type: "warning", text: "Change one, change both — they define the same 'this charge needs a cost' rule, and a drift shows up as a badge count that disagrees with what the editor flags. Until this was fixed, finalize-job-save.ts held a fourth, stricter rule of its own (cost > 0 on every named charge), which rejected zero-cost labour charges the badge and the Correct Costs modal were happy with." },
