...
import type { DbSizeType, HealthStatusType, ServiceCheckType, UsageHealthType } from "@/features/super-admin/types";
					...
					<div className="flex items-center gap-3">
						{data && (
							<Badge className={overallBadge[overall]} variant="outline">
								<span
									className={`mr-1.5 inline-block h-1.5 w-1.5 rounded-full ${statusDot[overall]}`}
								/>
							...
							<Card className="border border-slate-200/80 shadow-sm">
								<CardContent className="p-6">
									...
									<p className="mb-4 text-xs text-slate-400">
										Size of all service_plus_* databases on the server
									</p>
								...
								<CardContent className="p-6">
									...
									<div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
										{[
											...
											{
												label: "Debug Mode",
												value: data.server_info.debug ? "Enabled" : "Disabled",
											},
										].map(({ label, value }) => (
											<div className="flex items-center justify-between gap-2" key={label}>
												<span className="text-xs text-slate-500">{label}</span>
												<span
													className={`text-xs font-medium ${value === "Enabled" ? "text-emerald-600" : value === "Disabled" ? "text-slate-400" : "text-slate-700"}`}
												>
													{value}
												</span>
											</div>
