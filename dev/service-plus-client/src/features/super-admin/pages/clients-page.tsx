...
export const ClientsPage = () => {
	...
	const [mailAdminCredentials, setMailAdminCredentials] = useState<{
		admin: ClientAdminType;
		client: ClientType;
	} | null>(null);
	...
	const { data, error, loading, refetch } = useQuery<ClientsPageDataType>(GRAPHQL_MAP.superAdminClientsData, {
	...
	const displayList = useMemo(() => {
		...
		return sorted.filter(
			(c) =>
				...
				c.admins?.some((a) => a.full_name.toLowerCase().includes(q) || a.email.toLowerCase().includes(q)),
		);
	}, [clients, search, sort]);
	...
	const handleMailAdminCredentials = (admin: ClientAdminType, client: ClientType) =>
		setMailAdminCredentials({ admin, client });
	...
	const statCards = [
		...
		},
		{
			...
			active: {
				...
				iconBg: "bg-teal-100",
				label: "Active",
				value: clientStats?.activeAdmins ?? 0,
			},
			custom: 1,
			icon: ShieldIcon,
			...
			inactive: {
				...
				iconBg: "bg-slate-100",
				label: "Inactive",
				value: clientStats?.inactiveAdmins ?? 0,
			},
			label: "Admins",
			total: {
				accent: "text-violet-600",
				...
				iconBg: "bg-violet-100",
				label: "Total",
				value: clientStats?.totalAdmins ?? 0,
			},
		},
	];
		...
		<SuperAdminLayout>
										...
										<Card className="flex h-full flex-col border border-slate-200/80 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
											<CardContent className="p-3">
												...
												<div className="mb-3 flex items-center gap-1.5">
													<div
														className={`flex h-5 w-5 items-center justify-center rounded ${card.iconBg}`}
													>
												...
												<div className="flex flex-col gap-1">
													<div className="flex items-center justify-between">
														<div className="flex items-center gap-1.5">
															<div
																className={`flex h-4 w-4 items-center justify-center rounded ${card.total.iconBg}`}
															>
																<TotalIcon
																	className={`h-2.5 w-2.5 ${card.total.accent}`}
																/>
															</div>
															<span className="text-[11px] text-slate-500">Total</span>
														</div>
														<span className={`text-base font-bold ${card.total.accent}`}>
															{card.total.value}
														</span>
													</div>
													<div className="flex items-center justify-between">
														<div className="flex items-center gap-1.5">
															<div
																className={`flex h-4 w-4 items-center justify-center rounded ${card.active.iconBg}`}
															>
																<ActiveIcon
																	className={`h-2.5 w-2.5 ${card.active.accent}`}
																/>
															</div>
															<span className="text-[11px] text-slate-500">Active</span>
														</div>
														<span className={`text-base font-bold ${card.active.accent}`}>
															{card.active.value}
														</span>
													</div>
													<div className="flex items-center justify-between">
														<div className="flex items-center gap-1.5">
															<div
																className={`flex h-4 w-4 items-center justify-center rounded ${card.inactive.iconBg}`}
															>
																<InactiveIcon
																	className={`h-2.5 w-2.5 ${card.inactive.accent}`}
																/>
															</div>
															<span className="text-[11px] text-slate-500">Inactive</span>
														</div>
														<span className={`text-base font-bold ${card.inactive.accent}`}>
															{card.inactive.value}
														</span>
													</div>
												</div>
									...
									<CardContent className="p-3">
										...
										<div className="flex items-center justify-between">
											<span className="text-base font-bold text-orange-600">
												{clientStats?.orphanDatabaseCount ?? 0}{" "}
												<span className="text-[11px] font-normal text-slate-500">unlinked</span>
											</span>
											<Button
					...
					{loading && clients.length === 0 ? (
						...
						<div className="flex flex-col gap-3">
							{displayList.map((client, idx) => {
								...
								const canInitialize =
									client.is_active &&
									(!client.db_name || (client.db_name_valid && client.activeAdminCount === 0));
								...
								return (
									...
									>
										<Card
											className={`overflow-hidden bg-white shadow-sm transition-shadow hover:shadow-md ${client.is_active ? "border border-slate-200" : "border border-l-4 border-slate-200 border-l-slate-400"}`}
										>
											{/* Card header */}
											<div
												className={`flex items-center gap-3 px-4 py-3.5 ${!client.is_active ? "bg-slate-50/40" : ""}`}
											>
												{/* Index */}
												<span className="w-5 flex-shrink-0 text-center text-[11px] font-medium text-slate-300">
													{idx + 1}
												</span>

												{/* Avatar */}
												<div
													className={`relative flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl text-base font-bold shadow-sm ${client.is_active ? "bg-gradient-to-br from-emerald-400 to-teal-500 text-white" : "bg-gradient-to-br from-slate-300 to-slate-400 text-white"}`}
												>
												...
												>
													...
													<div className="flex flex-wrap items-center gap-x-2 gap-y-1">
														<span
															className={`text-sm font-semibold ${client.is_active ? "text-slate-900" : "text-slate-400 line-through decoration-slate-300"}`}
														>
															{client.name}
														</span>
														...
														>
															<span
																className={`mr-1 h-1.5 w-1.5 rounded-full ${client.is_active ? "bg-emerald-500" : "bg-slate-400"}`}
															/>
															{client.is_active ? "Active" : "Inactive"}
														</Badge>
														{client.db_name &&
															(client.db_name_valid ? (
																<span title="Database ready">
																	<DatabaseIcon className="h-3.5 w-3.5 text-slate-600" />
																</span>
															) : (
																<span title="Database not found">
													...
													<div className="mt-2 flex flex-wrap items-center gap-2">
															...
															{client.inactiveAdminCount > 0 && (
																<>
																	...
																	<span className="text-slate-500">
																		{client.inactiveAdminCount} inactive
																	</span>
																</>
															)}
														...
														<span className="inline-flex items-center gap-1.5 rounded-full border border-indigo-200 bg-indigo-50 px-2.5 py-0.5 text-xs font-semibold text-indigo-700">
															...
															{client.inactiveBuCount > 0 && (
																<>
																	...
																	<span className="text-slate-500">
																		{client.inactiveBuCount} inactive
																	</span>
																</>
															)}
														...
														<span className="hidden text-xs font-medium text-slate-400 lg:inline">
															· Upd. {formatDate(client.updated_at)}
														</span>
																	...
																	{client.admins.map((admin, aIdx) => (
																		...
																		>
																			...
																			<TableCell className="font-medium text-slate-900">
																				<div className="flex items-center gap-2">
																					<div
																						className={`flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold ${admin.is_active ? "bg-cyan-100 text-cyan-700" : "bg-slate-200 text-slate-400"}`}
																					>
																						{admin.full_name
																							.charAt(0)
																							.toUpperCase()}
																					...
																					<div>
																						<div
																							className={`text-sm ${admin.is_active ? "" : "text-slate-400 line-through decoration-slate-300"}`}
																						>
																							{admin.full_name}
																						</div>
																			...
																			<TableCell
																				className={`hidden text-sm sm:table-cell ${admin.is_active ? "text-slate-600" : "text-slate-400"}`}
																			>
																				{admin.email}
																			</TableCell>
																			<TableCell
																				className={`hidden text-sm md:table-cell ${admin.is_active ? "text-slate-500" : "text-slate-400"}`}
																			>
																				{admin.mobile ?? "—"}
																			</TableCell>
																			<TableCell
																				className={`hidden text-xs lg:table-cell ${admin.is_active ? "text-slate-500" : "text-slate-400"}`}
																			>
			...
			{createAdminClient && (
				<CreateAdminDialog
					...
					onOpenChange={(open) => {
						if (!open) setCreateAdminClient(null);
					}}
			...
			{initializeClient && (
				<InitializeClientDialog
					...
					onOpenChange={(open) => {
						if (!open) setInitializeClient(null);
					}}
			...
			<AttachDbDialog
				...
				open={!!attachDbClient}
				onOpenChange={(open) => {
					if (!open) setAttachDbClient(null);
				}}
				onSuccess={handleRefetch}
			/>
			<DeleteClientDialog
			...
			<DetachDbDialog
				...
				open={!!detachDbClient}
				onOpenChange={(open) => {
					if (!open) setDetachDbClient(null);
				}}
				onSuccess={handleRefetch}
			/>
			<DeactivateClientDialog
				...
				onOpenChange={(open) => {
					if (!open) setDeactivateClient(null);
				}}
				onSuccess={handleRefetch}
			/>
			<ActivateClientDialog
				...
				open={!!activateClient}
				onOpenChange={(open) => {
					if (!open) setActivateClient(null);
				}}
				onSuccess={handleRefetch}
			/>
			...
			{deactivateAdmin && (
				<DeactivateAdminDialog
					...
					onOpenChange={(open) => {
						if (!open) setDeactivateAdmin(null);
					}}
					onSuccess={handleRefetch}
				/>
			)}
			{mailAdminCredentials && (
				<MailAdminCredentialsDialog
					...
					onOpenChange={(open) => {
						if (!open) setMailAdminCredentials(null);
					}}
					onSuccess={handleRefetch}
				/>
			)}
