...
export function SearchableCombobox<T>({
	...
	const resolveId = (item: T) => (getIdentifier ? getIdentifier(item) : getFilterKey(item).split(" ")[0]);
	...
	return (
		<div className={`space-y-2 ${className ?? ""}`}>
			<Label className="text-xs font-extrabold text-(--cl-text) uppercase tracking-widest">{label}</Label>
				...
				<PopoverAnchor asChild>
					<div ref={anchorRef} className="relative group">
						{showDisplay ? (
							...
							>
								<div className="flex-1 min-w-0">{renderSelectedDisplay!(selectedItem!)}</div>
								<button
									type="button"
									tabIndex={-1}
					...
					{isLoading ? (
						...
						<>
							<div className="sticky top-0 z-10 flex items-center justify-between border-b border-(--cl-border) bg-white/95 px-3 py-1 backdrop-blur-sm select-none">
								...
								<span className="text-[10px] font-semibold text-(--cl-text-muted)">
									{filtered.length}
								</span>
							</div>
