...
function AlertDialogPortal({ container, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Portal>) {
	// See DialogPortal in dialog.tsx — same .client-theme CSS-variable scoping issue.
	const themeRoot = usePortalContainer();
	return (
		<AlertDialogPrimitive.Portal
			data-slot="alert-dialog-portal"
			container={container ?? themeRoot ?? undefined}
			{...props}
		/>
	);
}
	...
	return (
		<AlertDialogPortal>
			...
			<AlertDialogPrimitive.Content
				...
				data-size={size}
				className={cn(
					"group/alert-dialog-content fixed top-1/2 left-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-4 rounded-2xl bg-popover p-4 text-popover-foreground ring-1 ring-foreground/10 shadow-[0_2px_24px_-12px_rgba(0,0,0,0.35)] duration-150 outline-none data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
					className,
				)}
				{...props}
			/>
