	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent
				...
				overlayStyle={{ backgroundColor: "rgba(0,0,0,0.5)", backdropFilter: "none" }}
			>
				<DialogHeader>
					<DialogTitle>{title}</DialogTitle>
					<DialogDescription>
						Permanently delete <span className="font-semibold text-foreground">{entityName}</span>?
					</DialogDescription>
				</DialogHeader>
					...
					{!isBlocked && (
						<div className="flex flex-col gap-1.5">
							<Label htmlFor="delete_confirm_input">
								...
								<span
									className={`font-semibold text-foreground${confirmKeyClass ? ` ${confirmKeyClass}` : ""}`}
								>
									{confirmKey}
								</span>{" "}
								to confirm
