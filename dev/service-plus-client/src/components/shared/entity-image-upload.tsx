			...
			<div className="flex flex-col gap-3">
				{!readOnly && (
					...
					>
						<input {...getInputProps()} />
						<CloudUpload
							className={`h-5 w-5 shrink-0 ${isDragActive ? "text-(--cl-accent)" : "text-(--cl-text-muted) group-hover:text-(--cl-accent)"}`}
						/>
						<div className="text-left">
							...
							<p className="mt-0.5 text-[10px] text-(--cl-text-muted)">
								JPEG · PNG · WEBP — max {maxSizeKb}KB per file
							</p>
						</div>
					</div>
				)}
