						...
						<li key={i} className="flex gap-3">
							...
							<div className="flex flex-col items-center">
								...
								{i < block.items.length - 1 && (
									<span
										className={cn(
											"mt-1 w-0.5 flex-1 min-h-[12px] rounded-full opacity-30",
											s.stepBg,
										)}
									/>
								)}
							...
							<p
								className={cn(
									"pb-4 pt-0.5 text-[13px] leading-relaxed text-(--cl-text-muted)",
									i === block.items.length - 1 && "pb-1",
								)}
							>
								{item}
							</p>
							...
							<tr className={cn("border-b border-(--cl-border)")}>
								{block.headers.map((h, i) => (
									<th
										key={i}
										className={cn(
											"px-3 py-2.5 text-left text-[11px] font-bold uppercase tracking-wider whitespace-nowrap",
											s.pillText,
											s.pill,
										)}
									>
						...
						<tbody>
							{block.rows.map((row, ri) => (
								<tr
									key={ri}
									className={cn(
										"border-b border-(--cl-border)/50 last:border-0 transition-colors hover:bg-(--cl-hover)",
										ri % 2 === 0 ? "bg-(--cl-bg)" : "bg-(--cl-surface)",
									)}
								>
									{row.map((cell, ci) => (
										<td key={ci} className="px-3 py-2.5 text-(--cl-text-muted) align-top">
											{cell}
										</td>
									))}
								</tr>
	...
	return (
		<div className="border-b border-(--cl-border)/70 last:border-0">
			...
			>
				<span
					className={cn(
						"mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-black transition-transform",
						s.pill,
						s.pillText,
						open && "rotate-45",
					)}
			...
			<div className="flex-1 overflow-y-auto px-5 py-5">
								...
								return (
									...
									>
										...
										<div className="min-w-0">
											<p className="text-[12px] font-semibold text-(--cl-text) group-hover:text-(--cl-accent) transition-colors truncate">
												{r.title}
											</p>
				...
				<div className="mt-8 mb-2 text-center">
					<button
						onClick={onBack}
						className={cn(
							"cursor-pointer inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 text-[12px] font-semibold transition-colors",
							s.pill,
							s.pillText,
							"hover:opacity-80",
						)}
			...
			<div className="flex-1 overflow-y-auto py-2">
				{categoryArticles.map((article, i) => (
					...
					>
						...
						<div className="min-w-0">
							<p className="text-[13px] font-semibold text-(--cl-text) group-hover:text-(--cl-accent) transition-colors">
								{article.title}
							</p>
							<p className="mt-0.5 text-[11px] text-(--cl-text-muted) line-clamp-2 leading-relaxed">
								{article.summary}
							</p>
						</div>
...
function highlightMatch(text: string, query: string): React.ReactNode {
	...
	return (
		<>
			{text.slice(0, idx)}
			<mark className="bg-yellow-200/70 text-inherit rounded-sm px-0.5 dark:bg-yellow-500/30">
	...
	if (results.length === 0 && query.trim()) {
		...
		return (
			<div className="h-full overflow-y-auto px-5 py-5">
				...
				{popular.length > 0 && (
					<div>
						<p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-(--cl-text-muted)/70">
							Popular articles
						</p>
						<div className="space-y-1.5">
							{popular.map((article) => {
				...
				return (
					...
					>
						...
						<div className="min-w-0">
							...
							<p className="mt-1 text-[13px] font-semibold text-(--cl-text) group-hover:text-(--cl-accent) transition-colors">
								{highlightMatch(a.title, query)}
							</p>
							<p className="mt-0.5 text-[11px] text-(--cl-text-muted) line-clamp-2 leading-relaxed">
								{highlightMatch(a.summary, query)}
							</p>
						</div>
	...
	return (
		<div className="h-full overflow-y-auto">
			...
			<div className="px-4 pt-4 pb-3">
				<p className="mb-3 text-[11px] font-bold uppercase tracking-wider text-(--cl-text-muted)/70">
					Browse by topic
				</p>
			...
			{popular.length > 0 && (
				<div className="px-4 pb-4">
					<div className="mb-3 flex items-center gap-2">
						<BookOpen className="h-3.5 w-3.5 text-sky-600" />
						<p className="text-[11px] font-bold uppercase tracking-wider text-(--cl-text-muted)/70">
							Popular articles
						</p>
					</div>
					<div className="space-y-1.5">
							...
							return (
								...
								>
									...
									<div className="min-w-0">
										...
										<p className="text-[11px] text-(--cl-text-muted) truncate">
											{article.category}
										</p>
									</div>
...
export function HelpPanel({
	...
	const breadcrumb =
		view.kind === "home"
			? null
			: view.kind === "category"
				? [{ label: "Home", onClick: handleBack }, { label: view.category }]
				: view.kind === "article"
					? [
							{ label: "Home", onClick: () => setView({ kind: "home" }) },
							{
								label: view.article.category,
								onClick: () => setView({ kind: "category", category: view.article.category }),
							},
							{ label: view.article.title },
						]
	...
	return (
		<>
			...
			<div
				{...themeProps}
				className={cn(themeProps.className, "fixed inset-0 z-[60] bg-black/50 backdrop-blur-[2px]")}
				onClick={onClose}
			/>
			...
			<div
				...
				className={cn(
					...
					"fixed right-0 top-0 z-[61] flex h-full w-full max-w-[520px] flex-col shadow-2xl animate-in slide-in-from-right duration-200",
				)}
			>
				{/* ── Panel header ── */}
				<div
								...
								<span key={i} className="flex items-center gap-1">
									...
									{crumb.onClick ? (
										<button
											...
											className="cursor-pointer font-medium hover:text-(--cl-accent) transition-colors"
										>
											{crumb.label}
										</button>
									) : (
