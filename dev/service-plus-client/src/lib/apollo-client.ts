...
function isTokenExpiredError(error: unknown): boolean {
	return CombinedGraphQLErrors.is(error) && error.errors.some((err) => err.extensions?.code === "TOKEN_EXPIRED");
}
...
const errorLink = new ErrorLink(({ error, operation, forward }) => {
	const isUnauthorized =
		(!CombinedGraphQLErrors.is(error) && (error as ServerError)?.statusCode === 401) || isTokenExpiredError(error);
