# Undo Last Transaction — Implementation Plan
...
### 3. `app/graphql/resolvers/mutation.py`
...
### 1. `src/constants/graphql-map.ts`
...
### 2. SQL — `GET_UPDATE_JOBS_PAGED`

Add `transaction_count` to the paged query result so the client can suppress "Undo" for single-transaction RECEIVED jobs. The SQL should include:
...
#### 3c. Helper — should show Undo
```ts
function canUndo(row: OpenJobRow): boolean {
    // Blocked for RECEIVED (id=1) with only the initial transaction
    if (row.job_status_id === 1 && row.transaction_count <= 1) return false;
    return true;
}
```

#### 3d. Change `isReadOnly` definition
...
- `isNoAction && !isAllView` → show dropdown with **only** Undo item
- `!isNoAction && !isAllView` → show dropdown with transitions + separator + Undo item

#### 3e. Revised Actions cell render
...
{isAllView ? (
    ...
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="min-w-[210px] ...">
            {/* Normal transitions — hidden for no-action rows */}
            ...
            {/* Separator + Undo — always when not isAllView */}
...
#### 3g. Undo confirmation dialog (inline in UpdateJobSection render)
...
- [ ] COMPLETED_OK / RETURN / DELIVERED_OK / DELIVERED_NOT_OK → dropdown shows ONLY Undo (no transitions)
