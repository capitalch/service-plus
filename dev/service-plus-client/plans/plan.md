...
### Final detail form
3. Full-screen form: job header, Division, GSTIN, parts grid, charges grid, Back-Calc
   target, "show parts in invoice" toggle, IGST toggle.
4. **Parts**: add via part-code lookup → cost/selling/GST/HSN auto-fill (master + markup%).
   Editing cost cascades selling = cost×(1+markup%). Non-GST division inflates cost to
   absorb supplier GST.
5. **Warranty job** → selling prices and GST rate force to 0; amount = 0.
