...
### Step 1 — Decide and park unrelated work
- **Your part:**
...
### Step 3 — Drop the old tables and settings (CL-3, §A2)
- **My part:** write `../service-plus-server/scripts/ew_cleanup.sql`.
...
### Step 4 — Regenerate the schema DDL and seeds (CL-4)
- **Your part:** run `pg_dump --schema-only` to overwrite
  `../service-plus-server/app/db/schema_dumps/service_plus_service.sql` (same command you
  normally use).
...
### Step 5 — Remove server code (CL-5, §A3)
- **My part:** every edit and deletion in §A3; run the §A3 grep gate; give you a `curl`
  that POSTs a webhook status with event code `EW`.
...
### Step 6 — Remove client code (CL-6, §A4)
...
### Step 7 — Help and old plan documents (CL-7, §A5)
...
### Step 8 — Smoke test the app (CL-8)
...
Rollback for Part A: `git revert` the CL-9 commit(s), and restore `demo1` from the CL-2 dump.
