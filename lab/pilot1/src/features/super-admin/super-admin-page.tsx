...
import { ArrowLeftIcon, ArrowRightIcon, BarChart3Icon, BuildingIcon, ClipboardListIcon, SettingsIcon, ShieldIcon } from "lucide-react";
        ...
        <div className="min-h-full p-8">
            ...
            >
                <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1">
                    <ShieldIcon className="h-3.5 w-3.5 text-indigo-600" />
                    <span className="text-xs font-medium text-indigo-700 uppercase tracking-wider">Super Admin · v1</span>
                </div>
                <h1 className="text-3xl font-bold bg-gradient-to-br from-indigo-700 via-violet-600 to-indigo-500 bg-clip-text text-transparent">
                    Administration Panel
                </h1>
                <p className="mt-2 text-sm text-slate-500">
                    Manage all aspects of the platform from this central hub.
                </p>
            </motion.div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {modules.map(({ description, gradient, icon: Icon, label, to }, i) => (
                    ...
                    >
                        <Link
                            className="group flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-indigo-200 hover:shadow-lg"
                            to={to}
                        >
